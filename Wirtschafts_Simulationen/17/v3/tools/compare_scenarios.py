#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Compare multiple scenarios with localized colorful UTF-8 art reports.
...
from qsim.utf8art import build_scenario_comparison_art, normalize_width
from qsim.i18n import normalize_language, t, PUBLIC_LANGUAGE_NAMES
...
def main(argv=None):
    p = argparse.ArgumentParser(description="Compare Q economy scenarios with localized UTF-8 art.")
    ...
    for i, scenario in enumerate(args.scenarios):
        if scenario not in SCENARIOS:
            print("%s: %s" % (t(language, "unknown_scenario"), scenario))
            continue
        ...
        sim.run(args.periods, progress=False, language=language, width=width)
        sim.save(out_dir, language=language, width=width)
        ...
        rows.append(row)
        print("✅ %-20s %s" % (scenario, t(language, "scenario_done")))
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(build_scenario_comparison_art(rows, markdown=True, ansi=False, language=language, width=width))
    ansi_path = os.path.join(args.out, "scenario_comparison_ansi.txt")
    with open(ansi_path, "w", encoding="utf-8") as f:
        f.write(build_scenario_comparison_art(rows, markdown=False, ansi=True, language=language, width=width))
    plain_path = os.path.join(args.out, "scenario_comparison_utf8.txt")
    with open(plain_path, "w", encoding="utf-8") as f:
        f.write(build_scenario_comparison_art(rows, markdown=False, ansi=False, language=language, width=width))
    print("\n" + build_scenario_comparison_art(rows, markdown=False, ansi=not args.no_color, language=language, width=width))
    print("\n%s:" % t(language, "comparison_saved"))
    print("  ", csv_path)
    print("  ", md_path)
    print("  ", plain_path)
    print("  ", ansi_path)
