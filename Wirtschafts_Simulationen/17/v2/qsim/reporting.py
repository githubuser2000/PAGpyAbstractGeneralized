...
def write_summary_md(sim, path):
    ...
    """
    return write_visual_report(path, sim, markdown=True, ansi=False)


def write_visual_ansi_txt(sim, path):
    return write_visual_report(path, sim, markdown=False, ansi=True)


def write_visual_plain_txt(sim, path):
    return write_visual_report(path, sim, markdown=False, ansi=False)
...
def write_reports(sim, out_dir):
    ...
    paths["visual_report"] = write_visual_report(os.path.join(out_dir, "visual_report.md"), sim, markdown=True, ansi=False)
    paths["visual_report_ansi"] = write_visual_ansi_txt(sim, os.path.join(out_dir, "visual_report_ansi.txt"))
    paths["dashboard_utf8"] = write_visual_plain_txt(sim, os.path.join(out_dir, "dashboard_utf8.txt"))
    config = {
        "seed": sim.seed,
