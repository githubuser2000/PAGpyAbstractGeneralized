...
class EconomySimulation(object):
    ...
    def run(self, periods=120, progress=False):
        periods = int(periods)
        for _ in range(periods):
            m = self.step()
            if progress and (self.period % max(1, periods // 10) == 0 or self.period == 1):
                print("Periode %d: BQP %.2f, Arbeitslosigkeit %.1f%%, Inflation %.2f%%, Schulden %.2f ZW" % (
                    self.period, m["bqp"], m["unemployment_rate"] * 100.0, m["inflation"] * 100.0, m["total_debt_value"]
                ))
    ...
    def save(self, out_dir):
        ...
        return write_reports(self, out_dir)
