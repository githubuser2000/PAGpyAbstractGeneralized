...
# -*- coding: utf-8 -*-
"""
Mega Economy Simulation with Tradeable Fuzzy-Truth Currency
==========================================================
...
class TruthCurrencyMarket:
    ...
    def production_throttle_for_firm(self, firm: Firm) -> float:
        ...
        local = clamp(0.55 + 0.28 * score + 0.20 * knowledge + 0.22 * model - 0.55 * overuse, 0.12, 1.25)
...
class EconomicWorld:
    ...
    def request_firm_loan(self, firm: Firm, amount: float, purpose: str = "working_capital", collateral: Optional[float] = None) -> bool:
        ...
        rate = self.central_bank.policy_rate + self.financial_market.credit_spread + 0.025 + 0.035 * leverage + 0.020 * tightness - 0.010 * truth_bonus
        profitability = safe_div(firm.last_profit, max(1.0, firm.last_revenue), 0.0)
        score = 1.2 * bank.risk_appetite + 0.35 * profitability + 0.45 * safe_div(collateral_value, amount, 0.0) - 0.55 * leverage - 1.3 * tightness + 0.34 * truth_bonus + 0.12 * bank_truth
        if purpose == "green_investment":
    ...
    def request_household_loan(self, household: Household, amount: float, purpose: str = "consumer", collateral: float = 0.0) -> bool:
        ...
        risk = 0.35 * household.risk_aversion + 0.45 * safe_div(total_debt, max(1.0, income * 12.0), 0.0) + 0.35 * (not household.employed) - 0.18 * truth_bonus
        collateral_score = safe_div(collateral, amount, 0.0)
        score = 1.0 * bank.risk_appetite + 0.25 * collateral_score + 0.18 * household.education - risk - 1.1 * tightness + 0.28 * truth_bonus
        approve_prob = logistic(score)
        if self.rng.random() < approve_prob:
            rate = self.central_bank.policy_rate + self.financial_market.credit_spread + (0.028 if purpose == "mortgage" else 0.075) + 0.025 * risk - 0.006 * truth_bonus
