# Vector Currency Economy Simulation V4
...
V3 erweiterte V2 um zwei Dinge:
...
V4 ergänzt darauf eine terminal-sichere Breitenerkennung: Das Dashboard erkennt die Terminalbreite automatisch, zieht 5 Zeichen Sicherheitsabstand ab und bricht Boxinhalte/Tabellen/Ereignisse ANSI- und UTF‑8-bewusst um. Rahmen werden nicht breiter als die erkannte sichere Bildschirmbreite.
...
- `vector_currency_sim_v4.py`: identische V4-Kopie mit explizitem Namen.
...
- `CHANGELOG_V4.md`: Was in V4 ergänzt wurde.
...
pypy3 vector_currency_sim.py --steps 24 --art

# enger als das Terminal, aber nie breiter als Terminalbreite - 5
pypy3 vector_currency_sim.py --steps 24 --art --art-width 80

# für Logdateien oder monochrome Terminals
pypy3 vector_currency_sim.py --steps 24 --art --no-color
```
