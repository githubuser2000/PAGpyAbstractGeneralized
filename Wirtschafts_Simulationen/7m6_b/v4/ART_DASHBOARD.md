# Erklärung des UTF‑8/ANSI-Dashboards mit terminal-sicherem Umbruch

Das Dashboard ist absichtlich bunt und dicht. Es soll nicht nur hübsch aussehen, sondern die zentralen Spannungen der Vektorwährung sichtbar machen. In V4 erkennt es die Terminalbreite automatisch, zieht 5 Zeichen Sicherheitsreserve ab und bricht alle zu langen Inhalte innerhalb der Boxen um. ANSI-Farben werden bei der Breitenmessung ignoriert, breite UTF‑8-Zeichen werden konservativ gezählt.
...
## 11. Terminalbreite, Sicherheitsabzug und Umbruch

V4 misst vor jeder Dashboardausgabe die verfügbare Terminalbreite. Daraus wird eine sichere Breite gebildet:

```text
sichere Breite = Terminalspalten - 5
```

Danach gilt für jedes Diagramm:

- Rahmenbreite höchstens sichere Breite
- Tabellenzeilen werden innerhalb der Box umgebrochen
- lange Ereignisbeschreibungen werden innerhalb der Box umgebrochen
- ANSI-Sequenzen zählen als 0 sichtbare Zeichen
- breite UTF‑8-Zeichen werden konservativ als 2 Terminalzellen gezählt
- `--art-width N` darf das Dashboard schmaler machen, aber nie breiter als die sichere Terminalbreite

Damit kann dieselbe Simulation in breiten Terminals, schmalen Terminals, SSH-Sessions und Logdateien laufen, ohne dass die Boxrahmen über den Bildschirm hinauslaufen.
