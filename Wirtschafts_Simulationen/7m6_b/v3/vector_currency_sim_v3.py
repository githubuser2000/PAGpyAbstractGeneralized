...
def _hr(title: str, width: int = 118, color: bool = True, color_name: str = "bright_cyan") -> str:
    inner = f" {title} "
    left = max(2, (width - _visible_len(inner)) // 2)
    right = max(2, width - _visible_len(inner) - left)
    return _c("═" * left, color_name, color, bold=True) + _c(inner, "white", color, bold=True) + _c("═" * right, color_name, color, bold=True)


def _box(title: str, lines: List[str], color: bool = True, border_color: str = "bright_blue") -> List[str]:
    raw_width = max([_visible_len(title) + 4] + [_visible_len(x) for x in lines])
    top = _c("╭" + "─" * (raw_width + 2) + "╮", border_color, color)
    bot = _c("╰" + "─" * (raw_width + 2) + "╯", border_color, color)
    out = [top, _c("│ ", border_color, color) + _c(title, "white", color, bold=True) + " " * (raw_width - _visible_len(title)) + _c(" │", border_color, color)]
...
def render_art_dashboard(sim: VectorEconomySim, row: Dict[str, Any], width: int = 118, color: bool = True, final: bool = False) -> str:
    width = max(90, min(180, int(width)))
    title = "V3 UTF-8/ANSI Dashboard — Vektorwährung: Wert × Gutartigkeit × Beliebtheit"
    lines: List[str] = []
    lines.append(_hr(title, width, color, "bright_cyan"))
    if final:
        lines.append(_c("FINALER SNAPSHOT", "bright_yellow", color, True))
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--art", action="store_true", help="print colorful UTF-8/ANSI dashboard diagrams during the run")
    p.add_argument("--art-every", type=int, default=12, help="print art dashboard every N steps when --art is enabled")
    p.add_argument("--art-width", type=int, default=118, help="target width for art dashboard separators")
