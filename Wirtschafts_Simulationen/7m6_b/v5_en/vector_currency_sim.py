...
macro metrics. The extended version adds explicit holding-company graphs, securities and bond
...
def _angle_quadrant(theta: float) -> Tuple[str, str]:
    ...
    if g >= 0.0 and p >= 0.0:
        return "good + popular", "bright_green"
    if g >= 0.0 and p < 0.0:
        return "good + unpopular", "yellow"
    if g < 0.0 and p >= 0.0:
        return "bad + popular", "bright_magenta"
    return "bad + unpopular", "bright_red"
...
def _ascii_compass(theta: float, buy: Optional[float] = None, sell: Optional[float] = None, cash: Optional[float] = None, color: bool = True) -> List[str]:
    ...
    if buy is not None:
        points.append((buy, "B", "bright_green"))
    if sell is not None:
        points.append((sell, "S", "bright_red"))
    if cash is not None:
    ...
    for y in range(h):
        ...
        for x in range(w):
            if (x, y) in colored:
                ...
                elif ch in "good +badpopularun-":
                    row.append(_c(ch, "white", color))
                else:
                    row.append(ch)
        lines.append("".join(row))
    lines.append("Legend: " + _c("◎ World angle", _angle_color(theta), color, True) + "  " + _c("¤ Cash angle", "bright_yellow", color, True) + "  " + _c("B Buy angle", "bright_green", color, True) + "  " + _c("S Sell angle", "bright_red", color, True))
    return lines
...
def _render_macro_panel(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = [
        f"t={int(_val(row, 't')):4d}  GDP={_c(_fmt_num(_val(row, 'gdp')), 'bright_green', color, True)}  Money supply={_c(_fmt_num(_val(row, 'money_supply')), 'bright_yellow', color, True)}  Inflation={_c(_fmt_pct(_val(row, 'avg_inflation'), signed=True), 'yellow' if _val(row,'avg_inflation') >= 0 else 'cyan', color, True)}  Unemployment={_c(_fmt_pct(_val(row, 'unemployment')), 'bright_red' if _val(row, 'unemployment') > 0.12 else 'green', color, True)}",
        f"Confidence    {_bar01(_val(row, 'mean_cash_confidence'), 28, 'bright_cyan', color)} {_val(row, 'mean_cash_confidence'):.3f}",
        f"Goodness  {_signed_bar(_val(row, 'avg_goodness_axis'), 31, 'bright_green', 'bright_red', color)} {_val(row, 'avg_goodness_axis'):+.3f}",
        f"Popularity   {_signed_bar(_val(row, 'avg_popularity_axis'), 31, 'bright_magenta', 'yellow', color)} {_val(row, 'avg_popularity_axis'):+.3f}",
        f"Legitimacy   {_bar01(_val(row, 'avg_legitimacy'), 28, 'green', color)} {_val(row, 'avg_legitimacy'):.3f}   Legitimacy gap={_val(row, 'legitimacy_gap'):.3f}",
        f"Inequality  Households={_val(row, 'household_gini'):.3f} Firms={_val(row, 'firm_cash_gini'):.3f}  angle spread Ø={deg(_val(row, 'avg_angle_spread')):.1f}°",
        f"GDP   {gdp_spark}",
        f"Money {money_spark}",
        f"Conf  {conf_spark}",
        f"Good  {good_spark}",
    ...
    return _box("Macro cockpit: value, direction, confidence", lines, color, "bright_cyan")
...
def _render_vector_compass(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = []
    lines.append(f"World money angle θ={_angle_label(world_theta, color)}   concentration R={_val(row, 'world_money_concentration'):.3f}")
    lines.append(f"representative: Cash={deg(cash_theta):6.1f}°  Buy={deg(buy_theta):6.1f}°  Sell={deg(sell_theta):6.1f}°  B↔S distance={deg(angle_dist(buy_theta, sell_theta)):5.1f}°")
    lines.extend(_ascii_compass(world_theta, buy_theta, sell_theta, cash_theta, color))
    lines.append("Axes: x=good/bad set by governments+courts, y=popular/unpopular set by peoples+media.")
    return _box("Vector compass of the currency", lines, color, "bright_magenta")


def _render_triadic_exchange(row: Dict[str, Any], color: bool) -> List[str]:
    flows = [
        ("💰 Value", "⚖ Goodness", _val(row, "value_buy_goodness_volume"), "bright_green", "Compliance, audits, environment, labor safety"),
        ("💰 Value", "♥ Popularity", _val(row, "value_buy_popularity_volume"), "bright_magenta", "Advertising, service, community, media"),
        ("♥ Popularity", "⚖ Goodness", _val(row, "popularity_buy_goodness_volume"), "cyan", "social mandate lowers reform costs"),
        ("⚖ Goodness", "♥ Popularity", _val(row, "goodness_buy_popularity_volume"), "yellow", "credible conduct becomes visible"),
    ]
    ...
    lines.append("")
    lines.append(f"Triadic exchange fees / friction: {_c(_fmt_num(_val(row, 'value_good_pop_exchange_fees')), 'bright_red', color, True)}   Trades: {int(_val(row, 'triadic_exchange_count'))}   angle-laundering index: {_val(row, 'laundering_index'):.4f}")
    lines.append("Core claim: goodness can be bought only through auditable costs plus social credibility; pure popularity without substance lowers confidence.")
    lines.append(f"Live formula: M = m·e^(iθ), θ=atan2(popularity, goodness), purchasing power≈m·ρ·cos(Δθ/2)")
    return _box("Triadic trade: value ↔ goodness ↔ popularity", lines, color, "bright_green")


def _render_market_flows(row: Dict[str, Any], color: bool) -> List[str]:
    flow_keys = [
        ("Households → Firms", "consumer_sales", "bright_green"),
        ("Firms → Firms", "b2b_sales", "cyan"),
        ("Firms → Labor", "wages", "yellow"),
        ("Private sector → State", "taxes", "bright_blue"),
        ("State → Economy", "subsidies", "green"),
        ("Banks → Credit", "loans_issued", "bright_magenta"),
        ("International", "trade_volume", "blue"),
        ("Angle rotation", "angle_rotation_volume", "bright_yellow"),
        ("Black market", "black_market_volume", "bright_red"),
        ("Capital flow/FX", "capital_flow_volume", "magenta"),
    ]
    maxv = max([_val(row, k) for _, k, _ in flow_keys] + [1.0])
    lines = []
    lines.append(_c("Households", "bright_green", color, True) + " ═Consumption═▶ " + _c("Firms", "cyan", color, True) + " ═wages═▶ " + _c("Households", "bright_green", color, True) + "   │   " + _c("Taxes/Subsidies", "bright_blue", color, True) + "   │   " + _c("Banks/Credit", "magenta", color, True))
    lines.append(_c("Firms", "cyan", color, True) + " ═B2B/Supply chains═▶ " + _c("Firms", "cyan", color, True) + " ═Trade═▶ " + _c("Foreign sector", "blue", color, True) + " ═FX═▶ " + _c("Reserves/Central bank", "bright_yellow", color, True))
    lines.append(_c("All actors", "white", color, True) + " ═Buy angle/Sell angle═▶ " + _c("angle market", "bright_yellow", color, True) + " ═Rotation/friction═▶ " + _c("new cash angle", "bright_green", color, True))
    lines.append("")
    for label, key, col in flow_keys:
        lines.append(f"{_pad(label, 22)} {_bar(_val(row, key), maxv, 42, col, color)} {_fmt_num(_val(row, key))}")
    return _box("Market-flow diagram of the simulated sections", lines, color, "yellow")
...
def _render_angle_orderbook(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = []
    lines.append("Every actor quotes two directions: " + _c("Buy angle B", "bright_green", color, True) + " and " + _c("Sell angle S", "bright_red", color, True) + ". The amount/cash balance weights order depth.")
    lines.append("         Buy-bid depth                         angle zone                         Sell-ask depth")
    for i in range(bins):
    ...
    lines.append(f"Ø angle spread: {_c(f'{deg(_val(row, 'avg_angle_spread')):.1f}°', 'bright_yellow', color, True)}   rotation costs: {_c(_fmt_num(_val(row, 'angle_rotation_cost')), 'bright_red', color, True)}   rotation volume: {_fmt_num(_val(row, 'angle_rotation_volume'))}")
    return _box("Circular order book: angle trading", lines, color, "bright_yellow")
...
def _render_actor_angles(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines = []
    header = f"{'Type':<4} {'Name':<24} {'Value':>10} {'Cashθ':>8} {'Buyθ':>8} {'Sellθ':>9} {'Spread':>8} {'ρ':>5}  Direction"
    ...
    lines.append("B=what this actor accepts/wants to buy; S=which angle quality this actor demands when selling.")
    return _box("Actors with two angles: buying and selling", lines, color, "bright_green")
...
def _render_country_panel(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    for c in countries:
        ...
        line1 = f"{_c(_short(c.name,12), 'white', color, True):<20} GDP {_bar(gdp, max_gdp, 18, 'bright_green', color)} {_fmt_num(gdp):>8}  FX={c.exchange_rate:5.3f}  θ={_c(f'{deg(c.currency_theta):6.1f}°', col, color, True)} {q}"
        line2 = f"    Legitimacy {_bar01(leg, 14, 'green', color)} {leg:.2f}  Unemp={_fmt_pct(unemp)}  Infl={_fmt_pct(infl, signed=True)}  Protest={c.protests:.2f}  Infra={c.infrastructure_quality:.2f}  Bio={c.biodiversity:.2f}  Crime={c.crime_index:.2f}  Courts={gov.court_independence:.2f}"
        lines.append(line1)
        lines.append(line2)
    return _box("Countries/jurisdictions: macro state, legitimacy, currency angle", lines, color, "bright_blue")
...
def _render_firm_quadrant_map(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines.append("x-axis: government/court goodness ← bad | good →   y-axis: people/media popularity ↑ popular | unpopular ↓")
    lines.append("Points are firms; size/symbol ≈ value weight. This is where profit, morality, and popularity collide.")
    return _box("Firm map: goodness × popularity, weighted by value", lines, color, "bright_magenta")


def _render_externalities(row: Dict[str, Any], color: bool) -> List[str]:
    items = [
        ("Pollution", _val(row, "avg_pollution"), 2.0, "bright_red"),
        ("Health burden", _val(row, "avg_health_burden"), 1.5, "yellow"),
        ("Biodiversity", _val(row, "avg_biodiversity"), 1.0, "bright_green"),
        ("Crime index", _val(row, "avg_crime_index"), 1.0, "red"),
        ("Privacy damage", _val(row, "avg_privacy_damage"), 1.0, "magenta"),
        ("Infrastructure quality", _val(row, "avg_infrastructure_quality"), 1.0, "cyan"),
    ...
    lines.append(f"Shocks: Scandals={int(_val(row, 'scandals'))}, Strikes={int(_val(row, 'strikes'))}, Defaults={int(_val(row, 'firms_defaulted'))}, Overrides={int(_val(row, 'constitutional_overrides'))}")
    return _box("Externalities and political safety valves", lines, color, "bright_red")
...
def _render_events(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines = []
    if not events:
        lines.append(_c("No events yet.", "dim", color))
    for e in events:
    ...
    return _box("Event tape: shocks, scandals, courts, politics", lines, color, "white")



# ----------------------------------------------------------------------
# V6 extra dense UTF-8 / ANSI art renderers
# ----------------------------------------------------------------------
...
def _money_groups(sim: VectorEconomySim) -> List[Tuple[str, str, List[ActorBase]]]:
    return [
        ("HH", "Households", [h for h in sim.households.values() if h.active]),
        ("F", "Firms", [f for f in sim.firms.values() if f.active]),
        ("B", "Banks", [b for b in sim.banks.values() if b.active]),
        ("FND", "Funds", [f for f in sim.investment_funds.values() if f.active]),
        ("GOV", "Governments", [g for g in sim.governments.values() if g.active]),
        ("CB", "Central banks", [cb for cb in sim.central_banks.values() if cb.active]),
    ]
...
def _render_dashboard_legend(color: bool) -> List[str]:
    lines = [
        _c("██", "bright_green", color) + " good/popular or positive value   " + _c("██", "bright_red", color) + " risk, badness, loss, or crisis   " + _c("██", "bright_magenta", color) + " popularity/media power   " + _c("██", "bright_yellow", color) + " money/angle rotation",
        "UTF-8 diagrams are intentionally redundant: the same economy is shown as money flow, angle market, oracle conflict, sector structure, financial system, risk, and power map.",
        "All boxes remain capped at detected terminal width minus 5 characters; long diagrams are hard-wrapped instead of spilling past the screen.",
    ]
    return _box("V6 art legend: very colorful multi-view of the same vector economy", lines, color, "bright_cyan")
...
def _render_money_mass_river(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines.append("Money-mass flow band: " + " ".join(river_parts))
    lines.append("")
    for g in groups:
        col = _angle_color(g["theta"])
    ...
    lines.append("Interpretation: the amount is weight; the angle shows the moral-social direction of each money mass.")
    return _box("Money-mass river: who holds how much directed money?", lines, color, "bright_yellow")
...
def _render_class_vector_balance(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = []
    lines.append(_c("x-component", "bright_green", color, True) + " = goodness/badness of held purchasing power; " + _c("y-component", "bright_magenta", color, True) + " = popularity/unpopularity.")
    for g in groups:
        xn = g["x"] / maxcomp
        yn = g["y"] / maxcomp
        xlab = "good" if g["x"] >= 0 else "bad"
        ylab = "popular" if g["y"] >= 0 else "unpopular"
    ...
    lines.append("When x and y move against each other, the system is not simply poor/rich but normatively split.")
    return _box("Vector balance by actor class: value decomposed into goodness and popularity", lines, color, "bright_green")
...
def _render_angle_liquidity_wheel(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = [
        "0° good→ | 90° popular↑ | 180° bad← | 270° unpopular↓ | back to 360°",
        "Cash θ   " + _mini_heat_line(cash_bins, bins, "bright_yellow", color),
        "Buy θ   " + _mini_heat_line(buy_bins, bins, "bright_green", color),
        "Sell θ " + _mini_heat_line(sell_bins, bins, "bright_red", color),
        "B/S Gap  " + _mini_heat_line(spread_bins, bins, "bright_magenta", color),
        "          " + _c("0        90        180       270       360", "white", color),
        "Bright blocks = high liquidity/weight in the respective angle segment; B/S gap shows missing opposite-side liquidity.",
    ]
    return _box("360° angle liquidity wheel: cash, buy angle, sell angle, spread", lines, color, "bright_magenta")
...
def _render_execution_ladder(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines = []
    lines.append(_c("Execution idea:", "white", color, True) + " buyer buy-angle B meets seller sell-angle S; compatible value ≈ min(Cash)·ρ·cos(Δθ/2).")
    header = f"{'Buyer B':<18} {'Seller S':<18} {'Δθ':>7} {'Compat.':>8} {'eff.Value':>10}  Bridge"
    lines.append(_c(header, "white", color, True))
    ...
    lines.append("This is the concrete trade of the three things: value is fully effective only when buy angle and sell angle are close enough.")
    return _box("Order execution ladder: two angles per actor in trade contact", lines, color, "bright_yellow")
...
def _render_sector_heatmap(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines = []
    lines.append(f"{'Sector':<15} {'Value weight':<28} {'Good':<27} {'Pop':<27} {'ρ':>4} θ")
    ...
    lines.append("Sector rows show value mass, the good/bad axis, the popular/unpopular axis, and the resulting angle.")
    return _box("Sector heatmap: Value × goodness × popularity", lines, color, "bright_green")
...
def _render_sector_economics(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines = []
    lines.append(f"{'Sector':<15} {'Labor':<22} {'Price':>8} {'Wage':>8} {'Profit':<24} {'Tech×Prod':>9}")
    ...
    lines.append("This box makes the real-economic body behind the angles visible: labor, prices, wages, profit, technology.")
    return _box("Sector economy: labor market, prices, wages, productivity", lines, color, "cyan")


def _render_oracle_divergence(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    lines = []
    lines.append("G=government/court angle, P=weighted people ideology, W=currency angle. Large G↔P distance = legitimacy risk.")
    for c in sim.countries.values():
        ...
        lines.append(f"{_short(c.name,12):<13} ΔG/P={_c(f'{deg(d_gp):5.1f}°', col, color, True)} ΔW/center={deg(d_wc):5.1f}°  Sat={avg_sat:+.2f} Anger={avg_anger:.2f} Pol={c.polarization:.2f}  {markers}")
    return _box("Oracle divergence: governments define good/bad, peoples define popular/unpopular", lines, color, "bright_blue")
...
def _render_confidence_spectrum(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    for i, v in enumerate(bins):
        ...
        lines.append(f"ρ {lo:.1f}-{hi:.1f} {_bar(v, maxv, 42, col, color)} {_fmt_num(v):>9} Actors={counts[i]}")
    lines.append(f"Mean confidence: {_val(row, 'mean_cash_confidence'):.3f}; low ρ creates angle inflation: everything claims to be good, but nobody fully believes it.")
    return _box("Confidence spectrum of money angles", lines, color, "bright_cyan")
...
def _render_spread_histogram(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    for i, label in enumerate(labels):
        ...
        lines.append(f"{label:<9} {_bar(bins[i], maxv, 48, col, color)} {_fmt_num(bins[i]):>9} Actors={counts[i]}")
    lines.append("Small spread = high angle liquidity. Large spread = moral-social market split and costly rotation.")
    return _box("Buy/sell angle spread histogram", lines, color, "bright_yellow")
...
def _render_value_good_pop_triangle(row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = [
        "                         " + _c("⚖ GOODNESS", "bright_green", color, True),
        "                       ╱" + _bar(pg, mx, 14, "cyan", color, "━") + "╲",
        "                      ╱  Pop→Good " + _fmt_num(pg) + "  ╲",
        _c("💰 VALUE", "bright_yellow", color, True) + " " + _bar(vg, mx, 18, "bright_green", color, "━") + "▶" + " " * 8 + "◀" + _bar(gp, mx, 18, "yellow", color, "━") + " " + _c("♥ POPULARITY", "bright_magenta", color, True),
        "  Value→Good " + _fmt_num(vg) + "        Good→Pop " + _fmt_num(gp),
        "                       ╲" + _bar(vp, mx, 14, "bright_magenta", color, "━") + "╱",
        "                        Value→Pop " + _fmt_num(vp),
        f"Friction/fees: {_bar(fees, mx, 32, 'bright_red', color)} {_fmt_num(fees)}   Trades={int(_val(row, 'triadic_exchange_count'))}",
        "This triangle shows the politically dangerous question directly: can one buy goodness with value and popularity — or only earn it credibly?",
    ]
    return _box("Triangle exchange: value, goodness, popularity", lines, color, "bright_magenta")


def _render_financial_system_map(row: Dict[str, Any], color: bool) -> List[str]:
    flows = [
        ("New loans", "loans_issued", "bright_magenta"),
        ...
        ("Dividends", "dividends", "yellow"),
        ("Mortgages", "mortgages_issued", "cyan"),
        ("Insurance premiums", "insurance_premiums", "bright_cyan"),
        ("Insurance claims", "insurance_claims", "bright_red"),
        ("FX Orderbook", "fx_orderbook_volume", "blue"),
        ("Reserve intervention", "reserve_intervention", "bright_yellow"),
    ]
    maxv = max([_val(row, key) for _, key, _ in flows] + [1.0])
    lines = [
        _c("Households", "bright_green", color, True) + " ⇄ " + _c("Banks", "bright_magenta", color, True) + " ⇄ " + _c("Firms", "cyan", color, True) + " ⇄ " + _c("Exchanges", "bright_blue", color, True) + " ⇄ " + _c("Funds/Pensions", "green", color, True) + " ⇄ " + _c("State/Central bank", "bright_yellow", color, True),
        "Financial markets trade not only risk and time, but also angle risk, confidence, and political revaluation.",
        "",
    ]
    for label, key, col in flows:
        lines.append(f"{_pad(label, 22)} {_bar(_val(row, key), maxv, 42, col, color)} {_fmt_num(_val(row, key))}")
    return _box("Financial-system map: credit, bonds, equities, mortgages, insurance, FX", lines, color, "bright_blue")
...
def _render_corporate_network_panel(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = []
    if not groups:
        lines.append(_c("No corporate structures active.", "dim", color))
    else:
        lines.append(f"{'Group':<20} {'Subs':>4} {'Opacity':<20} {'TaxHaven':>8} {'θ':>8} {'ρ':>4} Parent → Subsidiaries")
        for g in groups:
    ...
    lines.append(f"Transfer Pricing: {_c(_fmt_num(_val(row, 'transfer_pricing_volume')), 'bright_red', color, True)}   avoided taxes: {_c(_fmt_num(_val(row, 'tax_avoided')), 'bright_red', color, True)}")
    lines.append("Corporate opacity is an angle attack surface: value, goodness, and provenance can be pulled apart.")
    return _box("Corporate/holding network: power, opacity, tax arbitrage", lines, color, "bright_red")
...
def _render_supply_chain_matrix(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines = []
    lines.append("Every final good carries angles from intermediate inputs: energy, data, raw materials, media, software, transport, etc.")
    for sector in sectors:
        spec = GOOD_SPECS[sector]
        ...
        lines.append(f"{_short(sector,14):<15} ← {' '.join(pieces) if pieces else _c('no major inputs', 'dim', color)}")
        lines.append(f"    Environment {_bar(spec.environmental_harm, 1.0, 18, harm_col, color)} {spec.environmental_harm:.2f}  social benefit {_bar(clamp(spec.social_good, -0.2, 1.0)+0.2, 1.2, 18, social_col, color)} {spec.social_good:+.2f}  angle sensitivity={spec.angle_sensitivity:.2f}")
    return _box("Supply-chain matrix: where a product angle comes from", lines, color, "cyan")


def _render_institution_safety_board(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    lines = []
    lines.append(f"Overrides={int(_val(row, 'constitutional_overrides'))}  contract disputes={int(_val(row, 'contract_disputes'))}  Minority-Harm={_val(row, 'minority_harm_index'):.4f}")
    lines.append(f"{'Country':<13} {'Courts':<16} {'Constitution':<16} {'Minority':<16} {'Corruption':<16} {'Capture':<16} {'Propaganda'}")
    for c in sim.countries.values():
        gov = sim.governments[c.government_id]
    ...
    lines.append("This safeguard layer prevents angle money from becoming mere political obedience money.")
    return _box("Constitutional/court protection against angle abuse", lines, color, "white")
...
def _render_crisis_seismograph(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    keys = [
        ...
        ("Unemployment", "unemployment", "bright_red"),
        ("Legitimacy gap", "legitimacy_gap", "bright_magenta"),
        ("Angle volatility", "angle_volatility", "bright_yellow"),
        ("Angle laundering", "laundering_index", "bright_red"),
        ("Black Market", "black_market_volume", "red"),
        ("Capital flows", "capital_flow_volume", "blue"),
        ("Pollution Δ", "pollution_delta", "bright_red"),
    ]
    lines = []
    for label, key, col in keys:
        vals = [_val(r, key) for r in hist]
        lines.append(f"{_pad(label, 18)} {_spark(vals, 44, col, color)} now={_fmt_num(_val(row, key), 4)}")
    lines.append("Seismographs show regime shifts rather than calibration: panic, mistrust, rotation, environmental/political shocks.")
    return _box("Crisis seismograph: early signals over time", lines, color, "bright_red")
...
def _render_demography_human_capital(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    if not households:
        return _box("Demography and human capital", [_c("No active households.", "dim", color)], color, "green")
    ...
    for label, n, hc, health in counts:
        lines.append(f"Age {label:<5} {_bar(n, maxc, 32, 'bright_green', color)} n={n:<5} HumanCapital={hc:.2f} Health={health:.2f}")
    lines.append(f"Migration in this step: {int(_val(row, 'migration_count'))}; Human capital changes productivity, wages, political patience, and angle preferences.")
    return _box("Demography & human capital: labor-force body of the simulation", lines, color, "bright_green")
...
def _render_media_power_map(sim: VectorEconomySim, color: bool) -> List[str]:
    ...
    lines = []
    lines.append("Media/brand power moves popularity faster than real goodness; this creates populism and angle-laundering paths.")
    for f in firms:
        ...
        lines.append(f"{_short(f.name,22):<23} {_short(f.sector,12):<13} Media {_bar(v, maxv, 24, 'bright_magenta', color)} Pop={_signed_bar(pop, 17, 'bright_magenta', 'yellow', color)} Brand={f.brand_capital:.2f} Ads={_fmt_num(f.advertising_budget)}")
    return _box("Media/popularity power: who can move the y-angle?", lines, color, "bright_magenta")
...
def _render_rating_market(sim: VectorEconomySim, row: Dict[str, Any], color: bool) -> List[str]:
    ...
    lines = []
    lines.append(f"Rating actions in this step: {int(_val(row, 'rating_actions'))}; Rating agencies are additional, fallible oracles beside government and people.")
    for a in agencies[:8]:
        col = _angle_color(a.bias_theta)
        lines.append(f"Agency {_short(a.name,14):<15} bias θ={_c(f'{deg(a.bias_theta):6.1f}°', col, color, True)} Accuracy={a.accuracy:.2f} Corruption={a.corruption:.2f} Influence={a.influence:.2f}")
    if firms:
        lines.append(_c("Notable firm ratings:", "white", color, True))
    for f in firms:
        goodness = f.current_goodness_estimate()
        lines.append(f"{_short(f.name,20):<21} Rating {_bar01(f.rating, 14, 'bright_blue', color)} {f.rating:.2f}  ρ={f.reputation_confidence:.2f}  Good={goodness:+.2f}  Fraud={f.fraud_level:.2f}")
    return _box("Rating market: third oracle, creditworthiness, and angle confidence", lines, color, "bright_blue")
...
def render_art_dashboard(sim: VectorEconomySim, row: Dict[str, Any], width: int = 0, color: bool = True, final: bool = False) -> str:
    ...
    title = "V6 EXTREME UTF-8/ANSI Dashboard — Vector currency: value × goodness × popularity"
    lines: List[str] = []
    lines.append(_hr(title, width, color, "bright_cyan"))
    if final:
        lines.extend(_wrap_ansi_line(_c("FINAL SNAPSHOT", "bright_yellow", color, True), width))
    ...
    lines.append(_hr("End of dashboard — CSV/JSON contain the same metrics in machine-readable form", width, color, "bright_cyan"))
