    ...
    def update(self, world: "EconomicWorld") -> None:
        ...
        if digital_firms and fee_pool > 0:
            ...
            for f, w in zip(digital_firms, weights):
                f.cash += fee_pool * w / total
                f.last_revenue += fee_pool * w / total
...
class EconomicWorld:
    ...
    def request_firm_loan(self, firm: Firm, amount: float, purpose: str = "working_capital", collateral: Optional[float] = None) -> bool:
        ...
        score = 1.2 * bank.risk_appetite + 0.35 * profitability + 0.45 * safe_div(collateral_value, amount, 0.0) - 0.55 * leverage - 1.3 * tightness
    ...
    def request_household_loan(self, household: Household, amount: float, purpose: str = "consumer", collateral: float = 0.0) -> bool:
        ...
        score = 1.0 * bank.risk_appetite + 0.25 * collateral_score + 0.18 * household.education - risk - 1.1 * tightness
        approve_prob = logistic(score)
        if self.rng.random() < approve_prob:
    ...
    def _production(self) -> None:
        ...
        # Households also demand direct energy.
        household_energy_demand = sum((1.0 + 0.30 * h.children) * h.energy_efficiency for h in self.households) * 0.045
    ...
    def _goods_and_services_market(self) -> None:
        ...
        for sector in TRADABLE_SECTORS:
            ...
            sector_spending[sector] += export_qty * self.sector_price_index.get(sector, 1.0)
            self.step_exports += export_qty * self.sector_price_index.get(sector, 1.0)
