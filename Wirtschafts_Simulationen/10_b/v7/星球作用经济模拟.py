...
def art_buy_sell_replacement() -> None:
    ...
    rows = [
        ("买入", "Bedarfswirkung annehmen", "need_acceptance", (255, 106, 188)),
...
def art_difference_funnel(flow: Optional[EffectFlow]) -> None:
    ...
    terminal_print(color_text("     ╲       差异 wird          ╱", fg=fg, bold=True))
    terminal_print(color_text("      ╲      作用svertrag        ╱", fg=fg, bold=True))
    terminal_print(color_text("       ╲═══════════▼═══════════════╱", fg=fg, bold=True))
    terminal_print(color_text("             Handlung / 作用", fg=(38,222,129), bold=True))
...
def art_governance_feedback(last: GlobalMetrics) -> None:
    ...
    terminal_print(color_text("       ▲                                                           │", fg=fg, bold=True))
    terminal_print(color_text("       └──────────── Audit ◀── 错误检查 ◀── 现象 ◀──────┘", fg=fg, bold=True))
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument('--显示图画', dest="show_art", type=int, default=30, help='统一码图画数量')
