...
def print_dimension_guide_terminal() -> None:
    terminal_rule("DIMENSIONEN DES HANDELNS")
    print("Skala: 0 = nicht vorhanden/falsch | 1 = schwach/latent | 2 = teilweise | 3 = stark | 4 = kritisch/hoch real")
    print("Stapel-Reihenfolge: K Z I E P W S M D B Ph R")
    print("Der Stapel ist keine Geldzahl. Er ist die Vertrags- und Zustands-Signatur eines Wirkungsflusses.")
    print("")
    for dim in TRUTH_DIMS:
        g = DIMENSION_GUIDE_DE[dim]
        print("%s = %s" % (g["short"], g["name_de"]))
        print("  Frage:   %s" % g["question"])
        print("  Vertrag: %s" % g["contract_role"])
        print("  Ersetzt: %s" % g["economic_replacement"])
    print("")


def print_trade_catalog_terminal(limit: int = 0) -> None:
    terminal_rule("WAS GEHANDELT WIRD: PRODUKTE, ARBEITSPLÄTZE, DIENSTLEISTUNGEN, ÖKOLOGIE, KLIMA")
    ...
    for domain in domains:
        item = TRADE_CATALOG[domain]
        print("[%s]" % domain)
        print("  Gehandelte Wirkung: %s" % terminal_shorten(item["trade_object"], 170))
        print("  Gemeint als:         %s" % terminal_shorten(item["meant_as"], 170))
        print("  Produkte:            %s" % terminal_shorten(item["products"], 170))
        print("  Arbeitsplätze:       %s" % terminal_shorten(item["workplaces"], 170))
        print("  Dienstleistungen:    %s" % terminal_shorten(item["services"], 170))
        print("  Ökologie:            %s" % terminal_shorten(item["ecology"], 170))
        print("  Klima:               %s" % terminal_shorten(item["climate"], 170))
    print("")
...
def print_visible_trade_contracts(flows: List[EffectFlow], limit: int = 16, detail: bool = False) -> None:
    ...
    terminal_rule("SICHTBARES HANDELN IN DIMENSIONEN")
    if not selected:
        print("Keine Wirkungsflüsse vorhanden. Erhöhe --steps oder deaktiviere --show-trades 0 nicht.")
        print("")
        return
    print("Alte Form: Ware + Menge + Preis + Eigentum → kaufen/verkaufen/importieren/exportieren")
    print("Neue Form: Kausalität + Zeit + Intensität + Existenz + Potenzen + Wirkungen + Substanz + Materie + Differenz + Bestimmung + Phänomene + Winkelrichtung → Wirkungsvertrag")
    print("Anzeige: jeder Eintrag ist ein realer Wirkungsfluss, keine Geldtransaktion. activated_effect ist Wirkungseinheit, nicht Wert.")
    print("")
    for idx, flow in enumerate(selected, 1):
        ...
        terminal_rule("VERTRAG %03d / %s / %s" % (idx, flow.domain.upper(), validity_de.upper()), width=118)
        print("Art:              %s" % flow.kind)
        print("Ersetzt früher:   %s" % flow.legacy_term_replaced)
        print("Handlung:         %s" % flow.action)
        print("Von → Zu:         %s/%s → %s/%s" % (flow.from_region, flow.from_commune, flow.to_region, flow.to_commune))
        print("Sektor:           %s" % flow.sector)
        print("Aktivierte Wirkung: %.3f" % flow.activated_effect)
        print("Kausale Kette:    %s" % terminal_shorten(flow.causal_link, 170))
        print("Winkelrichtung:   %s" % terminal_shorten(flow.direction_vector, 170))
        print("Wahrheitsstapel:  %s" % flow.truth_stack_base5)
        print("Score 0..4:       %.3f | Priorität 0..1: %.3f" % (flow.truth_stack_score_0_4, flow.truth_stack_priority_0_1))
        print("Dimensionen:      %s" % flow.truth_stack_compact)
        print("Deutung:          %s" % flow.dimension_meaning)
        print("Was gehandelt:    %s" % terminal_shorten(flow.trade_object, 190))
        print("Gemeint als:      %s" % terminal_shorten(flow.meant_as, 190))
        if detail:
            print("Produkte:         %s" % terminal_shorten(flow.product_examples, 210))
            print("Arbeitsplätze:    %s" % terminal_shorten(flow.workplace_examples, 210))
            print("Dienstleistungen: %s" % terminal_shorten(flow.service_examples, 210))
            print("Ökologiebedingung:%s" % terminal_shorten(" " + flow.ecological_clause, 210))
            print("Klimabedingung:   %s" % terminal_shorten(flow.climate_clause, 210))
            print("Vertragsbedingungen:")
            for cond in sentence_items(flow.contract_conditions)[:12]:
                print("  - %s" % terminal_shorten(cond, 210))
        else:
            print("Bedingungen:      %s" % terminal_shorten(flow.contract_conditions, 260))
        print("")


def print_truth_stack_explanation_terminal() -> None:
    terminal_rule("GESTAPELTER WAHRHEITSWERT: BEZUG ZU DEN DIMENSIONEN")
    print("Beispiel: truth_stack_base5 = 334434234343")
    print("Lesung:   K=3 Z=3 I=4 E=4 P=3 W=4 S=2 M=3 D=4 B=3 Ph=4 R=3")
    print("Bedeutung:")
    print("  Kausalität=3: Ursache ist gut getroffen.")
    print("  Zeit=3: dringlich.")
    print("  Intensität=4: sehr starkes Phänomen.")
    print("  Existenz=4: real vorhanden, nicht nur behauptet.")
    print("  Potenzen=3: Lösungsmöglichkeiten existieren.")
    print("  Wirkungen=4: hoher positiver Systemeffekt.")
    print("  Substanz=2: Stoffe/Energie/Wissen sind nur teilweise verfügbar.")
    print("  Materie=3: Ort/Infrastruktur sind ausreichend erreichbar.")
    print("  Differenz=4: Bedarfslücke ist maximal sichtbar.")
    print("  Bestimmung=3: sozial/demokratisch gut bestimmt.")
    print("  Phänomene=4: stark sichtbar/gemeldet/gemessen.")
    print("  Winkelrichtung=3: Handlung wirkt eher regenerativ/freiheitlich.")
    print("")
    print("Wichtig: Der Stapel wird zwar als Zahl gespeichert, aber nicht als Wert/Preis benutzt.")
    print("Der Score sortiert Prioritäten; die Einzelziffern erzeugen Vertragsbedingungen.")
    print("Hohe Differenz bei niedriger Winkelrichtung heißt: Bedarf real, aber Handlung umbauen oder blockieren.")
    print("")
