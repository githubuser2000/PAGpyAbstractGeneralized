...
def print_dimension_bars(values: Dict[str, float], indent: str = "  ") -> None:
    for dim in TRUTH_DIMS:
        ...
        print("%s%-24s %s %s" % (indent, head, meter, value_txt))
        print("%s    %s" % (indent, role_txt))
...
def pretty_key_value(label: str, value: str, label_fg: Tuple[int, int, int] = (180, 220, 255), value_fg: Tuple[int, int, int] = (255, 255, 255)) -> None:
    print("%s %s" % (color_text(label.ljust(18), fg=label_fg, bold=True), color_text(value, fg=value_fg)))
...
def terminal_header(title: str, subtitle: str = "") -> None:
    width = 118
    terminal_rule(width=width)
    print(color_text("✦ " + title + " ✦", fg=(255, 255, 255), bg=(35, 35, 35), bold=True))
    if subtitle:
...
def terminal_rule(title: str = "", width: int = 118) -> None:
    if title:
        label = "❖ " + title.strip() + " ❖"
        line = "═" * max(0, width - len(label) - 2)
        left = line[: len(line) // 2]
        right = line[len(line) // 2 :]
        print(rainbow_text(left + " " + label + " " + right))
    else:
        print(rainbow_text("═" * width))
...
def print_dimension_guide_terminal() -> None:
    ...
    print(color_text("Skala:", fg=(255, 214, 10), bold=True), color_text("0 = nicht vorhanden/falsch | 1 = schwach/latent | 2 = teilweise | 3 = stark | 4 = kritisch/hoch real", fg=(240, 240, 240)))
    print(color_text("Stapel-Reihenfolge:", fg=(0, 229, 255), bold=True), render_truth_stack_badges({dim: 4.0 for dim in TRUTH_DIMS}))
    print(color_text("Der Stapel ist keine Geldzahl. Er ist die Vertrags- und Zustands-Signatur eines Wirkungsflusses.", fg=(255, 255, 255), italic=True))
    print("")
    for dim in TRUTH_DIMS:
        g = DIMENSION_GUIDE_DE[dim]
        theme = dim_theme(dim)
        ...
        print(head)
        print("  " + color_text("Frage:   ", fg=theme["glow"], bold=True) + color_text(g["question"], fg=(245, 245, 245)))
        print("  " + color_text("Vertrag: ", fg=theme["glow"], bold=True) + color_text(g["contract_role"], fg=(245, 245, 245), italic=True))
        print("  " + color_text("Ersetzt: ", fg=theme["glow"], bold=True) + color_text(g["economic_replacement"], fg=(245, 245, 245)))
        sample = {d: 0.0 for d in TRUTH_DIMS}
        sample[dim] = 4.0
        print("  " + color_text("Vollausprägung:", fg=theme["glow"], bold=True) + " " + render_truth_stack_badges(sample))
        print("")
...
def print_visible_trade_contracts(flows: List[EffectFlow], limit: int = 16, detail: bool = False) -> None:
    ...
    if not selected:
        print(color_text("Keine Wirkungsflüsse vorhanden. Erhöhe --steps oder setze --show-trades auf einen Wert > 0.", fg=(255, 120, 120), bold=True))
        print("")
        return
    print(color_text("Alte Form:", fg=(255, 106, 188), bold=True), color_text("Ware + Menge + Preis + Eigentum → kaufen/verkaufen/importieren/exportieren", fg=(240, 240, 240)))
    print(color_text("Neue Form:", fg=(0, 229, 255), bold=True), color_text("Kausalität + Zeit + Intensität + Existenz + Potenzen + Wirkungen + Substanz + Materie + Differenz + Bestimmung + Phänomene + Winkelrichtung → Wirkungsvertrag", fg=(240, 240, 240)))
    print(color_text("Anzeige:", fg=(255, 214, 10), bold=True), color_text("Jeder Eintrag ist ein realer Wirkungsfluss, keine Geldtransaktion. activated_effect ist Wirkungseinheit, nicht Wert.", fg=(240, 240, 240), italic=True))
    print("")
    for idx, flow in enumerate(selected, 1):
        ...
        print(domain_badge(flow.domain), color_text(validity_de.upper(), fg=dom_fg, bold=True), color_text("•", fg=(255,255,255)), color_text(flow.kind, fg=(220,220,220), italic=True))
        ...
        print(color_text("Wahrheitsstapel", fg=(255,255,255), bold=True) + ":  " + render_truth_stack_badges(flow.values))
        ...
        print(color_text("Dimensionen im Detail", fg=(255,255,255), bold=True, underline=True))
        ...
        if detail:
            ...
            print(color_text("Vertragsbedingungen", fg=(255,255,255), bold=True, underline=True))
...
def print_truth_stack_explanation_terminal() -> None:
    ...
    print(color_text("Beispiel-Base5:", fg=(255, 214, 10), bold=True), color_text("334434234343", fg=(255,255,255), bold=True))
    print(color_text("Farbstapel:", fg=(0, 229, 255), bold=True), render_truth_stack_badges(sample))
    print(color_text("Dimensionale Auslesung:", fg=(255,255,255), bold=True, underline=True))
    print_dimension_bars(sample, indent="  ")
    print(color_text("Bedeutung", fg=(255,255,255), bold=True, underline=True))
    print("  " + colorful_bullet("Kausalität=3: Ursache ist gut getroffen.", dim_theme("causality")["fg"]))
    print("  " + colorful_bullet("Zeit=3: dringlich.", dim_theme("time")["fg"]))
    print("  " + colorful_bullet("Intensität=4: sehr starkes Phänomen.", dim_theme("intensity")["fg"]))
    print("  " + colorful_bullet("Existenz=4: real vorhanden, nicht nur behauptet.", dim_theme("existence")["fg"]))
    print("  " + colorful_bullet("Potenzen=3: Lösungsmöglichkeiten existieren.", dim_theme("potencies")["fg"]))
    print("  " + colorful_bullet("Wirkungen=4: hoher positiver Systemeffekt.", dim_theme("effects")["fg"]))
    print("  " + colorful_bullet("Substanz=2: Stoffe/Energie/Wissen sind nur teilweise verfügbar.", dim_theme("substance")["fg"]))
    print("  " + colorful_bullet("Materie=3: Ort/Infrastruktur sind ausreichend erreichbar.", dim_theme("matter")["fg"]))
    print("  " + colorful_bullet("Differenz=4: Bedarfslücke ist maximal sichtbar.", dim_theme("difference")["fg"]))
    print("  " + colorful_bullet("Bestimmung=3: sozial/demokratisch gut bestimmt.", dim_theme("determination")["fg"]))
    print("  " + colorful_bullet("Phänomene=4: stark sichtbar/gemeldet/gemessen.", dim_theme("phenomena")["fg"]))
    print("  " + colorful_bullet("Winkelrichtung=3: Handlung wirkt eher regenerativ/freiheitlich.", dim_theme("angle_direction")["fg"]))
    print("")
    print(color_text("Wichtig:", fg=(255, 106, 188), bold=True), color_text("Der Stapel wird zwar als Zahl gespeichert, aber nicht als Wert/Preis benutzt.", fg=(245,245,245)))
    print(color_text("Der Score sortiert Prioritäten; die Einzelziffern erzeugen Vertragsbedingungen.", fg=(245,245,245), italic=True))
    print(color_text("Hohe Differenz bei niedriger Winkelrichtung heißt: Bedarf real, aber Handlung umbauen oder blockieren.", fg=(245,245,245)))
    print("")
...
def art_box_line(text: str, fg: Tuple[int, int, int], width: int = 100) -> str:
    body = (" " + terminal_shorten(text, width - 4) + " ").ljust(width - 2)
    return color_text("┃", fg=fg, bold=True) + color_text(body, fg=(245, 245, 245)) + color_text("┃", fg=fg, bold=True)


def art_panel(title: str, number: int, subtitle: str = "") -> None:
    fg = art_color(number)
    print("")
    print(color_text("╔" + "═" * 112 + "╗", fg=fg, bold=True))
    line = "  %02d  %s" % (number, title)
    print(color_text("║", fg=fg, bold=True) + color_text(line.ljust(112), fg=fg, bold=True) + color_text("║", fg=fg, bold=True))
    if subtitle:
        print(color_text("║", fg=fg, bold=True) + color_text(("  " + subtitle).ljust(112), fg=(255, 255, 255), italic=True) + color_text("║", fg=fg, bold=True))
    print(color_text("╚" + "═" * 112 + "╝", fg=fg, bold=True))
...
def art_ratio_bar(label: str, ratio: float, fg: Tuple[int, int, int], width: int = 50) -> str:
    ...
    return "%s %s %s" % (
        color_text(label.ljust(24), fg=fg, bold=True),
...
def art_planet_layer_stack(last: GlobalMetrics) -> None:
    ...
    for name, desc, fg, ratio in layers:
        print(art_ratio_bar(name, clamp(ratio), fg, width=44))
        print("   " + color_text("╰─ ", fg=fg) + color_text(desc, fg=(245, 245, 245)))
    print(color_text("        ╰──────────────────────────────────────────────────────────────────────╯", fg=(161,108,255)))
    print(color_text("        Wirtschaft = Wirklichkeitsänderung innerhalb planetarer Grenzen", fg=(255,255,255), bold=True))
...
def art_truth_stack_totem(flow: Optional[EffectFlow]) -> None:
    ...
    print(color_text("BASE5-STACK: ", fg=(255,214,10), bold=True) + render_truth_stack_badges(values))
    print(color_text("Zahlensignatur: ", fg=(0,229,255), bold=True) + color_text(stack, fg=(255,255,255), bold=True))
    print("")
    for dim in TRUTH_DIMS:
        print("      " + art_dim_line(dim, values.get(dim, 0.0), width=32))
    print(color_text("      │", fg=(255,255,255)))
    print(color_text("      ▼", fg=(255,255,255), bold=True))
    print(color_text("  Vertragsgültigkeit + Priorität + Bedingungen", fg=(255,106,188), bold=True))
...
def art_causal_pipeline(flow: Optional[EffectFlow]) -> None:
    ...
    print(line)
    if flow is not None:
        print(art_box_line("Beispiel: %s" % flow.causal_link, fg, width=110))
        print(art_box_line("Aktivierte Wirkung: %.3f | Domain: %s | Art: %s" % (flow.activated_effect, flow.domain, flow.kind), fg, width=110))
    print(color_text("          ╭──────────── Rückkopplung: Phänomene + Wahrheitfehler + Betroffenenberichte ────────────╮", fg=(255,106,188)))
    print(color_text("          ╰──────────────────────────────────────◀──────────────────────────────────────────────────╯", fg=(255,106,188)))
...
def art_buy_sell_replacement() -> None:
    ...
    for old, new, kind, fg in rows:
        print(styled_badge(old, fg, tuple(int(c*0.22) for c in fg)) + color_text("  ─────╮", fg=fg, bold=True))
        print(color_text("              ├──▶ ", fg=fg, bold=True) + color_text(new, fg=(255,255,255), bold=True) + "  " + color_text("[%s]" % kind, fg=fg))
        print(color_text("              ╰──▶ Wahrheit: K/Z/I/E/P/W/S/M/D/B/Ph/R", fg=fg))
...
def art_boundary_dashboard(last: GlobalMetrics) -> None:
    ...
    for i, (name, pressure) in enumerate(proxies):
        ...
        print(art_ratio_bar(label, min(pressure, 1.35) / 1.35, fg, width=54) + "  " + color_text("%.3f" % pressure, fg=(255, 255, 255), bold=True))
    print(color_text("Overshoot gesamt: %.4f | schlimmste Grenze: %s %.3f" % (last.overshoot, last.worst_boundary, last.worst_boundary_pressure), fg=(255, 87, 87), bold=True))
...
def art_climate_contract_shield(last: GlobalMetrics) -> None:
    ...
    print(color_text("                 ╭────────────────────────────╮", fg=fg1, bold=True))
    print(color_text("             ╭───┤   KLIMA-RELEVANZ-PRÜFUNG  ├───╮", fg=fg1, bold=True))
    print(color_text("             │   ╰────────────────────────────╯   │", fg=fg1, bold=True))
    print(color_text("        CO₂  │   Energie  Material  Transport     │  Hitze", fg=fg2, bold=True))
    print(color_text("             │        ╲      │      ╱             │", fg=fg2, bold=True))
    print(color_text("             │         ╲     ▼     ╱              │", fg=fg2, bold=True))
    print(color_text("             │       WINKELRICHTUNG R             │", fg=fg3, bold=True))
    print(color_text("             ╰──────────────┬─────────────────────╯", fg=fg1, bold=True))
    print(color_text("                            ▼", fg=fg1, bold=True))
    print(color_text("                  Vertrag: gültig / bedingt / blockiert", fg=(255,255,255), bold=True))
    print(art_ratio_bar("Klimadruck Proxy", clamp(last.worst_boundary_pressure / 1.4), fg3, width=50))
...
def art_material_cycle(last: GlobalMetrics) -> None:
    ...
    fg = (38, 222, 129)
    print(color_text("        ┌──────────────┐      ┌──────────────┐      ┌──────────────┐", fg=fg, bold=True))
    print(color_text("        │  SUBSTANZ S  │ ───▶ │  PRODUKTION  │ ───▶ │  NUTZUNG     │", fg=fg, bold=True))
    print(color_text("        └──────┬───────┘      └──────┬───────┘      └──────┬───────┘", fg=fg, bold=True))
    print(color_text("               │                     │                     │", fg=fg, bold=True))
    print(color_text("               ▼                     ▼                     ▼", fg=fg, bold=True))
    print(color_text("        ┌──────────────┐      ┌──────────────┐      ┌──────────────┐", fg=(255,214,10), bold=True))
    print(color_text("        │  REPARATUR   │ ◀─── │  ABFALL Δ    │ ◀─── │  VERSCHLEISS │", fg=(255,214,10), bold=True))
    print(color_text("        └──────────────┘      └──────────────┘      └──────────────┘", fg=(255,214,10), bold=True))
    print(art_ratio_bar("circularity_index", last.circularity_index, (0,245,212), width=46))
    print(art_ratio_bar("repair_materials", clamp(last.repair_materials / max(1.0, last.repair_materials + last.waste_stock)), (255,106,188), width=46))
...
def art_commune_network(flows: List[EffectFlow]) -> None:
    ...
    for i, flow in enumerate(selected):
        ...
        arrow = "═" * (8 + (i % 5)) + "▶"
        print(color_text(left.ljust(34), fg=fg, bold=True) + color_text(arrow, fg=fg, bold=True) + color_text(right, fg=(255,255,255), bold=True))
        print("    " + domain_badge(flow.domain) + " " + color_text(flow.kind, fg=fg) + " " + color_text("%.2f Wirkungseinheiten" % flow.activated_effect, fg=(255,240,170), bold=True))
    if not selected:
        print(color_text("Keine Flows vorhanden.", fg=(255,87,87), bold=True))
...
def art_products_jobs_services_map() -> None:
    ...
    print(color_text("                 ╭──────────── WIRKUNGSVERTRAG ────────────╮", fg=(255,255,255), bold=True))
    for name, desc, fg in trio:
        print(color_text("    ", fg=fg) + styled_badge(name, fg, tuple(int(c*0.20) for c in fg)) + color_text(" ───▶ ", fg=fg, bold=True) + color_text(desc, fg=(245,245,245)))
    print(color_text("                 ╰────── Wahrheit + Bedingungen + Audit ───╯", fg=(255,255,255), bold=True))
...
def art_service_constellation() -> None:
    ...
    print(color_text("             health", fg=domain_color("health"), bold=True) + "        " + color_text("care", fg=domain_color("care"), bold=True))
    print(color_text("                ╲         ╱", fg=(255,106,188), bold=True))
    print(color_text("                 ╲       ╱", fg=(255,106,188), bold=True))
    print("                  " + center)
    print(color_text("                 ╱   │   ╲", fg=(0,229,255), bold=True))
    print(color_text("        education    │    mobility", fg=(0,229,255), bold=True))
    print(color_text("                     governance", fg=domain_color("governance"), bold=True))
    print(color_text("Jede Kante ist ein Wirkungsfluss: Zeit, Vertrauen, Pflege, Wissen, Zugang.", fg=(245,245,245)))
...
def art_angle_compass(flow: Optional[EffectFlow]) -> None:
    ...
    print(color_text("                         ↑ regenerativ / freiheitlich", fg=(38,222,129), bold=True))
    print(color_text("                         │", fg=fg, bold=True))
    print(color_text("        lokal passend ◀──┼──▶ planetar passend", fg=fg, bold=True))
    print(color_text("                         │", fg=fg, bold=True))
    print(color_text("                         ↓ ausbeutend / kontrollierend", fg=(255,87,87), bold=True))
    print(art_ratio_bar("R Winkelrichtung", r / 4.0, fg, width=52))
    if flow is not None:
        print(color_text("Beispielrichtung: ", fg=fg, bold=True) + color_text(flow.direction_vector, fg=(245,245,245)))
...
def art_difference_funnel(flow: Optional[EffectFlow]) -> None:
    ...
    print(color_text("        Bedarf / Not / Möglichkeit", fg=(255,255,255), bold=True))
    print(color_text("      ╱════════════════════════════╲", fg=fg, bold=True))
    print(color_text("     ╱   Wohnung  Nahrung  Pflege   ╲", fg=fg, bold=True))
    print(color_text("    ╱   Wasser  Energie  Ökologie    ╲", fg=fg, bold=True))
    print(color_text("    ╲              Δ                 ╱", fg=fg, bold=True))
    print(color_text("     ╲       Differenz wird          ╱", fg=fg, bold=True))
    print(color_text("      ╲      Wirkungsvertrag        ╱", fg=fg, bold=True))
    print(color_text("       ╲═══════════▼═══════════════╱", fg=fg, bold=True))
    print(color_text("             Handlung / Wirkung", fg=(38,222,129), bold=True))
    print(art_ratio_bar("D Differenz", d / 4.0, fg, width=52))
...
def art_contract_gate(flow: Optional[EffectFlow]) -> None:
    ...
    for name, question, dim in gates:
        ...
        print(styled_badge(name, fg, dim_theme(dim)["bg"]) + " " + art_ratio_bar(question, value / 4.0, fg, width=38))
    print(color_text("    ╰──────▶ Ergebnis: ", fg=(255,255,255), bold=True) + styled_badge(validity.upper(), (255,255,255), (40,70,40)) + color_text("  Priorität %.3f" % score, fg=(255,214,10), bold=True))
...
def art_no_money_map() -> None:
    ...
    for i in range(len(old)):
        print(styled_badge(old[i], (255,87,87), (70,10,10)) + color_text("  ═══════▶  ", fg=art_color(i), bold=True) + styled_badge(new[i], art_color(i), tuple(int(c*0.20) for c in art_color(i))))
...
def art_macro_accounts(macro_accounts: List[MacroAccountRow]) -> None:
    ...
    for i, row in enumerate(rows):
        ...
        print(domain_badge(row.domain) + " " + color_text(row.sector, fg=(245,245,245), bold=True))
        print("  " + art_ratio_bar("satisfaction", row.satisfaction, fg, width=36))
        print("  " + art_ratio_bar("priority", row.priority, art_color(i + 3), width=36))
        print("  " + art_ratio_bar("boundary penalty", row.boundary_penalty, art_color(i + 5), width=36))
...
def art_ecology_mandala(last: GlobalMetrics) -> None:
    ...
    print(color_text("                  ✺ Boden ✺", fg=(190,140,90), bold=True))
    print(color_text("             ╭──────┼──────╮", fg=fg, bold=True))
    print(color_text("        Wasser ─── 🌍 ─── Biodiversität", fg=fg, bold=True))
    print(color_text("             ╰──────┼──────╯", fg=fg, bold=True))
    print(color_text("                  ✺ Kühlung ✺", fg=(0,229,255), bold=True))
    print(color_text("        Regeneration senkt Risiken und erhöht Reproduktionsfähigkeit.", fg=(245,245,245)))
    print(art_ratio_bar("reproduction index", last.planetary_reproduction_index, fg, width=54))
...
def art_storage_time_bridge(last: GlobalMetrics) -> None:
    ...
    print(color_text("    JETZT ────── Nahrung/Wasser/Energie ──────▶ SPÄTER", fg=fg, bold=True))
    print(color_text("          ╲                                   ╱", fg=fg, bold=True))
    print(color_text("           ╲_______ Speicher / Reserve ______╱", fg=fg, bold=True))
    print(art_ratio_bar("basic_buffer_months", clamp(last.basic_buffer_months / 6.0), fg, width=52))
    print(art_ratio_bar("resilience_index", last.resilience_index, (255,166,0), width=52))
...
def art_governance_feedback(last: GlobalMetrics) -> None:
    ...
    print(color_text("   Betroffene ──▶ Meldung ──▶ Wahrheitswert ──▶ Vertrag ──▶ Wirkung", fg=fg, bold=True))
    print(color_text("       ▲                                                           │", fg=fg, bold=True))
    print(color_text("       └──────────── Audit ◀── Fehlerprüfung ◀── Phänomene ◀──────┘", fg=fg, bold=True))
    print(art_ratio_bar("coordination_quality", last.coordination_quality, fg, width=52))
    print(art_ratio_bar("truth_error invertiert", 1.0 - clamp(last.avg_truth_error), (38,222,129), width=52))
...
def art_truth_dna(flow: Optional[EffectFlow]) -> None:
    ...
    for i in range(len(TRUTH_DIMS)):
        twist = "╲╱" if i % 2 == 0 else "╱╲"
        print("      %s  %s  %s" % (left[i].ljust(20), color_text(twist, fg=art_color(i), bold=True), right[i]))
...
def art_domain_rainbow() -> None:
    ...
    if row:
        print(row)
    print(color_text("Alle Domänen teilen dieselben 12 Wahrheitsdimensionen, aber andere Produkte, Arbeitsplätze und Klimaauflagen.", fg=(245,245,245)))
...
def art_resilience_radar(last: GlobalMetrics) -> None:
    ...
    for name, ratio, fg in metrics:
        print(art_ratio_bar(name, ratio, fg, width=48))
...
def art_phenomena_wall(truth_vectors: List[TruthVector]) -> None:
    ...
    for tv in selected:
        ...
        print(domain_badge(tv.domain) + " " + color_text("%s/%s" % (tv.region, tv.commune), fg=(245,245,245), bold=True))
        print("  " + art_ratio_bar("Phänomene", ph / 4.0, dim_theme("phenomena")["fg"], width=44))
        print("  " + color_text(terminal_shorten(tv.explanation, 120), fg=(220,220,220)))
...
def art_potency_garden(truth_vectors: List[TruthVector]) -> None:
    ...
    for i, tv in enumerate(selected):
        ...
        print(color_text(("✿" * flowers).ljust(6), fg=fg, bold=True) + domain_badge(tv.domain) + " " + color_text("P=%.2f" % p, fg=fg, bold=True) + " " + color_text(tv.commune, fg=(245,245,245)))
...
def art_labor_lattice(macro_accounts: List[MacroAccountRow]) -> None:
    ...
    for i, row in enumerate(rows):
        ...
        print(domain_badge(row.domain) + " " + color_text("●─" * nodes + "●", fg=fg, bold=True) + " " + color_text("time %.2f" % row.contribution_time, fg=(255,255,255)))
...
def art_product_wave() -> None:
    ...
    for i, domain in enumerate(["water", "food", "energy", "shelter", "health", "repair", "ecology", "waste"]):
        ...
        print(domain_badge(domain) + " " + wave + " " + color_text(" | ".join(p.strip() for p in products), fg=(245,245,245)) + " " + wave)
...
def art_effect_ocean(flows: List[EffectFlow]) -> None:
    ...
    for i, flow in enumerate(selected):
        ...
        print(color_text("≋" * (height + 4), fg=fg, bold=True) + " " + domain_badge(flow.domain) + " " + color_text(flow.kind, fg=(245,245,245)) + " " + color_text("%.2f" % flow.activated_effect, fg=(255,240,170), bold=True))
...
def art_heatmap_dimensions(flows: List[EffectFlow]) -> None:
    ...
    if not selected:
        print(color_text("Keine Flows vorhanden.", fg=(255,87,87), bold=True))
        return
    print(color_text("          ", fg=(255,255,255)) + " ".join(color_text("%02d" % (i + 1), fg=domain_color(f.domain), bold=True) for i, f in enumerate(selected)))
    shade = "░▒▓█"
    for dim in TRUTH_DIMS:
        theme = dim_theme(dim)
        guide = DIMENSION_GUIDE_DE[dim]
        ...
        print(row + color_text(guide["name_de"], fg=theme["fg"], bold=True))
...
def art_time_river(timeline: List[GlobalMetrics]) -> None:
    ...
    print(color_text("wellbeing     ", fg=(38,222,129), bold=True) + art_spark(wellbeing, (38,222,129), width=72))
    print(color_text("unmet_basic   ", fg=(255,87,87), bold=True) + art_spark(unmet, (255,87,87), width=72))
    print(color_text("overshoot     ", fg=(255,166,0), bold=True) + art_spark(overs, (255,166,0), width=72))
...
def art_final_sigil() -> None:
    ...
    for i, line in enumerate(sigil):
        print(color_text(line, fg=colors[i % len(colors)], bold=True))
...
def print_utf8_art_gallery_terminal(flows: List[EffectFlow], truth_vectors: List[TruthVector], timeline: List[GlobalMetrics], macro_accounts: List[MacroAccountRow], limit: int = 30) -> None:
    if limit <= 0:
        return
    terminal_header("EXTREM BUNTE UTF-8-ART-GALERIE", "Darstellungen unter den Verträgen: Diagramme, Abbildungen, Kreisläufe, Kompasse, Heatmaps und Sigillen.")
    last = timeline[-1]
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--show-art", type=int, default=30, help="print N extremely colorful UTF-8/ANSI art diagrams below the trade contracts; 0 disables, default 30")
