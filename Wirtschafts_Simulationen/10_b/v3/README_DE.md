...
| `effect_flow_audit.csv` | Audit der letzten Handlungsrunde: Kauf/Verkauf/Handel als Wirkungsfluss ersetzt, jetzt mit Produkten, Arbeitsplätzen, Dienstleistungen, Vertragsbedingungen und gestapelten Wahrheitswerten |
| `trade_contracts_report.md` | Lesbarer Bericht: Handeln in allen Dimensionen, Beispiele, Verträge, Wahrheitsstapel |
| `trade_dimension_catalog.csv` | Katalog je Domain: was gehandelt wird, was gemeint ist, Produkte, Arbeitsplätze, Dienstleistungen, Ökologie, Klima |
| `dimension_guide.csv` | Bedeutung jeder Dimension, Vertragsrolle, ökonomische Ersetzung |
...
4. `trade_contracts_report.md` für die lesbare Handels-/Vertragslogik.
5. `effect_flow_audit.csv` für alle Wirkungsflüsse mit gestapelten Wahrheitswerten.
6. `trade_dimension_catalog.csv` für Produkte, Arbeitsplätze, Dienstleistungen, Öko- und Klimaklauseln.
7. `truth_audit.csv` für die Prioritäten aus Wahrheitswerten.
8. `communes_final.csv` für lokale Unterschiede.

## Handelsdimensionen und Verträge

Diese Version ergänzt die sichtbare Handelslogik besonders stark. Jeder Flow enthält nun:
...
contract_conditions
truth_stack_base5
truth_stack_score_0_4
truth_stack_compact
dimension_meaning
```

Damit wird sichtbar, wie aus „kaufen/verkaufen“ ein Vertrag aus Kausalität, Zeit, Intensität, Existenz, Potenzen, Wirkungen, Substanz, Materie, Differenz, Bestimmung, Phänomenen und Winkelrichtung wird. Siehe zusätzlich `HANDEL_IN_DIMENSIONEN.md`.
