...
def art_macro_control_cockpit(last: GlobalMetrics) -> None:
    ...
    art_metric_rows(36, ['自主', '协调', '韧性', '真值错误'], [last.avg_autonomy, last.coordination_quality, last.resilience_index, 1.0 - last.avg_truth_error])
...
def art_macro_crisis_buffer(last: GlobalMetrics) -> None:
    ...
    art_metric_rows(38, ['储存逻辑', '知识缓冲', '信任储备', '危机压力'], [last.resilience_index, 1.0 - last.avg_truth_error, last.avg_autonomy, clamp(last.overshoot, 0.0, 1.0)])
...
def art_business_inventory_buffers(last: GlobalMetrics) -> None:
    ...
    art_metric_rows(43, ['材料缓冲', '能源缓冲', '时间缓冲', '修理储备'], [last.resilience_index, max(0.0, min(1.0, 0.55 + 0.35 * last.planetary_reproduction_index)), max(0.0, min(1.0, 0.45 + 0.45 * last.coordination_quality)), max(0.0, min(1.0, 0.40 + 0.45 * last.resilience_index))])
