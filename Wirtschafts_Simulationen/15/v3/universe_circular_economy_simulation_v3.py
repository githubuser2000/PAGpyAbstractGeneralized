...
def select_calculation_entries(cosmos: 'Cosmos', mode: str, limit: int) -> List[LedgerEntry]:
    ...
    if mode == "all":
        return entries
    # Sample mode: show a representative but still serious output stream.
    selected: List[LedgerEntry] = []
    seen: set = set()

    def add_many(rows: Iterable[LedgerEntry]) -> None:
        for row in rows:
            key = (row.tick, row.planet, row.company_id, row.identity_id, row.from_station, row.to_station)
            if key not in seen and len(selected) < limit:
                selected.append(row)
                seen.add(key)

    add_many(entries[:max(2, limit // 6)])
    add_many(sorted(entries, key=lambda r: r.esv, reverse=True)[:max(2, limit // 5)])
    add_many([r for r in entries if not r.verified][:max(2, limit // 5)])
    add_many([r for r in entries if r.debt_created > max(0.01, r.esv * 0.05)][:max(2, limit // 5)])
    add_many([r for r in entries if r.to_station in (5, 17, 19, 1, 2)][:max(2, limit // 5)])
    if len(selected) < limit:
        step = max(1, len(entries) // max(1, limit - len(selected)))
        add_many(entries[::step])
    selected.sort(key=lambda r: (r.tick, r.universe, r.system, r.planet, r.company_id))
    return selected[:limit]


def calculation_intro_text(lang: str, mode: str, selected: int, total: int, color: bool) -> str:
    title = text(lang, "🧮 COLORFUL TERMINAL CALCULATION OUTPUT", "🧮 BUNTE TERMINAL-RECHNUNGSAUSGABE")
    body = [
        text(lang,
             f"The detailed explanation now happens in the console output, not inside the Markdown report. This run generated {total} ledger calculations and prints {selected} of them in mode '{mode}'.",
             f"Die ausführliche Erklärung findet jetzt in der Konsolenausgabe statt, nicht im Markdown-Bericht. Dieser Lauf erzeugte {total} Hauptbuch-Rechnungen und gibt {selected} davon im Modus '{mode}' aus."),
        text(lang,
             "Each printed calculation has three colored parts: BEFORE explains what is being simulated and which abbreviation colors are used; FORMULA shows the arithmetic; AFTER evaluates the outcome, debt, verification and alternative branches.",
             "Jede ausgegebene Rechnung hat drei bunte Teile: VORHER erklärt, was simuliert wird und welche Kürzelfarben gelten; FORMEL zeigt die Arithmetik; NACHHER bewertet Ausgang, Schuld, Prüfung und alternative Zweige."),
        text(lang,
             "Use --calculation-output all for every single calculation, --calculation-output sample for a readable representative stream, or --calculation-output none for summary only.",
             "Nutze --calculation-output all für jede einzelne Rechnung, --calculation-output sample für eine lesbare repräsentative Auswahl oder --calculation-output none nur für die Zusammenfassung."),
    ]
    return console_box(title, body, color_hex="#FF00AF", color=color)


def render_single_calculation_output(r: LedgerEntry, idx: int, total: int, lang: str, color: bool = True) -> str:
    ...
    seg_expl = text(lang, seg.explanation_en, seg.explanation_de) if seg else text(lang, "Licensed company segment.", "Lizenziertes Unternehmenssegment.")
    planet_line = text(lang,
                       f"Planet {r.planet} is {r.planet_type}; therefore {term_badge('NCL', color)} = {r.ncl}. The native loop is not universal: Earth-type uses 20 stations, Vulcan-type uses 22.",
                       f"Planet {r.planet} ist {r.planet_type}; daher ist {term_badge('NCL', color)} = {r.ncl}. Die native Schleife ist nicht universell: Erde-Typ nutzt 20 Stationen, Vulkan-Typ 22.")
    company_type = text(lang, "partial-cycle company" if r.partial_company else "loop-integrating company",
                        "Teilzyklus-Unternehmen" if r.partial_company else "Schleifenintegrator")
    before_body = [
        text(lang,
             f"Calculation {idx}/{total}: {r.company_name} is a {company_type}. It does not necessarily own the whole cosmic economy; it operates {term_badge('SEG', color)} {term_segment(r.segment_code, color)} ({seg_label}) over segment {r.segment_span}.",
             f"Rechnung {idx}/{total}: {r.company_name} ist ein {company_type}. Es besitzt nicht unbedingt die ganze kosmische Wirtschaft, sondern bearbeitet {term_badge('SEG', color)} {term_segment(r.segment_code, color)} ({seg_label}) über Segment {r.segment_span}."),
        text(lang,
             f"What is simulated: identity {r.identity_id} / {r.identity_label} moves from {station_label(r.from_station, lang, color)} to {station_label(r.to_station, lang, color)}. This models a real division of labor handoff between specialized firms.",
             f"Was simuliert wird: Identität {r.identity_id} / {r.identity_label} bewegt sich von {station_label(r.from_station, lang, color)} nach {station_label(r.to_station, lang, color)}. Das modelliert eine reale Arbeitsteilungs-Übergabe zwischen spezialisierten Firmen."),
        text(lang,
             f"Why this matters: a cosmos economy is not one giant company. Fertilizer firms, harvest firms, digestion firms, compost firms, geology firms and validators each close only part of the loop. {seg_expl}",
             f"Warum das wichtig ist: Eine Kosmoswirtschaft ist kein einziges Riesenunternehmen. Düngerfirmen, Erntefirmen, Verdauungsfirmen, Kompostfirmen, Geologiefirmen und Validatoren schließen jeweils nur einen Teil der Schleife. {seg_expl}"),
        planet_line,
        text(lang, "Relevant abbreviation colors for this calculation:", "Relevante Kürzelfarben für diese Rechnung:"),
    ]
    codes = ["SG", "UKE", "NCL", "STA", "END", "REP", "MAT", "QAL", "USE", "JUS", "ESV", "VSG", "USG", "TAX", "SUB", "CLD", "SEG", r.segment_code]
    before_body.extend(console_abbreviation_legend(lang, codes, color=color))
    before = console_box(text(lang, "🌈 BEFORE THE CALCULATION", "🌈 VOR DER RECHNUNG"), before_body,
                         color_hex=SEGMENT_COLORS.get(r.segment_code, "#FF7F00"), color=color)
    ...
    formula_body = [
        ...
        text(lang,
             "Interpretation of the arithmetic: SG measures the raw distance through the local planet loop; UKE normalizes that distance for interplanetary comparison; ESV multiplies the distance by matter, quality, usefulness and justice.",
             "Deutung der Arithmetik: SG misst die rohe Distanz durch die lokale Planetenschleife; UKE normalisiert diese Distanz für interplanetaren Vergleich; ESV multipliziert die Distanz mit Materie, Qualität, Nutzen und Gerechtigkeit."),
    ]
    formula = console_box(text(lang, "🔢 THE CALCULATION", "🔢 DIE RECHNUNG"), formula_body,
                          color_hex="#00D7FF", color=color)

    net_company = r.minted_sg - r.tax + r.subsidy
    if r.verified:
        verdict = text(lang,
                       f"Verification succeeded. The claim becomes {term_badge('VSG', color)} and can circulate as spendable loop money.",
                       f"Die Prüfung war erfolgreich. Der Anspruch wird zu {term_badge('VSG', color)} und kann als ausgebbares Schleifengeld zirkulieren.")
    else:
        verdict = text(lang,
                       f"Verification failed. The claim remains {term_badge('USG', color)} and is not minted into public money.",
                       f"Die Prüfung scheiterte. Der Anspruch bleibt {term_badge('USG', color)} und wird nicht als öffentliches Geld geprägt.")
    debt_reading = text(lang,
                        "low debt: the transaction is mostly regenerative" if r.debt_created < max(0.01, r.esv * 0.03) else "visible debt: the transition creates repair pressure or pollution risk",
                        "niedrige Schuld: die Transaktion ist überwiegend regenerativ" if r.debt_created < max(0.01, r.esv * 0.03) else "sichtbare Schuld: der Übergang erzeugt Reparaturdruck oder Verschmutzungsrisiko")
    after_body = [
        text(lang,
             f"Outcome: {term_paint_hex(r.outcome, status_color_hex(r.outcome, r.verified, r.debt_created), bold=True, enabled=color)}. {verdict}",
             f"Ausgang: {term_paint_hex(r.outcome, status_color_hex(r.outcome, r.verified, r.debt_created), bold=True, enabled=color)}. {verdict}"),
        ...
        text(lang,
             f"Company net money effect from this calculation is approximately {net_company:.3f}: minted value minus tax plus subsidy. The debt reading is: {debt_reading}.",
             f"Der Netto-Geldeffekt für das Unternehmen beträgt ungefähr {net_company:.3f}: geprägter Wert minus Steuer plus Subvention. Die Schuld-Lesart lautet: {debt_reading}."),
        text(lang,
             "Alternative branches: if verification infrastructure, technology or justice had been stronger, USG could have become VSG; if pollution, low quality or unfair labor had dominated, ESV would shrink and CLD could make the same physical transition economically negative; if material quantity were larger, the same station distance would scale upward through MAT.",
             "Alternative Zweige: Bei stärkerer Prüfinfrastruktur, Technologie oder Gerechtigkeit hätte USG zu VSG werden können; bei dominanter Verschmutzung, niedriger Qualität oder unfairer Arbeit würde ESV schrumpfen und CLD denselben physischen Übergang ökonomisch negativ machen; bei größerer Materialmenge würde dieselbe Stationsdistanz über MAT hochskalieren."),
        text(lang,
             f"This is why the simulation separates raw loop distance from real economic quality: {term_badge('SG', color)} alone is not enough; {term_badge('MAT', color)}, {term_badge('QAL', color)}, {term_badge('USE', color)}, {term_badge('JUS', color)}, verification and {term_badge('CLD', color)} decide whether the movement is wealth or damage.",
             f"Darum trennt die Simulation rohe Schleifendistanz von echter ökonomischer Qualität: {term_badge('SG', color)} allein reicht nicht; {term_badge('MAT', color)}, {term_badge('QAL', color)}, {term_badge('USE', color)}, {term_badge('JUS', color)}, Prüfung und {term_badge('CLD', color)} entscheiden, ob die Bewegung Wohlstand oder Schaden ist."),
    ]
    after = console_box(text(lang, "✅ AFTER THE CALCULATION", "✅ NACH DER RECHNUNG"), after_body,
                        color_hex=status_color_hex(r.outcome, r.verified, r.debt_created), color=color)
    return before + "\n" + formula + "\n" + after


def print_calculation_output(cosmos: 'Cosmos', lang: str, color: bool, mode: str, limit: int) -> None:
    all_entries = ledger_entries_sorted(cosmos)
    selected = select_calculation_entries(cosmos, mode, max(1, limit))
    if mode == "none":
        return
    print(calculation_intro_text(lang, mode, len(selected), len(all_entries), color))
    for idx, entry in enumerate(selected, start=1):
        print(render_single_calculation_output(entry, idx, len(selected), lang, color=color))


def render_markdown_report_minimal(cosmos: 'Cosmos', lang: str = "en") -> str:
    """A lean file report. The detailed calculation teaching now belongs to the terminal output."""
    s = cosmos.summary()
    lines: List[str] = []
    lines.append("# " + text(lang, "Universe Circular Economy Simulation v3 — file report", "Universum-Kreislaufwirtschaft Simulation v3 — Datei-Bericht"))
    lines.append("")
...
def print_console_summary(cosmos: Cosmos, lang: str, color: bool = True) -> None:
    s = cosmos.summary()
    print("\n" + ("=" * 72))
    print(text(lang, "Universe Circular Economy Simulation v3", "Universum-Kreislaufwirtschaft Simulation v3"))
    print("=" * 72)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PyPy3 simulation of the Universe Circular Economy v3 with partial-cycle companies and very colorful terminal calculation output."
    )
    ...
    parser.add_argument("--report-dir", type=str, default="uce_report_v3", help="Directory for CSV/JSON/Markdown reports. Empty string disables files.")
    parser.add_argument("--explain-limit", type=int, default=24, help="Compatibility setting; Markdown is compact in v3, detailed explanations are in terminal output.")
    ...
    parser.add_argument("--calculation-limit", type=int, default=36, help="How many calculations to print when --calculation-output sample is used.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print_calculation_output(cosmos, lang=args.lang, color=not args.no_color, mode=args.calculation_output, limit=args.calculation_limit)
    ...
    if not args.no_files and args.report_dir:
        ...
        print("- final_report.md  # compact; detailed calculation explanations are in terminal output")
