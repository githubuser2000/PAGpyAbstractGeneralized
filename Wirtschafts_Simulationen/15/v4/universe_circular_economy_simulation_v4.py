...
def write_reports(cosmos: Cosmos, report_dir: Path, lang: str, explain_limit: int) -> None:
    ...
    with (report_dir / "final_report.md").open("w", encoding="utf-8") as f:
        f.write(render_markdown_report_minimal(cosmos, lang=lang))
...
def hex_to_ansi256(hex_color: str) -> int:
    ...
    rr = int(round(r / 255.0 * 5))
    gg = int(round(g / 255.0 * 5))
    bb = int(round(b / 255.0 * 5))
    return 16 + 36 * rr + 6 * gg + bb


def readable_foreground_for_hex(hex_color: str) -> int:
    r, g, b = hex_to_rgb(hex_color)
    luminance = 0.2126 * r + 0.7152 * g + 0.0722 * b
    return 16 if luminance > 155 else 231


def term_paint(value: str, *, fg: Optional[int] = None, bg: Optional[int] = None,
               bold: bool = False, underline: bool = False, enabled: bool = True) -> str:
    if not enabled:
        return value
    codes: List[str] = []
    if bold:
    ...
    if not codes:
        return value
    return "\033[" + ";".join(codes) + "m" + value + "\033[0m"


def term_paint_hex(value: str, hex_color: str, *, bg_mode: bool = False, bold: bool = False,
                   enabled: bool = True) -> str:
    if not enabled:
        return value
    c = hex_to_ansi256(hex_color)
    if bg_mode:
        return term_paint(value, fg=readable_foreground_for_hex(hex_color), bg=c, bold=bold, enabled=enabled)
    return term_paint(value, fg=c, bold=bold, enabled=enabled)


def term_badge(code: str, enabled: bool = True) -> str:
    if code in ABBREVIATIONS:
        h = ABBREVIATIONS[code].color
    elif code in SEGMENT_COLORS:
        h = SEGMENT_COLORS[code]
    else:
        h = "#FF7F00"
    if not enabled:
        return f"[{code}]"
    return term_paint_hex(f" {code} ", h, bg_mode=True, bold=True, enabled=enabled)


def term_segment(code: str, enabled: bool = True) -> str:
    return term_badge(code, enabled)


def station_label(no: int, lang: str, color: bool = True) -> str:
    st = STATIONS[no]
    label = f"{no}:{st.letter}/{text(lang, st.name_en, st.name_de)}"
    palette = ["#E6194B", "#3CB44B", "#4363D8", "#F58231", "#911EB4", "#46F0F0", "#F032E6", "#BCF60C", "#9A6324", "#008080", "#FFD700", "#A6CEE3"]
    return term_paint_hex(label, palette[(no - 1) % len(palette)], bold=True, enabled=color)


def status_color_hex(outcome: str, verified: bool, debt: float) -> str:
    if verified and debt < 1e-9:
        return "#00FF87"
    if verified and "regenerative" in outcome:
        return "#3CB44B"
    if verified:
        return "#FFE119"
    if "debt" in outcome:
...
def wrap_plain(text_value: str, width: int = 102) -> List[str]:
    if not text_value:
        return [""]
    lines: List[str] = []
    for raw in text_value.splitlines():
        if len(strip_ansi(raw)) <= width:
            lines.append(raw)
        else:
            # Keep ANSI-heavy badge lines intact as much as possible; wrap only plain text fragments.
            lines.extend(textwrap.wrap(raw, width=width, replace_whitespace=False, drop_whitespace=False) or [""])
    return lines


def console_box(title: str, body: Sequence[str], *, color_hex: str = "#4363D8", color: bool = True,
                width: int = 118) -> str:
    bar = "═" * max(20, width - 2)
    fg = hex_to_ansi256(color_hex)
    top = term_paint(f"╔{bar}╗", fg=fg, bold=True, enabled=color)
    bottom = term_paint(f"╚{bar}╝", fg=fg, bold=True, enabled=color)
    title_plain = f" {title} "
    title_line = term_paint_hex(title_plain, color_hex, bg_mode=True, bold=True, enabled=color)
    out = [top, "║ " + title_line]
    for raw in body:
        for ln in wrap_plain(raw, width=width - 4):
            out.append("║ " + ln)
    out.append(bottom)
    return "\n".join(out)


def colored_number(value: object, code: str, color: bool = True, digits: int = 3) -> str:
    if isinstance(value, float):
        s_val = f"{value:.{digits}f}"
    else:
        s_val = str(value)
    return term_paint_hex(s_val, ABBREVIATIONS.get(code, ABBREVIATIONS["SG"]).color, bold=True, enabled=color)


def console_abbreviation_legend(lang: str, codes: Sequence[str], color: bool = True) -> List[str]:
    lines: List[str] = []
    for code in codes:
        if code in ABBREVIATIONS:
            ab = ABBREVIATIONS[code]
            lines.append(f"{term_badge(code, color)} {text(lang, ab.name_en, ab.name_de)} — {text(lang, ab.explanation_en, ab.explanation_de)}")
        elif code in SEGMENT_COLORS:
            seg = next((s for s in SEGMENTS if s.code == code), None)
            if seg:
                lines.append(f"{term_segment(code, color)} {text(lang, seg.label_en, seg.label_de)} — {text(lang, seg.explanation_en, seg.explanation_de)}")
            else:
                lines.append(f"{term_segment(code, color)} {text(lang, 'Company segment code.', 'Unternehmenssegment-Code.')}")
    return lines



def ledger_entries_sorted(cosmos: 'Cosmos') -> List[LedgerEntry]:
    entries: List[LedgerEntry] = []
    for p in cosmos.all_planets():
        entries.extend(p.ledger)
    entries.sort(key=lambda r: (r.tick, r.universe, r.system, r.planet, r.company_id, r.identity_id))
    return entries


class ExplanationLimiter:
    """Limits repeated explanatory paragraphs in terminal output."""
    ...
    def allow(self, key: str) -> bool:
        ...
        if used >= self.max_repeat:
            return False
        self.counts[key] = used + 1
        return True


def limited_lines(limiter: ExplanationLimiter, key: str, lines: Sequence[str]) -> List[str]:
    return list(lines) if limiter.allow(key) else []


def code_legend_limited(lang: str, codes: Sequence[str], limiter: ExplanationLimiter, color: bool = True) -> List[str]:
    """Explain only codes present in the local calculation/art; each long explanation max twice."""
    out: List[str] = []
    seen: set = set()
    omitted = 0
    for code in codes:
        ...
        if code in ABBREVIATIONS:
            ab = ABBREVIATIONS[code]
            if limiter.allow('abbr:' + code):
                out.append(f"{term_badge(code, color)} {text(lang, ab.name_en, ab.name_de)} — {text(lang, ab.explanation_en, ab.explanation_de)}")
            else:
                # Keep the unit visible by color, but do not repeat the explanation paragraph.
                out.append(f"{term_badge(code, color)} {text(lang, ab.name_en, ab.name_de)}")
                omitted += 1
        elif code in SEGMENT_COLORS:
            seg = next((s for s in SEGMENTS if s.code == code), None)
            if limiter.allow('segabbr:' + code):
                if seg:
                    out.append(f"{term_segment(code, color)} {text(lang, seg.label_en, seg.label_de)} — {text(lang, seg.explanation_en, seg.explanation_de)}")
                else:
                    out.append(term_segment(code, color))
            else:
                out.append(term_segment(code, color) + (f" {text(lang, seg.label_en, seg.label_de)}" if seg else ""))
                omitted += 1
    if omitted and limiter.allow('omitted_abbrev_note'):
        out.append(text(lang,
                        f"{omitted} unit explanations in this block were already printed twice; the color code remains, but the explanatory paragraph is not repeated.",
                        f"{omitted} Einheitenerklärungen in diesem Block wurden bereits zweimal gedruckt; der Farbcode bleibt, aber der Erklärabsatz wird nicht wiederholt."))
    return out


def value_bar(value: float, code: str, color: bool = True, width: int = 20, scale: float = 1.0) -> str:
    frac = clamp(safe_div(value, max(scale, 1e-9), 0.0), 0.0, 1.0)
    filled = int(round(frac * width))
    h = ABBREVIATIONS.get(code, ABBREVIATIONS['SG']).color
    return term_paint_hex('█' * filled, h, bold=True, enabled=color) + term_paint_hex('░' * max(0, width - filled), '#404040', enabled=color)


def select_calculation_entries(cosmos: 'Cosmos', mode: str, limit: int) -> List[LedgerEntry]:
    entries = ledger_entries_sorted(cosmos)
    ...
    def add(rows: Iterable[LedgerEntry]) -> None:
        for r in rows:
            key = (r.tick, r.planet, r.company_id, r.identity_id, r.from_station, r.to_station)
            if key not in seen and len(selected) < limit:
                selected.append(r)
                seen.add(key)

    add(entries[:max(2, limit // 6)])
    add(sorted(entries, key=lambda r: r.esv, reverse=True)[:max(2, limit // 5)])
    add([r for r in entries if not r.verified][:max(2, limit // 5)])
    add([r for r in entries if r.debt_created > max(0.01, r.esv * 0.05)][:max(2, limit // 5)])
    add([r for r in entries if r.segment_code in ('FHV', 'RSO', 'CMP', 'DIG')][:max(2, limit // 5)])
    if len(selected) < limit:
        step = max(1, len(entries) // max(1, limit - len(selected)))
        add(entries[::step])
    selected.sort(key=lambda r: (r.tick, r.universe, r.system, r.planet, r.company_id))
    return selected[:limit]


def calculation_intro_text(lang: str, mode: str, selected: int, total: int, color: bool,
                           limiter: ExplanationLimiter) -> str:
    title = text(lang, '🧮 COLORFUL DEDUPLICATED CALCULATION OUTPUT', '🧮 BUNTE DEDUPLIZIERTE RECHNUNGSAUSGABE')
    body: List[str] = [text(lang,
        f"This terminal stream prints {selected} of {total} ledger calculations in mode '{mode}'. The Markdown report remains compact; detailed explanations and UTF-8 diagrams are terminal output.",
        f"Diese Terminalausgabe druckt {selected} von {total} Hauptbuch-Rechnungen im Modus '{mode}'. Der Markdown-Bericht bleibt kompakt; ausführliche Erklärungen und UTF-8-Diagramme erscheinen im Terminal.")]
    body.extend(limited_lines(limiter, 'calc_intro_discipline', [text(lang,
        'Repeated explanatory paragraphs are capped at two appearances. Numbers, formulas and colored unit badges may repeat because they are data, not explanatory filler.',
        'Wiederholte Erklärabsätze sind auf zwei Vorkommen begrenzt. Zahlen, Formeln und farbige Einheiten dürfen sich wiederholen, weil sie Daten sind, keine Erklärfüllung.')]))
    body.extend(limited_lines(limiter, 'calc_intro_units', [text(lang,
        'Only multi-letter units actually used in a calculation are explained in that calculation block.',
        'Nur mehrbuchstabige Einheiten, die in einer Rechnung tatsächlich vorkommen, werden im jeweiligen Rechnungsblock erklärt.')]))
    return console_box(title, body, color_hex='#FF00AF', color=color, width=132)


def render_single_calculation_output(r: LedgerEntry, idx: int, total: int, lang: str, color: bool = True,
                                     limiter: Optional[ExplanationLimiter] = None) -> str:
    limiter = limiter or ExplanationLimiter(2)
    seg = next((s for s in SEGMENTS if s.code == r.segment_code), None)
    seg_label = text(lang, seg.label_en, seg.label_de) if seg else r.segment_code
    before_body: List[str] = [
        text(lang,
             f"Calculation {idx}/{total}: {r.company_name} operates {term_badge('SEG', color)} {term_segment(r.segment_code, color)} ({seg_label}) over {r.segment_span}; it is {'partial' if r.partial_company else 'integrating'} rather than a monopoly over the whole cycle.",
             f"Rechnung {idx}/{total}: {r.company_name} bearbeitet {term_badge('SEG', color)} {term_segment(r.segment_code, color)} ({seg_label}) über {r.segment_span}; sie ist {'teilzyklisch' if r.partial_company else 'integrierend'} statt Monopol über den ganzen Kreislauf."),
        text(lang,
             f"Simulated movement: identity {r.identity_id}/{r.identity_label} moves from {station_label(r.from_station, lang, color)} to {station_label(r.to_station, lang, color)}. Only this actual segment earns value.",
             f"Simulierte Bewegung: Identität {r.identity_id}/{r.identity_label} bewegt sich von {station_label(r.from_station, lang, color)} nach {station_label(r.to_station, lang, color)}. Nur dieses tatsächliche Segment verdient Wert."),
    ]
    before_body.extend(limited_lines(limiter, 'partial_firm_reason', [text(lang,
        'Why this matters: the cosmos economy becomes realistic when fertilizer, harvest, digestion, compost, geology and validation firms each close bounded intervals and then hand identities onward.',
        'Warum das wichtig ist: Die Kosmoswirtschaft wird realistisch, wenn Dünger-, Ernte-, Verdauungs-, Kompost-, Geologie- und Prüfungsfirmen jeweils begrenzte Intervalle schließen und Identitäten weitergeben.')]))
    if seg:
        before_body.extend(limited_lines(limiter, 'segment_expl:' + r.segment_code, [text(lang, seg.explanation_en, seg.explanation_de)]))
    before_body.append(text(lang, 'Only multi-letter units used here:', 'Nur hier verwendete mehrbuchstabige Einheiten:'))
    before_body.extend(code_legend_limited(lang, ['SEG', r.segment_code, 'NCL', 'STA', 'END', 'REP', 'SG', 'UKE', 'MAT', 'QAL', 'USE', 'JUS', 'ESV', 'VSG', 'USG', 'TAX', 'SUB', 'CLD'], limiter, color))
    before = console_box(text(lang, '🌈 BEFORE THIS CALCULATION', '🌈 VOR DIESER RECHNUNG'), before_body,
                         color_hex=SEGMENT_COLORS.get(r.segment_code, '#FF7F00'), color=color, width=132)

    formula_body = [
        f"{term_badge('SG', color)} = ({term_badge('REP', color)}_end - {term_badge('REP', color)}_start) × {term_badge('NCL', color)} + {term_badge('END', color)} - {term_badge('STA', color)}",
        f"     = ({colored_number(r.end_repetition, 'REP', color, 0)} - {colored_number(r.start_repetition, 'REP', color, 0)}) × {colored_number(r.ncl, 'NCL', color, 0)} + {colored_number(r.to_station, 'END', color, 0)} - {colored_number(r.from_station, 'STA', color, 0)} = {colored_number(r.raw_sg, 'SG', color, 3)}",
        f"{term_badge('UKE', color)} = {term_badge('SG', color)} / {term_badge('NCL', color)} = {colored_number(r.raw_sg, 'SG', color, 3)} / {colored_number(r.ncl, 'NCL', color, 0)} = {colored_number(r.uke, 'UKE', color, 4)}",
        f"{term_badge('ESV', color)} = {term_badge('SG', color)} × {term_badge('MAT', color)} × {term_badge('QAL', color)} × {term_badge('USE', color)} × {term_badge('JUS', color)}",
        f"      = {colored_number(r.raw_sg, 'SG', color, 3)} × {colored_number(r.mat, 'MAT', color, 3)} × {colored_number(r.qal, 'QAL', color, 3)} × {colored_number(r.use, 'USE', color, 3)} × {colored_number(r.jus, 'JUS', color, 3)} = {colored_number(r.esv, 'ESV', color, 3)}",
        f"{station_label(r.from_station, lang, color)} {term_paint_hex('━━ ' + r.segment_code + ' ━━▶', SEGMENT_COLORS.get(r.segment_code, '#FF7F00'), bold=True, enabled=color)} {station_label(r.to_station, lang, color)}",
    ]
    formula_body.extend(limited_lines(limiter, 'arith_reading', [text(lang,
        'Arithmetic reading: SG is local distance, UKE is interplanetary normalization, and ESV weights the movement by matter, quality, use and justice.',
        'Arithmetische Lesart: SG ist lokale Distanz, UKE ist interplanetare Normalisierung, und ESV gewichtet die Bewegung mit Materie, Qualität, Nutzen und Gerechtigkeit.')]))
    formula = console_box(text(lang, '🔢 FORMULA AND MINI UTF-8 FLOW', '🔢 FORMEL UND MINI-UTF-8-FLUSS'), formula_body,
                          color_hex='#00D7FF', color=color, width=132)

    status = 'VSG' if r.verified else 'USG'
    net_company = r.minted_sg - r.tax + r.subsidy
    after_body = [
        text(lang,
             f"Observed branch: {term_paint_hex(r.outcome, status_color_hex(r.outcome, r.verified, r.debt_created), bold=True, enabled=color)}. {term_badge(status, color)} is the money-status unit for this result.",
             f"Beobachteter Zweig: {term_paint_hex(r.outcome, status_color_hex(r.outcome, r.verified, r.debt_created), bold=True, enabled=color)}. {term_badge(status, color)} ist die Geldstatus-Einheit dieses Ergebnisses."),
        f"{term_badge('VSG', color)}={colored_number(r.minted_sg, 'VSG', color)}   {term_badge('USG', color)}={colored_number(r.usg, 'USG', color)}   {term_badge('TAX', color)}={colored_number(r.tax, 'TAX', color)}   {term_badge('SUB', color)}={colored_number(r.subsidy, 'SUB', color)}   {term_badge('CLD', color)}={colored_number(r.debt_created, 'CLD', color)}",
        text(lang,
             f"Net company effect ≈ {net_company:.3f}. If proof or justice were weaker, the same movement could lose ESV or become CLD-heavy.",
             f"Nettoeffekt für das Unternehmen ≈ {net_company:.3f}. Bei schwächerem Beweis oder geringerer Gerechtigkeit könnte dieselbe Bewegung ESV verlieren oder CLD-lastig werden."),
    ]
    after_body.extend(limited_lines(limiter, 'calc_alt_scenarios', [text(lang,
        'Possible outcomes: verified regeneration mints VSG; weak proof leaves USG; pollution, low quality or unfair labor push the transition toward CLD.',
        'Mögliche Ausgänge: Verifizierte Regeneration prägt VSG; schwacher Beweis lässt USG; Verschmutzung, niedrige Qualität oder unfaire Arbeit drücken den Übergang Richtung CLD.')]))
    after = console_box(text(lang, '✅ AFTER THIS CALCULATION', '✅ NACH DIESER RECHNUNG'), after_body,
                        color_hex=status_color_hex(r.outcome, r.verified, r.debt_created), color=color, width=132)
    return before + '\n' + formula + '\n' + after


def print_calculation_output(cosmos: 'Cosmos', lang: str, color: bool, mode: str, limit: int,
                             limiter: Optional[ExplanationLimiter] = None) -> ExplanationLimiter:
    entries = ledger_entries_sorted(cosmos)
    selected = select_calculation_entries(cosmos, mode, max(1, limit))
    limiter = limiter or ExplanationLimiter(2)
    if mode == 'none':
        return limiter
    print(calculation_intro_text(lang, mode, len(selected), len(entries), color, limiter))
    for idx, entry in enumerate(selected, start=1):
        print(render_single_calculation_output(entry, idx, len(selected), lang, color=color, limiter=limiter))
    return limiter


# ---------------------------------------------------------------------------
# Very colorful UTF-8 art output
# ---------------------------------------------------------------------------


def planet_rows(cosmos: 'Cosmos') -> List['PlanetEconomy']:
    return list(cosmos.all_planets())


def choose_planet(cosmos: 'Cosmos') -> Optional['PlanetEconomy']:
    rows = planet_rows(cosmos)
    if not rows:
        return None
    return max(rows, key=lambda p: len(p.ledger) + sum(r.minted_sg for r in p.ledger) / 1000.0)


def art_panel(lang: str, color: bool, limiter: ExplanationLimiter, number: int, total: int,
              title_en: str, title_de: str, color_hex: str, why_en: str, why_de: str,
              codes: Sequence[str], art_lines: Sequence[str], eval_en: Sequence[str], eval_de: Sequence[str]) -> str:
    title = text(lang, title_en, title_de)
    body: List[str] = [term_paint_hex(text(lang, 'What is simulated and why:', 'Was simuliert wird und warum:'), color_hex, bold=True, enabled=color), text(lang, why_en, why_de), '']
    body.append(term_paint_hex(text(lang, 'Only multi-letter units used in this UTF-8 art:', 'Nur mehrbuchstabige Einheiten in dieser UTF-8-Grafik:'), '#FFFFFF', bold=True, enabled=color))
    body.extend(code_legend_limited(lang, codes, limiter, color))
    body.append('')
    body.append(term_paint_hex('UTF-8 ART', color_hex, bg_mode=True, bold=True, enabled=color))
    body.extend(art_lines)
    body.append('')
    body.append(term_paint_hex(text(lang, 'Evaluation with possible outcomes:', 'Auswertung mit möglichen Ausgängen:'), color_hex, bold=True, enabled=color))
    body.extend(eval_de if lang == 'de' else eval_en)
    return console_box(f"🖼️ {number}/{total} — {title}", body, color_hex=color_hex, color=color, width=138)


def art_native_ring(p: 'PlanetEconomy', lang: str, color: bool) -> List[str]:
    n = p.kind.cycle_length
    labels = [station_label(i, lang, color) for i in range(1, n + 1)]
    lines = [f"{term_badge('NCL', color)}={n}  {p.name}/{p.kind.code}"]
    for i in range(0, n, 6):
        lines.append('  ' + term_paint_hex(' ━▶ ', '#FF00AF', bold=True, enabled=color).join(labels[i:i+6]))
    lines.append('  ' + station_label(n, lang, color) + ' ' + term_paint_hex('━━ wrap ━━▶', '#00D7FF', bold=True, enabled=color) + ' ' + station_label(1, lang, color))
    return lines


def art_segment_lattice(cosmos: 'Cosmos', lang: str, color: bool) -> List[str]:
    counts: Dict[str, int] = {s.code: 0 for s in SEGMENTS}
    for p in planet_rows(cosmos):
        for c in p.companies:
            counts[c.segment_code] = counts.get(c.segment_code, 0) + 1
    max_count = max(list(counts.values()) + [1])
    rows = [['ORG', 'GRO', 'FHV', 'FOD', 'PRC'], ['DIG', 'CMP', 'RSO', 'LCI', 'GOV'], ['GEO', 'VUL', 'MIN', 'FUL', 'FUV']]
    lines: List[str] = []
    for row in rows:
        lines.append('  ' + term_paint_hex(' ──► ', '#FFD700', bold=True, enabled=color).join(f"{term_segment(c, color)}×{counts.get(c,0)}" for c in row))
    lines.append('')
    for code in sorted(counts, key=lambda c: (-counts[c], c))[:14]:
        h = SEGMENT_COLORS.get(code, '#FF7F00')
        filled = int(round(24 * counts[code] / max_count))
        lines.append(f"  {term_segment(code, color)} {term_paint_hex('█'*filled + '░'*(24-filled), h, bold=True, enabled=color)} {counts[code]}")
    return lines


def art_formula_machine(sample: Optional[LedgerEntry], lang: str, color: bool) -> List[str]:
    if sample is None:
        raw = mat = qal = use = jus = esv = 0.0
    else:
        raw, mat, qal, use, jus, esv = sample.raw_sg, sample.mat, sample.qal, sample.use, sample.jus, sample.esv
    return [
        f"{term_badge('SG', color)} {colored_number(raw, 'SG', color)}",
        f"   ├─× {term_badge('MAT', color)} {colored_number(mat, 'MAT', color)}",
        f"   ├─× {term_badge('QAL', color)} {colored_number(qal, 'QAL', color)}",
        f"   ├─× {term_badge('USE', color)} {colored_number(use, 'USE', color)}",
        f"   └─× {term_badge('JUS', color)} {colored_number(jus, 'JUS', color)}",
        f"      {term_paint_hex('══════════════════▶', '#FF00AF', bold=True, enabled=color)} {term_badge('ESV', color)} {colored_number(esv, 'ESV', color)}",
    ]


def art_verification_gate(entries: List[LedgerEntry], lang: str, color: bool) -> List[str]:
    esv = sum(r.esv for r in entries); vsg = sum(r.minted_sg for r in entries); usg = sum(r.usg for r in entries)
    tax = sum(r.tax for r in entries); sub = sum(r.subsidy for r in entries); cld = sum(r.debt_created for r in entries)
    return [
        f"{term_badge('ESV', color)} {fmt(esv)}",
        "      │",
        f"      ▼   {term_paint_hex('╔════ VALIDATOR / JUSTICE GATE ════╗', '#46F0F0', bold=True, enabled=color)}",
        f" pass ├────────► {term_badge('VSG', color)} {fmt(vsg)}",
        f" fail ├────────► {term_badge('USG', color)} {fmt(usg)}",
        f" risk └────────► {term_badge('CLD', color)} {fmt(cld)}",
        f" public steering: {term_badge('TAX', color)} {fmt(tax)}  {term_badge('SUB', color)} {fmt(sub)}",
    ]


def art_planet_dashboard(p: 'PlanetEconomy', lang: str, color: bool) -> List[str]:
    return [
        f"{p.name}/{p.kind.code}  {term_badge('NCL', color)}={p.kind.cycle_length}",
        f"{term_badge('SOF', color)} {value_bar(p.sof, 'SOF', color)} {p.sof:.3f}",
        f"{term_badge('WAT', color)} {value_bar(p.wat, 'WAT', color)} {p.wat:.3f}",
        f"{term_badge('MIC', color)} {value_bar(p.mic, 'MIC', color)} {p.mic:.3f}",
        f"{term_badge('HLT', color)} {value_bar(p.hlt, 'HLT', color)} {p.hlt:.3f}",
        f"{term_badge('POL', color)} {value_bar(p.pol, 'POL', color)} {p.pol:.3f}",
        f"{term_badge('INF', color)} {value_bar(p.inf, 'INF', color)} {p.inf:.3f}  {term_badge('TEC', color)} {value_bar(p.tec, 'TEC', color)} {p.tec:.3f}",
    ]


def art_food_chain(lang: str, color: bool) -> List[str]:
    return [
        f"{term_segment('GRO', color)} 3→5 {term_paint_hex('━━▶', '#3CB44B', bold=True, enabled=color)} {term_segment('FHV', color)} 5→7 {term_paint_hex('━━▶', '#FFE119', bold=True, enabled=color)} {term_segment('PRC', color)} 7→11 {term_paint_hex('━━▶', '#F58231', bold=True, enabled=color)} {term_segment('DIG', color)} 8→13",
        "     seedling/growth       fertilizer/harvest        market/bread          digestion/slurry",
        f"     {term_badge('SG', color)} distance accumulates while {term_badge('ESV', color)} depends on quality and justice",
    ]


def art_return_chain(lang: str, color: bool) -> List[str]:
    return [
        f"{station_label(13, lang, color)} {term_paint_hex('━━ compost ━━▶', '#F032E6', bold=True, enabled=color)} {term_segment('CMP', color)} {term_paint_hex('━━ chemistry ━━▶', '#BCF60C', bold=True, enabled=color)} {term_segment('RSO', color)} {term_paint_hex('━━ soil ━━▶', '#3CB44B', bold=True, enabled=color)} {station_label(19, lang, color)}",
        f"     {term_badge('MIC', color)} microbes convert residue into return value; failed return becomes {term_badge('CLD', color)}",
    ]


def art_vulcan_extension(lang: str, color: bool) -> List[str]:
    return [
        f"{station_label(20, lang, color)} {term_paint_hex('━━ lava/heat ━━▶', '#FF5F00', bold=True, enabled=color)} {station_label(21, lang, color)} {term_paint_hex('━━ ash/minerals ━━▶', '#A9A9A9', bold=True, enabled=color)} {station_label(22, lang, color)}",
        f"       {term_segment('VUL', color)}                                   {term_segment('MIN', color)}",
        f"       {term_badge('HEA', color)} feeds extra stations; {term_badge('NCL', color)}=22 before {term_badge('UKE', color)} normalization",
    ]


def art_uke_trade(cosmos: 'Cosmos', lang: str, color: bool) -> List[str]:
    earth = [p for p in planet_rows(cosmos) if p.kind.code == 'EarthType']; vulc = [p for p in planet_rows(cosmos) if p.kind.code == 'VulcanType']
    evsg = sum(sum(r.minted_sg for r in p.ledger) for p in earth); vvsg = sum(sum(r.minted_sg for r in p.ledger) for p in vulc)
    euke = sum(sum(r.uke for r in p.ledger) for p in earth); vuke = sum(sum(r.uke for r in p.ledger) for p in vulc)
    return [
        f"EARTH   {term_badge('NCL', color)}=20  {term_badge('VSG', color)}={fmt(evsg)}  {term_badge('UKE', color)}={fmt(euke)}",
        f"        {term_paint_hex('════════════ UKE TRADE BRIDGE ════════════', '#00D7FF', bold=True, enabled=color)}",
        f"VULCAN  {term_badge('NCL', color)}=22  {term_badge('VSG', color)}={fmt(vvsg)}  {term_badge('UKE', color)}={fmt(vuke)}",
        f"Check: Earth 4→17 pass4={original_loop_value('EarthType',4,17,4)} SG; Vulcan 4→17 pass4={original_loop_value('VulcanType',4,17,4)} SG",
    ]


def art_policy(cosmos: 'Cosmos', lang: str, color: bool) -> List[str]:
    entries = ledger_entries_sorted(cosmos)
    tax = sum(r.tax for r in entries); sub = sum(r.subsidy for r in entries); cld = sum(p.cld for p in planet_rows(cosmos))
    return [
        f"damage/extraction ──► {term_badge('TAX', color)} {fmt(tax)} ──► public repair fund",
        f"clean return/fairness ──► {term_badge('SUB', color)} {fmt(sub)} ──► compost, soil, water, justice",
        f"open pressure: {term_badge('CLD', color)} {fmt(cld)}",
    ]


def art_cpi_map(cosmos: 'Cosmos', lang: str, color: bool) -> List[str]:
    rows = planet_rows(cosmos)[:8]
    m = max([p.cpi for p in rows] + [1.0])
    return [f"{p.name:14s} {term_badge('CPI', color)} {value_bar(p.cpi, 'CPI', color, 18, m)} {p.cpi:.3f}  {term_badge('CLD', color)}={fmt(p.cld)}" for p in rows]


def art_identity(cosmos: 'Cosmos', lang: str, color: bool) -> List[str]:
    p = choose_planet(cosmos)
    if not p or not p.identities:
        return ['no identity available']
    ident = sorted(p.identities, key=lambda x: x.mass, reverse=True)[0]
    return [
        f"╭─ {ident.identity_id} / {ident.label}",
        f"│  planet={p.name}  type={p.kind.code}  {term_badge('NCL', color)}={p.kind.cycle_length}",
        f"│  station={station_label(ident.station, lang, color)}  {term_badge('REP', color)}={ident.repetition}",
        f"│  {term_badge('MAT', color)}={ident.mass:.3f} {term_badge('QAL', color)}={ident.quality:.3f} {term_badge('USE', color)}={ident.use:.3f} {term_badge('JUS', color)}={ident.justice:.3f}",
        f"╰─ tags={', '.join(ident.tags[:8])}",
    ]


def art_station_activity(cosmos: 'Cosmos', lang: str, color: bool) -> List[str]:
    counts: Dict[int, int] = {i: 0 for i in range(1, 23)}
    for r in ledger_entries_sorted(cosmos):
        counts[r.to_station] = counts.get(r.to_station, 0) + 1
    m = max(list(counts.values()) + [1])
    lines: List[str] = []
    for i in range(1, 23):
        filled = int(round(12 * counts[i] / m)) if counts[i] else 0
        lines.append(f"{station_label(i, lang, color):24s} {term_paint_hex('█'*filled + '░'*(12-filled), '#FF00AF', bold=True, enabled=color)} {counts[i]}")
    return lines


def build_art_panels(cosmos: 'Cosmos', lang: str, color: bool, limiter: ExplanationLimiter) -> List[str]:
    entries = ledger_entries_sorted(cosmos)
    sample = max(entries, key=lambda r: r.esv) if entries else None
    p = choose_planet(cosmos)
    if p is None:
        return []
    specs: List[Tuple[str, str, str, str, str, Sequence[str], Sequence[str], Sequence[str], Sequence[str]]] = []
    specs.append(('Native planet ring','Nativer Planetenring','#3CB44B',
        f"This art simulates {p.name}'s actual station loop. The native cycle length decides how SG is counted before any universal comparison.",
        f"Diese Grafik simuliert den tatsächlichen Stationsring von {p.name}. Die native Kreislauflänge entscheidet, wie SG gezählt wird, bevor universell verglichen wird.",
        ['NCL','SG'], art_native_ring(p, lang, color),
        ['Scenario A: the ring closes and money is backed by return.', 'Scenario B: some stations advance while return waits.', 'Scenario C: a broken ring converts output into open-loop debt.'],
        ['Szenario A: Der Ring schließt und Geld ist durch Rückführung gedeckt.', 'Szenario B: Einige Stationen laufen, während Rückführung wartet.', 'Szenario C: Ein gebrochener Ring verwandelt Output in offene Schuld.']))
    specs.append(('Partial-cycle company lattice','Teilzyklus-Unternehmensgitter','#FF7F00',
        'This art simulates that most companies operate only intervals such as 5→7 or 15→19, so the whole economy is a colorful handoff lattice.',
        'Diese Grafik simuliert, dass die meisten Unternehmen nur Intervalle wie 5→7 oder 15→19 bearbeiten; die ganze Wirtschaft ist daher ein buntes Übergabegitter.',
        ['SEG','ORG','GRO','FHV','FOD','PRC','DIG','CMP','RSO','LCI','GOV','GEO','VUL','MIN','FUL','FUV'], art_segment_lattice(cosmos, lang, color),
        ['Scenario A: many segment types create resilience.', 'Scenario B: one missing segment causes queues.', 'Scenario C: too many full-loop firms create monopoly pressure.'],
        ['Szenario A: Viele Segmenttypen schaffen Resilienz.', 'Szenario B: Ein fehlendes Segment erzeugt Warteschlangen.', 'Szenario C: Zu viele Vollkreislauffirmen erzeugen Monopoldruck.']))
    specs.append(('Stacked formula machine','Gestapelte Formelmaschine','#911EB4',
        'This art simulates how raw cycle distance becomes effective value only after material, quality, use and justice factors are multiplied in.',
        'Diese Grafik simuliert, wie rohe Kreislaufdistanz erst durch Material, Qualität, Nutzen und Gerechtigkeit wirksamer Wert wird.',
        ['SG','MAT','QAL','USE','JUS','ESV'], art_formula_machine(sample, lang, color),
        ['Scenario A: high QAL/JUS makes even short paths valuable.', 'Scenario B: high MAT scales value only if quality holds.', 'Scenario C: unfair work weakens ESV.'],
        ['Szenario A: Hohe QAL/JUS macht selbst kurze Wege wertvoll.', 'Szenario B: Hohe MAT skaliert Wert nur bei stabiler Qualität.', 'Szenario C: Unfaire Arbeit schwächt ESV.']))
    specs.append(('Verification gate','Prüfungstor','#46F0F0',
        'This art simulates why claimed value is not automatically spendable money; validators split ESV into VSG, USG and debt pressure.',
        'Diese Grafik simuliert, warum beanspruchter Wert nicht automatisch ausgebbares Geld ist; Validatoren teilen ESV in VSG, USG und Schulddruck.',
        ['ESV','VSG','USG','TAX','SUB','CLD'], art_verification_gate(entries, lang, color),
        ['Scenario A: strong proof mints VSG.', 'Scenario B: weak proof freezes value as USG.', 'Scenario C: risky transitions raise CLD but may fund repair through TAX.'],
        ['Szenario A: Starker Beweis prägt VSG.', 'Szenario B: Schwacher Beweis friert Wert als USG ein.', 'Szenario C: Riskante Übergänge erhöhen CLD, können aber über TAX Reparatur finanzieren.']))
    specs.append(('Planet dashboard','Planetenarmatur','#00AF87',
        f"This art simulates the living condition of {p.name}. Soil, water, microbes, health, infrastructure and technology determine whether money remains credible.",
        f"Diese Grafik simuliert den lebendigen Zustand von {p.name}. Boden, Wasser, Mikroben, Gesundheit, Infrastruktur und Technologie bestimmen, ob Geld glaubwürdig bleibt.",
        ['SOF','WAT','MIC','HLT','POL','INF','TEC','NCL'], art_planet_dashboard(p, lang, color),
        ['Scenario A: balanced resources absorb shocks.', 'Scenario B: high POL damages every loop.', 'Scenario C: strong INF/TEC rescues verification and return.'],
        ['Szenario A: Ausgeglichene Ressourcen puffern Schocks.', 'Szenario B: Hohe POL beschädigt jede Schleife.', 'Szenario C: Starke INF/TEC rettet Prüfung und Rückführung.']))
    specs.append(('Food and digestion chain','Nahrungs- und Verdauungskette','#3CB44B',
        'This art simulates the passage from seedling and fertilizer into food, market, digestion and slurry. Food wealth is unfinished without digestion and return.',
        'Diese Grafik simuliert den Weg von Keimling und Dünger zu Nahrung, Markt, Verdauung und Gülle. Nahrungswohlstand ist ohne Verdauung und Rückführung unfertig.',
        ['GRO','FHV','PRC','DIG','SG','ESV'], art_food_chain(lang, color),
        ['Scenario A: food and digestion both work, so consumption remains circular.', 'Scenario B: food works but digestion lags, so waste grows.', 'Scenario C: low justice in the chain lowers ESV.'],
        ['Szenario A: Nahrung und Verdauung funktionieren, Konsum bleibt kreisförmig.', 'Szenario B: Nahrung funktioniert, Verdauung hinkt, Abfall wächst.', 'Szenario C: Niedrige Gerechtigkeit in der Kette senkt ESV.']))
    specs.append(('Waste-to-soil return','Abfall-zu-Boden-Rückführung','#BCF60C',
        'This art simulates the anti-waste core: compost and return-soil firms turn excretion and residue into soil capacity.',
        'Diese Grafik simuliert den Anti-Abfall-Kern: Kompost- und Rückführungsbodenfirmen verwandeln Ausscheidung und Reste in Bodenkapazität.',
        ['CMP','RSO','MIC','CLD','VSG'], art_return_chain(lang, color),
        ['Scenario A: microbes and compost mint regenerative VSG.', 'Scenario B: chemistry is incomplete and value waits.', 'Scenario C: unsafe residue becomes CLD.'],
        ['Szenario A: Mikroben und Kompost prägen regeneratives VSG.', 'Szenario B: Chemie ist unvollständig und Wert wartet.', 'Szenario C: Unsichere Reste werden CLD.']))
    specs.append(('Vulcan extension','Vulkan-Erweiterung','#9A6324',
        'This art simulates why Vulcan-type planets extend to 22 stations: heat and ash must become mineral fertility before the loop is complete.',
        'Diese Grafik simuliert, warum Vulkan-Planeten bis 22 gehen: Hitze und Asche müssen zu mineralischer Fruchtbarkeit werden, bevor die Schleife vollständig ist.',
        ['HEA','NCL','VUL','MIN','UKE'], art_vulcan_extension(lang, color),
        ['Scenario A: ash becomes mineral fertility.', 'Scenario B: eruption without reintegration raises POL/CLD.', 'Scenario C: UKE prevents unfair Earth/Vulcan comparison.'],
        ['Szenario A: Asche wird mineralische Fruchtbarkeit.', 'Szenario B: Eruption ohne Rückintegration erhöht POL/CLD.', 'Szenario C: UKE verhindert unfairen Erde/Vulkan-Vergleich.']))
    specs.append(('Interplanetary trade bridge','Interplanetare Handelsbrücke','#00D7FF',
        'This art simulates trade between planet types. Earth and Vulcan keep local SG, but UKE is the bridge for fair comparison.',
        'Diese Grafik simuliert Handel zwischen Planetentypen. Erde und Vulkan behalten lokales SG, aber UKE ist die Brücke für fairen Vergleich.',
        ['NCL','VSG','UKE','SG'], art_uke_trade(cosmos, lang, color),
        ['Scenario A: UKE makes trade fair.', 'Scenario B: local VSG remains useful for local salaries.', 'Scenario C: raw SG comparison creates arbitrage.'],
        ['Szenario A: UKE macht Handel fair.', 'Szenario B: Lokales VSG bleibt für lokale Löhne nützlich.', 'Szenario C: Roher SG-Vergleich erzeugt Arbitrage.']))
    specs.append(('Tax-subsidy repair channel','Steuer-Subventions-Reparaturkanal','#E31A1C',
        'This art simulates public steering: damaging transitions pay taxes, while clean return work receives subsidies.',
        'Diese Grafik simuliert öffentliche Steuerung: Schädliche Übergänge zahlen Steuern, saubere Rückführungsarbeit erhält Subventionen.',
        ['TAX','SUB','CLD'], art_policy(cosmos, lang, color),
        ['Scenario A: TAX funds repair.', 'Scenario B: SUB accelerates regeneration.', 'Scenario C: bad verification misallocates public money.'],
        ['Szenario A: TAX finanziert Reparatur.', 'Szenario B: SUB beschleunigt Regeneration.', 'Szenario C: Schlechte Prüfung verteilt öffentliches Geld falsch.']))
    specs.append(('Circular price map','Kreislauf-Preiskarte','#33A02C',
        'This art simulates price pressure by planet. CPI rises when money, scarcity and debt diverge from closed-loop output.',
        'Diese Grafik simuliert Preisdruck pro Planet. CPI steigt, wenn Geld, Knappheit und Schuld von geschlossener Kreislaufleistung abweichen.',
        ['CPI','CLD'], art_cpi_map(cosmos, lang, color),
        ['Scenario A: low CPI/CLD is stable.', 'Scenario B: high CPI without CLD means excess money.', 'Scenario C: high CPI and CLD means monetary and ecological stress.'],
        ['Szenario A: Niedrige CPI/CLD ist stabil.', 'Szenario B: Hohe CPI ohne CLD bedeutet Geldüberschuss.', 'Szenario C: Hohe CPI und CLD bedeuten monetären und ökologischen Stress.']))
    specs.append(('Identity passport','Identitätspass','#A6CEE3',
        'This art simulates the bookkeeping core: every good is a traceable identity with station, repetition and factors.',
        'Diese Grafik simuliert den Buchhaltungskern: Jedes Gut ist eine verfolgbare Identität mit Station, Umlauf und Faktoren.',
        ['NCL','REP','MAT','QAL','USE','JUS'], art_identity(cosmos, lang, color),
        ['Scenario A: a complete passport lowers verification cost.', 'Scenario B: missing history leaves value as USG.', 'Scenario C: pollution follows the identity and prevents hidden disposal.'],
        ['Szenario A: Ein vollständiger Pass senkt Prüfungskosten.', 'Szenario B: Fehlende Geschichte lässt Wert als USG.', 'Szenario C: Verschmutzung folgt der Identität und verhindert versteckte Entsorgung.']))
    specs.append(('Station activity heatmap','Stationsaktivitäts-Heatmap','#FF00AF',
        'This art simulates where transformations landed in the ledger. It reveals active and underused parts of the cycle.',
        'Diese Grafik simuliert, wo Transformationen im Hauptbuch landeten. Sie zeigt aktive und untergenutzte Teile des Kreislaufs.',
        ['SEG'], art_station_activity(cosmos, lang, color),
        ['Scenario A: broad activity means the whole loop is alive.', 'Scenario B: harvest-heavy activity may outrun return.', 'Scenario C: strong 15–19 activity means repair capacity.'],
        ['Szenario A: Breite Aktivität bedeutet, dass die ganze Schleife lebt.', 'Szenario B: Erntelastige Aktivität kann Rückführung überholen.', 'Szenario C: Starke 15–19-Aktivität bedeutet Reparaturkapazität.']))

    # Segment-by-segment UTF-8 panels: this expands the atlas so the output contains
    # many colorful diagrams while still explaining only the units used in each panel.
    company_counts: Dict[str, int] = {seg.code: 0 for seg in SEGMENTS}
    for pp in planet_rows(cosmos):
        for cc in pp.companies:
            company_counts[cc.segment_code] = company_counts.get(cc.segment_code, 0) + 1
    for seg in SEGMENTS:
        seg_entries = [rr for rr in entries if rr.segment_code == seg.code]
        seg_vsg = sum(rr.minted_sg for rr in seg_entries)
        seg_usg = sum(rr.usg for rr in seg_entries)
        seg_cld = sum(rr.debt_created for rr in seg_entries)
        seg_esv = sum(rr.esv for rr in seg_entries)
        denom = max(1, len(seg_entries))
        avg_qal = sum(rr.qal for rr in seg_entries) / denom
        avg_use = sum(rr.use for rr in seg_entries) / denom
        avg_jus = sum(rr.jus for rr in seg_entries) / denom
        hx = SEGMENT_COLORS.get(seg.code, '#FF7F00')
        art_lines = [
            f"{term_segment(seg.code, color)} {seg.start}→{seg.end}  {text(lang, seg.label_en, seg.label_de)}",
            f"  companies={company_counts.get(seg.code,0)}  ledger_moves={len(seg_entries)}",
            f"  {term_badge('ESV', color)} {value_bar(seg_esv, 'ESV', color, 28, max(1.0, seg_esv, seg_vsg, seg_usg))} {fmt(seg_esv)}",
            f"  {term_badge('VSG', color)} {value_bar(seg_vsg, 'VSG', color, 28, max(1.0, seg_esv, seg_vsg, seg_usg))} {fmt(seg_vsg)}",
            f"  {term_badge('USG', color)} {value_bar(seg_usg, 'USG', color, 28, max(1.0, seg_esv, seg_vsg, seg_usg))} {fmt(seg_usg)}",
            f"  {term_badge('CLD', color)} {value_bar(seg_cld, 'CLD', color, 28, max(1.0, seg_cld))} {fmt(seg_cld)}",
            f"  {term_badge('QAL', color)}={avg_qal:.3f}  {term_badge('USE', color)}={avg_use:.3f}  {term_badge('JUS', color)}={avg_jus:.3f}",
            f"  {term_paint_hex('╰─ handoff slice ─────────────────────────────╯', hx, bold=True, enabled=color)}",
        ]
        specs.append((f'Segment unit {seg.code}', f'Segmenteinheit {seg.code}', hx,
            f'This UTF-8 art simulates the licensed segment {seg.code} ({seg.start}→{seg.end}) as its own economic unit. It shows whether this slice creates verified money, only claims, or cycle debt.',
            f'Diese UTF-8-Grafik simuliert das lizenzierte Segment {seg.code} ({seg.start}→{seg.end}) als eigene Wirtschaftseinheit. Sie zeigt, ob dieser Ausschnitt verifiziertes Geld, nur Ansprüche oder Kreislaufschuld erzeugt.',
            [seg.code, 'SEG', 'ESV', 'VSG', 'USG', 'CLD', 'QAL', 'USE', 'JUS'], art_lines,
            [f'Scenario A: {seg.code} has many companies and high VSG, so the handoff slice is healthy.',
             f'Scenario B: {seg.code} produces mostly USG, meaning real work exists but proof, quality or interfaces are weak.',
             f'Scenario C: {seg.code} creates CLD, so the segment must repair quality, justice or return safety before growth should continue.'],
            [f'Szenario A: {seg.code} hat viele Unternehmen und hohes VSG; der Übergabeausschnitt ist gesund.',
             f'Szenario B: {seg.code} produziert vor allem USG; reale Arbeit existiert, aber Beweis, Qualität oder Schnittstellen sind schwach.',
             f'Szenario C: {seg.code} erzeugt CLD; das Segment muss Qualität, Gerechtigkeit oder Rückführungssicherheit reparieren, bevor Wachstum sinnvoll ist.']))

    panels: List[str] = []
    total = len(specs)
    for i, (te, td, h, wy_en, wy_de, codes, art, ev_en, ev_de) in enumerate(specs, start=1):
        panels.append(art_panel(lang, color, limiter, i, total, te, td, h, wy_en, wy_de, codes, art, ev_en, ev_de))
    return panels


def print_utf8_art_gallery(cosmos: 'Cosmos', lang: str, color: bool, mode: str, limit: int,
                           limiter: ExplanationLimiter) -> None:
    if mode == 'none':
        return
    panels = build_art_panels(cosmos, lang, color, limiter)
    selected = panels if mode == 'all' else panels[:max(1, min(limit, len(panels)))]
    intro = console_box(text(lang, '🌌 VERY COLORFUL UTF-8 SIMULATION ATLAS', '🌌 SEHR BUNTER UTF-8-SIMULATIONSATLAS'), [
        text(lang,
             f"The following {len(selected)} UTF-8 panels are generated from the simulated economy itself: ledgers, planets, companies, stocks and cycle formulas.",
             f"Die folgenden {len(selected)} UTF-8-Panels werden aus der simulierten Wirtschaft selbst erzeugt: Hauptbuch, Planeten, Unternehmen, Bestände und Kreislaufformeln."),
        text(lang,
             'Each panel has explanation above, colorful art in the middle, and scenario evaluation below. Repeated explanations are capped at two appearances.',
             'Jedes Panel hat Erklärung darüber, bunte Grafik in der Mitte und Szenarioauswertung darunter. Wiederholte Erklärungen sind auf zwei Vorkommen begrenzt.'),
    ], color_hex='#FF00AF', color=color, width=138)
    print(intro)
    for panel in selected:
        print(panel)

def render_markdown_report_minimal(cosmos: 'Cosmos', lang: str = "en") -> str:
    """A lean file report. The detailed calculation and UTF-8 art teaching now belongs to the terminal output."""
    s = cosmos.summary()
    lines: List[str] = []
    lines.append("# " + text(lang, "Universe Circular Economy Simulation v4 — file report", "Universum-Kreislaufwirtschaft Simulation v4 — Datei-Bericht"))
    lines.append("")
    lines.append(text(lang,
                      "The detailed explanations before and after calculations are intentionally printed in the console output. This Markdown file is kept as a compact archive summary; use `ledger.csv` for the raw calculation ledger.",
                      "Die ausführlichen Erklärungen vor und nach den Rechnungen werden absichtlich in der Konsolenausgabe gedruckt. Diese Markdown-Datei bleibt eine kompakte Archiv-Zusammenfassung; nutze `ledger.csv` für das rohe Rechnungshauptbuch."))
    lines.append("")
    lines.append("## " + text(lang, "Cosmos summary", "Kosmos-Zusammenfassung"))
    lines.append("")
    lines.append("| Metric | Value |")
    lines.append("|---|---:|")
    rows = [
        ("Seed", s["seed"]),
        (text(lang, "Universes", "Universen"), s["universes"]),
        (text(lang, "Planets", "Planeten"), s["total_planets"]),
        ("EarthType", sum(u.get("earth_type_planets", 0) for u in s.get("universes_detail", []))),
        ("VulcanType", sum(u.get("vulcan_type_planets", 0) for u in s.get("universes_detail", []))),
        ("VSG", fmt(s["total_vsg"])),
        ("USG", fmt(s["total_usg"])),
        ("CLD", fmt(s["total_cld"])),
        (text(lang, "Partial company share", "Anteil Teilzyklus-Unternehmen"), f"{s['partial_company_share']:.2%}"),
    ]
    for k, v in rows:
        lines.append(f"| {k} | {v} |")
    lines.append("")
    lines.append("## " + text(lang, "Planet summaries", "Planeten-Zusammenfassungen"))
    lines.append("")
    lines.append("| Planet | Type | NCL | HLT | SOF | WAT | MIC | POL | CPI | VSG | USG | CLD | Partial firms |")
    lines.append("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for p in cosmos.all_planets():
        vsg = sum(r.minted_sg for r in p.ledger)
        usg = sum(r.usg for r in p.ledger)
        lines.append(f"| {p.name} | {p.kind.code} | {p.kind.cycle_length} | {p.hlt:.2f} | {p.sof:.2f} | {p.wat:.2f} | {p.mic:.2f} | {p.pol:.2f} | {p.cpi:.2f} | {fmt(vsg)} | {fmt(usg)} | {fmt(p.cld)} | {safe_div(sum(1 for c in p.companies if c.partial), len(p.companies), 0.0):.1%} |")
    lines.append("")
    lines.append("## " + text(lang, "Core formula checks", "Kernformel-Prüfungen"))
    lines.append("")
    lines.append("```text")
    lines.append(f"EarthType 4→17 pass 4 = {original_loop_value('EarthType', 4, 17, 4)} SG")
    lines.append(f"VulcanType 4→17 pass 4 = {original_loop_value('VulcanType', 4, 17, 4)} SG")
    lines.append("```")
    lines.append("")
    lines.append("## " + text(lang, "Files", "Dateien"))
    lines.append("")
    lines.append("- `ledger.csv` — " + text(lang, "full calculation ledger with formula fields", "vollständiges Rechnungshauptbuch mit Formelfeldern"))
    lines.append("- `companies.csv` — " + text(lang, "partial-cycle company structure", "Struktur der Teilzyklus-Unternehmen"))
    lines.append("- `snapshots.csv` — " + text(lang, "planet time series", "Planeten-Zeitreihen"))
    lines.append("- `final_state.json` — " + text(lang, "machine-readable final state", "maschinenlesbarer Endzustand"))
    lines.append("")
    return "\n".join(lines)
...
def print_console_summary(cosmos: Cosmos, lang: str, color: bool = True) -> None:
    s = cosmos.summary()
    print("\n" + ("=" * 72))
    print(text(lang, "Universe Circular Economy Simulation v4", "Universum-Kreislaufwirtschaft Simulation v4"))
    print("=" * 72)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PyPy3 simulation of the Universe Circular Economy v4 with partial-cycle companies, repetition-capped explanations, and colorful UTF-8 terminal art."
    )
    ...
    parser.add_argument("--report-dir", type=str, default="uce_report_v4", help="Directory for CSV/JSON/Markdown reports. Empty string disables files.")
    parser.add_argument("--explain-limit", type=int, default=24, help="Compatibility setting; Markdown is compact in v4; detailed explanations are in terminal output.")
    parser.add_argument("--calculation-output", choices=["none", "sample", "all"], default="sample", help="Print colorful before/formula/after blocks for calculations in the terminal. Default: sample. Use all for every calculation.")
    parser.add_argument("--calculation-limit", type=int, default=24, help="How many calculations to print when --calculation-output sample is used.")
    parser.add_argument("--art-output", "--utf8-art-output", dest="art_output", choices=["none", "sample", "all"], default="sample", help="Print very colorful UTF-8 art diagrams in the terminal. Default: sample. Alias: --utf8-art-output.")
    parser.add_argument("--art-limit", "--utf8-art-limit", dest="art_limit", type=int, default=28, help="How many UTF-8 art diagrams to print when --art-output sample is used. Alias: --utf8-art-limit. Default: 28.")
    parser.add_argument("--explanation-repeat-limit", "--repeat-explanations-max", "--max-explanation-repeat", dest="explanation_repeat_limit", type=int, default=2, help="Maximum number of times the same detailed explanation is printed. Default: 2. Aliases: --repeat-explanations-max, --max-explanation-repeat.")
...
def validate_args(args: argparse.Namespace) -> Optional[str]:
    ...
    if args.calculation_limit < 1:
        return "calculation-limit must be >= 1"
    if args.art_limit < 1:
        return "art-limit must be >= 1"
    if args.explanation_repeat_limit < 1:
        return "explanation-repeat-limit must be >= 1"
    return None
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    explanation_limiter = ExplanationLimiter(args.explanation_repeat_limit)
    print_calculation_output(cosmos, lang=args.lang, color=not args.no_color,
                             mode=args.calculation_output, limit=args.calculation_limit,
                             limiter=explanation_limiter)
    print_utf8_art_gallery(cosmos, lang=args.lang, color=not args.no_color,
                           mode=args.art_output, limit=args.art_limit,
                           limiter=explanation_limiter)
    ...
    if not args.no_files and args.report_dir:
        ...
        print("- final_report.md  # compact; detailed calculation and UTF-8 art explanations are in terminal output")
...
def v4_panel_handover(cosmos: 'Cosmos', lang: str, color: bool) -> str:
    trades = []
    for p in cosmos.all_planets():
    ...
    if recent:
        for tr in recent:
            art.append(term_paint_hex(f"{tr.seller_id[-7:]} ── identity:{tr.identity_id[-6:]} @ station {tr.station:02d} ──▶ {tr.buyer_id[-7:]}", "#FFFFFF", bold=True, enabled=color) +
                       f"  {ub('MAT', color)}{unum('MAT', tr.mass, color)}  {ub('ESV', color)}{unum('ESV', tr.value, color)}")
    else:
        art.append(term_paint_hex("No handover trades were recorded yet. Increase ticks or company count.", "#FF005F", bold=True, enabled=color))
    scenarios = [
        ("Smooth handover", "Glatte Übergabe", f"Identity moves from one {ub('SEG', color)} to the next without losing traceability."),
        ("Broken chain", "Gebrochene Kette", f"If the next firm is missing, material waits, quality falls and future {ub('ESV', color)} shrinks."),
        ("Speculative trade", "Spekulativer Handel", f"If the price outruns useful transformation, the ledger may show {ub('USG', color)} instead of verified money."),
    ]
    return v4_panel("Company handover conveyor", "Übergabe-Förderband der Unternehmen",
                    "The conveyor shows why partial-cycle companies can still form a full economy. They do not need to own the entire cycle; they need reliable handovers of identities between stations.",
                    "Das Förderband zeigt, warum Teilzyklus-Unternehmen trotzdem eine vollständige Wirtschaft bilden können. Sie müssen nicht den ganzen Kreislauf besitzen; sie brauchen zuverlässige Übergaben von Identitäten zwischen Stationen.",
                    ["SEG", "MAT", "ESV", "USG"], art, scenarios, lang, color, "#BCF60C")


def v4_panel_matrix(cosmos: 'Cosmos', lang: str, color: bool) -> str:
    t = v4_planet_totals(cosmos)
    rows = [
        ("BIOSPHERE", "SOF", "WAT", "MIC"),
        ("SOCIETY", "JIX", "INF", "TEC"),
        ("MONEY", "VSG", "CLD", "CPI"),
        ("PLANET", "HEA", "POL", "CLR"),
    ]
    art = [term_paint_hex("┌───────────┬────────────┬────────────┬────────────┐", "#FFFFFF", enabled=color)]
    art.append(term_paint_hex("│ DOMAIN    │  UNIT A    │  UNIT B    │  UNIT C    │", "#FFFFFF", bold=True, enabled=color))
    art.append(term_paint_hex("├───────────┼────────────┼────────────┼────────────┤", "#FFFFFF", enabled=color))
    for name, a, b, c in rows:
        art.append(f"│ {term_paint_hex(name.ljust(9), '#FFFFFF', bold=True, enabled=color)} │ {ub(a,color)} {unum(a,t.get(a,0),color)} │ {ub(b,color)} {unum(b,t.get(b,0),color)} │ {ub(c,color)} {unum(c,t.get(c,0),color)} │")
    art.append(term_paint_hex("└───────────┴────────────┴────────────┴────────────┘", "#FFFFFF", enabled=color))
    scenarios = [
        ("Green quadrant", "Grüner Quadrant", f"Strong biosphere plus strong society means high {ub('CLR', color)} and robust {ub('VSG', color)}."),
        ("Red quadrant", "Roter Quadrant", f"High {ub('POL', color)} and {ub('CLD', color)} reveal hidden extraction under the money surface."),
        ("Technical rescue", "Technische Rettung", f"High {ub('TEC', color)} can rescue bad material flows only if {ub('JIX', color)} and {ub('INF', color)} cooperate."),
    ]
    return v4_panel("Four-domain status matrix", "Vier-Domänen-Statusmatrix",
                    "This matrix compresses the simulation into biosphere, society, money and planet. It helps diagnose which layer explains the result of the economy.",
                    "Diese Matrix verdichtet die Simulation auf Biosphäre, Gesellschaft, Geld und Planet. Sie hilft zu erkennen, welche Schicht das Ergebnis der Wirtschaft erklärt.",
                    ["SOF", "WAT", "MIC", "JIX", "INF", "TEC", "VSG", "CLD", "CPI", "HEA", "POL", "CLR"], art, scenarios, lang, color, "#A6CEE3")


def v4_panel_paths(cosmos: 'Cosmos', lang: str, color: bool) -> str:
    entries = select_calculation_entries(cosmos, "sample", 8)
    art = []
    for r in entries:
        line = f"t{r.tick:03d} {ub(r.segment_code, color)} {st_tok(r.from_station, lang, color)}"
        jump = "═" * max(2, min(16, abs(r.to_station - r.from_station) + 2))
        line += term_paint_hex(jump + "▶", SEGMENT_COLORS.get(r.segment_code, "#FFFFFF"), bold=True, enabled=color)
        line += f"{st_tok(r.to_station, lang, color)}  {ub('SG', color)}{unum('SG', r.raw_sg, color)} {ub('ESV', color)}{unum('ESV', r.esv, color)}"
        if r.verified:
            line += " " + term_paint_hex("✓", "#00FF87", bold=True, enabled=color)
        else:
            line += " " + term_paint_hex("?", "#FFAF00", bold=True, enabled=color)
        art.append(line)
    if not art:
        art = [term_paint_hex("No paths exist yet. Increase --ticks.", "#FF005F", bold=True, enabled=color)]
    scenarios = [
        ("Short profitable jump", "Kurzer profitabler Sprung", f"A small {ub('SG', color)} can still matter if {ub('QAL', color)}, {ub('USE', color)} and {ub('JUS', color)} are strong."),
        ("Long weak jump", "Langer schwacher Sprung", f"A large {ub('SG', color)} can disappoint if quality factors reduce {ub('ESV', color)}."),
        ("Unverified path", "Unverifizierter Pfad", f"The path may be physically real but remain {ub('USG', color)} until proof is sufficient."),
    ]
    return v4_panel("Recent identity paths", "Jüngste Identitätspfade",
                    "This figure turns ledger rows into visible paths. It shows how individual identities move from start station to end station through specialized companies.",
                    "Diese Abbildung macht Buchungszeilen als Wege sichtbar. Sie zeigt, wie einzelne Identitäten durch spezialisierte Unternehmen von Startstation zu Endstation wandern.",
                    ["SG", "ESV", "QAL", "USE", "JUS", "USG"] + [r.segment_code for r in entries[:5]], art, scenarios, lang, color, "#F032E6")


def v4_panel_inflation(cosmos: 'Cosmos', lang: str, color: bool) -> str:
    planets = cosmos.all_planets()
    mx = max([p.cpi for p in planets] or [1.0])
    art = []
    for p in sorted(planets, key=lambda x: x.cpi, reverse=True)[:10]:
        art.append(term_paint_hex(f"{p.name:<18}", "#FFFFFF", bold=True, enabled=color) +
                   f" {ub('CPI', color)} {v4_short_bar('CPI', p.cpi, max(mx,1.0), color)}  {ub('MSI', color)}{unum('MSI', p._material_stock_index(), color)}  {ub('CLR', color)}{unum('CLR', p._closed_loop_rate(), color)}")
    scenarios = [
        ("Stable prices", "Stabile Preise", f"High {ub('MSI', color)} and {ub('CLR', color)} keep {ub('CPI', color)} near the base level."),
        ("Loop inflation", "Schleifeninflation", f"If money grows faster than material support, {ub('CPI', color)} rises."),
        ("Supply restoration", "Versorgungsreparatur", f"More soil, food and return infrastructure can lower pressure without deleting {ub('VSG', color)}."),
    ]
    return v4_panel("Inflation pressure constellation", "Inflationsdruck-Konstellation",
                    "This constellation links prices to material stocks and loop closure. Inflation is not treated as a purely monetary issue; it is also a broken-cycle issue.",
                    "Diese Konstellation verbindet Preise mit Materialbeständen und Kreislaufschluss. Inflation wird nicht rein monetär verstanden, sondern auch als Problem gebrochener Kreisläufe.",
                    ["CPI", "MSI", "CLR", "VSG"], art, scenarios, lang, color, "#FB9A99")


def v4_panel_atlas_summary(cosmos: 'Cosmos', lang: str, color: bool) -> str:
    t = v4_planet_totals(cosmos)
    art = [
        term_paint_hex("        ╭──────────── UNIVERSE / UNIVERSUM ────────────╮", "#FFFFFF", bold=True, enabled=color),
        f"        │ {ub('VSG', color)} {unum('VSG', t['VSG'], color)}   {ub('CLD', color)} {unum('CLD', t['CLD'], color)}   {ub('CLR', color)} {unum('CLR', t['CLR'], color)} │",
        term_paint_hex("╭───────┴──────╮         ╭──────────────╮        ╭──────┴────────╮", "#FFFFFF", enabled=color),
        f"│ {ub('SOF', color)} soil   │ ─────▶ │ {ub('SEG', color)} firms     │ ─────▶ │ {ub('UKE', color)} trade      │",
        f"│ {ub('MIC', color)} microbes│ ◀───── │ {ub('VSG', color)}/{ub('USG', color)} gate │ ◀───── │ {ub('NCL', color)} planets    │",
        term_paint_hex("╰──────────────╯         ╰──────────────╯        ╰───────────────╯", "#FFFFFF", enabled=color),
    ]
    scenarios = [
        ("Closed cosmos", "Geschlossener Kosmos", f"The best outcome has strong {ub('CLR', color)}, low {ub('CLD', color)} and trade normalized by {ub('UKE', color)}."),
        ("Fragmented cosmos", "Zersplitterter Kosmos", f"Many {ub('SEG', color)} firms without handovers create stranded identities."),
        ("Financial mirage", "Finanzielle Fata Morgana", f"High {ub('VSG', color)} with rising {ub('CLD', color)} is not true wealth; it is delayed repair."),
    ]
    return v4_panel("Whole-system UTF-8 synthesis", "Gesamtsystem-UTF-8-Synthese",
                    "The final figure summarizes the whole model: planets with native cycle lengths, partial companies, verification gates, material stocks, trade normalization and debt.",
                    "Die letzte Abbildung fasst das ganze Modell zusammen: Planeten mit nativer Kreislauflänge, Teilunternehmen, Verifikationstore, Materialbestände, Handelsnormalisierung und Schuld.",
                    ["VSG", "CLD", "CLR", "SOF", "MIC", "SEG", "USG", "UKE", "NCL"], art, scenarios, lang, color, "#FFD700")


def build_utf8_panels(cosmos: 'Cosmos', lang: str, color: bool) -> List[str]:
    builders = [
        v4_panel_cycle,
        v4_panel_segments,
        v4_panel_formula,
        ...
        v4_panel_paths,
        v4_panel_inflation,
        v4_panel_atlas_summary,
    ]
    return [fn(cosmos, lang, color) for fn in builders]


def print_utf8_art_atlas(cosmos: 'Cosmos', lang: str, color: bool, mode: str, limit: int) -> None:
    if mode == "none":
        return
    panels = build_utf8_panels(cosmos, lang, color)
    if mode == "summary":
        panels = panels[:max(0, limit)]
    print(console_box(text(lang, "VERY COLORFUL UTF-8 ECONOMIC ATLAS", "SEHR BUNTER UTF-8-WIRTSCHAFTSATLAS"), [
        text(lang,
             "The following figures are printed to the terminal, not to the Markdown report. Each figure explains only the multi-letter abbreviations used in that figure, and repeated explanatory patterns are limited.",
             "Die folgenden Abbildungen werden im Terminal ausgegeben, nicht im Markdown-Bericht. Jede Abbildung erklärt nur die mehrbuchstabigen Kürzel, die in genau dieser Abbildung vorkommen; wiederholte Erklärungsmuster sind begrenzt."),
        f"{ub('SEG', color)} {text(lang, 'partial-cycle companies', 'Teilzyklus-Unternehmen')}  {ub('SG', color)} {text(lang, 'loop money', 'Schleifen-Geld')}  {ub('CLD', color)} {text(lang, 'cycle debt', 'Kreislaufschuld')}"
    ], color_hex="#FF5FD7", color=color, width=122))
    for panel in panels:
        print(panel)
