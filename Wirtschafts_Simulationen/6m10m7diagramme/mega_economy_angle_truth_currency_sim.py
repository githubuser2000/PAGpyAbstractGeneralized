...
# ---------------------------------------------------------------------------
# UTF-8 art diagrams for the three-currency system
# ---------------------------------------------------------------------------
...
def _bar(value: float, max_value: float, width: int = 38, fill: str = "█", empty: str = "░") -> str:
    width = max(4, int(width))
    max_value = max(EPS, abs(max_value))
    n = int(round(clamp(abs(value) / max_value, 0.0, 1.0) * width))
    return fill * n + empty * (width - n)


def _signed_bar(value: float, lo: float = -1.0, hi: float = 1.0, width: int = 38) -> str:
    width = max(9, int(width))
    mid = width // 2
    value = clamp(value, lo, hi)
    span_left = max(EPS, abs(lo))
    span_right = max(EPS, abs(hi))
    cells = ["░"] * width
    cells[mid] = "┼"
    if value >= 0:
        n = int(round((value / span_right) * (width - mid - 1)))
        for i in range(mid + 1, min(width, mid + 1 + n)):
            cells[i] = "█"
    else:
        n = int(round((abs(value) / span_left) * mid))
        for i in range(max(0, mid - n), mid):
            cells[i] = "█"
    return "".join(cells)


def _sparkline(values: Sequence[float], width: int = 64) -> str:
    vals = [_as_float(v) for v in values if math.isfinite(_as_float(v))]
    if not vals:
        return "∅"
    ...
    if len(vals) > width:
        ...
        for i in range(width):
            ...
            reduced.append(mean(vals[a:b], vals[a]))
        vals = reduced
    lo = min(vals)
    hi = max(vals)
    if abs(hi - lo) <= EPS:
    ...
    for v in vals:
        ...
        out.append(SPARK_CHARS[clamp(idx, 0, len(SPARK_CHARS) - 1).__int__()])
    return "".join(out)


def _framed(title: str, body: str, width: int = 92) -> str:
    width = max(50, int(width))
    top = "╭" + "─" * (width - 2) + "╮"
    bottom = "╰" + "─" * (width - 2) + "╯"
    title_line = "│ " + title[: width - 4].ljust(width - 4) + " │"
...
def _angle_good_fraction_for(angle_deg: float, objective_good: float) -> float:
    return clamp(1.0 - angular_distance(angle_deg, objective_good) / 180.0, 0.0, 1.0)


def _angle_good_circumference_for(angle_deg: float, objective_good: float) -> float:
    return math.pi * _angle_good_fraction_for(angle_deg, objective_good)


def _synthetic_market_angle(last: Dict[str, float], objective_good: float) -> float:
    alignment = clamp(_as_float(last.get("angle_market_alignment", 0.0)), 0.0, 1.0)
    popularity = clamp(_as_float(last.get("angle_popularity_index", 0.0)), -1.0, 1.0)
    distance = (1.0 - alignment) * 180.0
    sign = 1.0 if popularity >= 0.0 else -1.0
    return norm_angle(objective_good + sign * distance)
...
def _draw_angle_compass(
    ...
    width = max(31, int(width))
    height = max(17, int(height))
    if width % 2 == 0:
        width += 1
    if height % 2 == 0:
        height += 1
    ...
    def point(angle: float, scale: float = 1.0) -> Tuple[int, int]:
        rad = math.radians(norm_angle(angle))
        x = int(round(cx + math.cos(rad) * rx * scale))
        y = int(round(cy - math.sin(rad) * ry * scale))
        return x, y
    ...
    for deg in range(0, 360, 2):
        x, y = point(deg)
        put(x, y, "○")
    # Axes: good/evil and popular/unpopular.
    for deg, ch in [(objective_good, "·"), (evil_angle, "·"), (popular_axis, "·"), (unpopular_axis, "·")]:
    ...
    for deg, ch in labels:
        x, y = point(deg, 0.92)
        put(x, y, ch)
    return "\n".join("".join(row).rstrip() for row in grid)


def _angle_axis_table(last: Dict[str, float], config: SimulationConfig, width: int) -> str:
    objective = _as_float(last.get("angle_objective_good_deg", config.angle_objective_good), config.angle_objective_good)
    evil = _as_float(last.get("angle_evil_deg", norm_angle(objective + 180.0)), norm_angle(objective + 180.0))
    popular = _as_float(last.get("angle_popular_axis_deg", norm_angle(objective + 90.0)), norm_angle(objective + 90.0))
    unpopular = _as_float(last.get("angle_unpopular_axis_deg", norm_angle(objective - 90.0)), norm_angle(objective - 90.0))
    rows = [
        ("GUT", objective),
        ("BÖSE", evil),
        ("BELIEBT", popular),
    ...
    lines = []
    lines.append("Begriff      Winkel   Gut/Böse     Beliebt/Unbeliebt   Güte-Fraktion   Teil-Halbumfang")
    lines.append("───────────  ──────   ─────────    ──────────────────   ─────────────   ───────────────")
    for label, angle in rows:
        good_coord = angle_axis_coordinate(angle, objective)
        pop_coord = angle_axis_coordinate(angle, popular)
        gf = _angle_good_fraction_for(angle, objective)
        circ = _angle_good_circumference_for(angle, objective)
        lines.append(
            f"{label:11s}  {angle:6.1f}°  {good_coord:9.3f}    {pop_coord:18.3f}   {gf:13.3f}   {circ:15.6f}"
        )
        lines.append(f"             Höhe    [{_bar(circ, math.pi, max(12, width // 3))}]  {circ:.6f} / π")
    market_angle = _synthetic_market_angle(last, objective)
    m_good = _angle_good_fraction_for(market_angle, objective)
    lines.append("")
    lines.append(f"Synthetischer aktueller Marktwinkel M ≈ {market_angle:.2f}°")
    lines.append(f"M-Güte-Höhe: [{_bar(m_good, 1.0, max(16, width // 2))}] {m_good:.3f}")
    lines.append(f"M-Gut/Böse:  [{_signed_bar(angle_axis_coordinate(market_angle, objective), -1.0, 1.0, max(16, width // 2))}] {angle_axis_coordinate(market_angle, objective):+.3f}")
    lines.append(f"M-Beliebt:   [{_signed_bar(angle_axis_coordinate(market_angle, popular), -1.0, 1.0, max(16, width // 2))}] {angle_axis_coordinate(market_angle, popular):+.3f}")
    return "\n".join(lines)


def _fiat_value_diagrams(summary: Dict[str, Any], metrics: List[Dict[str, float]], width: int, sector_limit: int) -> str:
    last = metrics[-1] if metrics else {}
    items = [
        ("BIP / GDP", _as_float(summary.get("final_gdp", last.get("gdp", 0.0)))),
        ("Gesamtkredit", _as_float(last.get("total_credit", 0.0))),
        ("Neukredit", _as_float(last.get("new_credit", 0.0))),
        ("Aktienindex", _as_float(last.get("stock_index", 0.0))),
        ("Wohnungspreis", _as_float(last.get("housing_price", 0.0))),
        ("Mietindex", _as_float(last.get("rent_index", 0.0))),
        ("Exporte", _as_float(last.get("exports", 0.0))),
        ("Importe", _as_float(last.get("imports", 0.0))),
        ("Staatsschuld", _as_float(last.get("gov_debt", 0.0))),
    ]
    maxv = max(1.0, max(abs(v) for _, v in items))
    lines = ["Gewöhnliche kapitalistische Wertmessung: Geldpreis × Menge, Bilanzsumme, Kredit, Output.", ""]
    for label, value in items:
        lines.append(f"{label:16s} [{_bar(value, maxv, max(18, width // 2))}] {fmt_money(value)}")
    lines.append("")
    lines.append("Zeitreihen-Sparklines:")
    for key, label in [("gdp", "GDP"), ("stock_index", "Aktien"), ("total_credit", "Kredit"), ("energy_price", "Energiepreis"), ("inflation", "Inflation")]:
        vals = [_as_float(m.get(key, 0.0)) for m in metrics]
        if vals:
            lines.append(f"{label:12s} { _sparkline(vals, max(24, width - 20)) }  min={_fmt_float(min(vals))} max={_fmt_float(max(vals))}")
    sector_rows = []
    for sector in SECTORS:
        key = sector.lower().replace(" ", "_")
        price = _as_float(last.get(f"price_{key}", 0.0))
        output = _as_float(last.get(f"output_{key}", 0.0))
        hhi_value = _as_float(last.get(f"hhi_{key}", 0.0))
        sector_rows.append((sector, price * output, price, output, hhi_value))
    sector_rows.sort(key=lambda x: x[1], reverse=True)
    lines.append("")
    lines.append("Sektorwerte Fiat = Preis × Output; HHI zeigt Marktkonzentration:")
    max_sector = max(1.0, max(abs(v) for _, v, _, _, _ in sector_rows))
    for sector, value, price, output, hhi_value in sector_rows[: max(1, sector_limit)]:
        lines.append(f"{sector:22s} [{_bar(value, max_sector, max(16, width // 3))}] value={_fmt_float(value)} price={price:.3f} output={output:.3f} hhi={hhi_value:.3f}")
    return "\n".join(lines)


def _truth_stack_example(width: int) -> str:
    bits = [0.5, 0.7, -1.0, 1.0, 1.0]
    labels = ["halb richtig", "fast richtig", "Lüge", "richtig", "richtig"]
    total = sum(bits)
    frac = rationalize(total).limit_denominator(1000)
    lines = []
    lines.append("Gestapelte Fuzzy-Wahrheitsbits als rationale Zahl:")
    lines.append("  0.5 + 0.7 - 1 + 1 + 1 = 2.2 = " + str(frac))
    lines.append("")
    for i, (b, lab) in enumerate(zip(bits, labels), 1):
        sign = "+" if b >= 0 else "−"
        lines.append(f"Bit {i:02d} {sign} [{_signed_bar(b, -1.0, 1.0, max(18, width // 2))}] {b:+.3f}  {lab}")
    lines.append("")
    lines.append("Stack-Höhe")
    stack_height = max(-3.0, min(3.0, total))
    lines.append(f"  [{_signed_bar(stack_height, -3.0, 3.0, max(18, width // 2))}] {total:+.3f} LOGOS-Wahrheitszahl")
    lines.append("  Verifikation/Falsifikation: jedes Audit fügt dem Prädikat ein Element hinzu: n → n+1 → n+2.")
    return "\n".join(lines)


def _truth_diagrams(summary: Dict[str, Any], metrics: List[Dict[str, float]], width: int) -> str:
    last = metrics[-1] if metrics else {}
    lines = [_truth_stack_example(width), ""]
    balances = [
        ("Haushalte", _as_float(summary.get("final_truth_household_mean", last.get("truth_household_mean", 0.0)))),
        ("Firmen", _as_float(summary.get("final_truth_firm_mean", last.get("truth_firm_mean", 0.0)))),
        ("Banken", _as_float(summary.get("final_truth_bank_mean", last.get("truth_bank_mean", 0.0)))),
        ("Staat", _as_float(summary.get("final_truth_government_balance", last.get("truth_government_balance", 0.0)))),
    ]
    max_abs = max(1.0, max(abs(v) for _, v in balances))
    lines.append("Aktuelle LOGOS-Balancen / Stack-Höhen:")
    for label, value in balances:
        lines.append(f"{label:12s} [{_signed_bar(value, -max_abs, max_abs, max(18, width // 2))}] {value:+.6f}")
    lines.append("")
    lines.append("LOGOS-Indizes:")
    idx_items = [
        ("Korrektheit", _as_float(last.get("truth_correctness_index", 0.0)), 1.0),
        ("Planetarer Stress", _as_float(last.get("truth_planetary_stress", 0.0)), 1.0),
        ("Cross-Resource-Drossel", _as_float(last.get("truth_cross_resource_throttle", 0.0)), 1.0),
        ("Modell-Konkurrenz", _as_float(last.get("truth_model_competition", 0.0)), 1.8),
        ("Audit-Druck", _as_float(last.get("truth_audit_pressure", 0.0)), 1.0),
    ]
    for label, value, maxv in idx_items:
        lines.append(f"{label:24s} [{_bar(value, maxv, max(18, width // 2))}] {value:.6f}")
    lines.append("")
    lines.append("Ressourcen-Kontingente mit Fuzzy-Wahrheitsbereich [-1, 1]:")
    lines.append("Ressource       Nutzung      Limit       Truth-Bar")
    lines.append("────────────    ─────────    ─────────   ─────────────────────────────────")
    for resource in RESOURCE_TYPES:
        use = _as_float(last.get(f"truth_resource_use_{resource}", 0.0))
        limit = max(EPS, _as_float(last.get(f"truth_resource_limit_{resource}", 0.0), 1.0))
        truth = _as_float(last.get(f"truth_resource_truth_{resource}", 1.0), 1.0)
        lines.append(f"{resource:12s}  {use:10.3f}  {limit:10.3f}  [{_signed_bar(truth, -1.0, 1.0, max(18, width // 2))}] {truth:+.3f}")
    return "\n".join(lines)


def _arete_diagrams(summary: Dict[str, Any], metrics: List[Dict[str, float]], config: SimulationConfig, width: int) -> str:
    last = metrics[-1] if metrics else {}
    objective = _as_float(last.get("angle_objective_good_deg", config.angle_objective_good), config.angle_objective_good)
    evil = _as_float(last.get("angle_evil_deg", norm_angle(objective + 180.0)), norm_angle(objective + 180.0))
    popular = _as_float(last.get("angle_popular_axis_deg", norm_angle(objective + 90.0)), norm_angle(objective + 90.0))
    unpopular = _as_float(last.get("angle_unpopular_axis_deg", norm_angle(objective - 90.0)), norm_angle(objective - 90.0))
    market_angle = _synthetic_market_angle(last, objective)
    compass = _draw_angle_compass(objective, config.angle_state_a_good, config.angle_state_b_good, evil, popular, unpopular, market_angle, width=min(61, max(41, width // 2 + 17)), height=23)
    lines = []
    lines.append(compass)
    lines.append("Legende: A=Land A gutmütig 0°, B=Land B gutmütig 90°, G=synthetisches Gut 45°, X=böse, P=beliebt, U=unbeliebt, M=aktueller Markt.")
    lines.append("")
    lines.append(_angle_axis_table(last, config, width))
    lines.append("")
    balances = [
        ("Haushalte", _as_float(summary.get("final_angle_household_mean", last.get("angle_household_mean", 0.0)))),
        ("Firmen", _as_float(summary.get("final_angle_firm_mean", last.get("angle_firm_mean", 0.0)))),
        ("Banken", _as_float(summary.get("final_angle_bank_mean", last.get("angle_bank_mean", 0.0)))),
        ("Staat", _as_float(summary.get("final_angle_government_balance", last.get("angle_government_balance", 0.0)))),
    ]
    max_abs = max(1.0, max(abs(v) for _, v in balances))
    lines.append("ARETE-Balancen: Nähe zum 45°-Guten wird als Teil des Halbkreisumfangs vermögenswirksam.")
    for label, value in balances:
        lines.append(f"{label:12s} [{_signed_bar(value, -max_abs, max_abs, max(18, width // 2))}] {value:+.6f}")
    lines.append("")
    idx_items = [
        ("Marktausrichtung zu 45°", _as_float(last.get("angle_market_alignment", 0.0)), 1.0),
        ("Beliebtheit", _as_float(last.get("angle_popularity_index", 0.0)), 1.0),
        ("Polarisierung", _as_float(last.get("angle_polarization_index", 0.0)), 1.0),
        ("Akteurs-Güte", _as_float(last.get("angle_mean_actor_goodness", 0.0)), 1.0),
        ("Moral-Arbitrage", _as_float(last.get("angle_moral_arbitrage_volume", 0.0)), max(1.0, _as_float(last.get("angle_moral_arbitrage_volume", 0.0)))),
    ]
    for label, value, maxv in idx_items:
        if label == "Beliebtheit":
            lines.append(f"{label:24s} [{_signed_bar(value, -1.0, 1.0, max(18, width // 2))}] {value:+.6f}")
        else:
            lines.append(f"{label:24s} [{_bar(value, maxv, max(18, width // 2))}] {value:.6f}")
    lines.append("")
    lines.append("ARETE-Zeitreihen:")
    for key, label in [("angle_market_alignment", "45°-Ausrichtung"), ("angle_popularity_index", "Beliebtheit"), ("angle_polarization_index", "Polarisierung"), ("angle_trade_volume", "Handelsvolumen"), ("angle_total_events", "Ereignisse")]:
        vals = [_as_float(m.get(key, 0.0)) for m in metrics]
        if vals:
            lines.append(f"{label:18s} { _sparkline(vals, max(24, width - 24)) }  min={_fmt_float(min(vals))} max={_fmt_float(max(vals))}")
    return "\n".join(lines)


def _tri_currency_market_matrix(summary: Dict[str, Any], metrics: List[Dict[str, float]], config: SimulationConfig, width: int, sector_limit: int) -> str:
    last = metrics[-1] if metrics else {}
    correctness = clamp(_as_float(last.get("truth_correctness_index", 0.0)), -1.0, 1.0)
    resource_stress = clamp(_as_float(last.get("truth_planetary_stress", 0.0)), 0.0, 1.0)
    lines = []
    lines.append("Jeder Marktgegenstand wird dreifach gelesen:")
    lines.append("  Fiat: gewöhnlicher Preis-/Outputwert")
    lines.append("  LOGOS: gestapelte Fuzzy-Korrektheit als rationale Wahrheitszahl")
    lines.append("  ARETE: Winkelgeld; Nähe zu 45° erzeugt Teil-Halbkreisumfang als Güte-Vermögen")
    lines.append("")
    lines.append("Marktgegenstand           Fiat-Wert        LOGOS-Bar                     ARETE-Winkel / Gütehöhe")
    lines.append("──────────────────────    ─────────────    ─────────────────────────    ─────────────────────────────")
    rows = []
    for sector in SECTORS:
        key = sector.lower().replace(" ", "_")
        ...
        output = _as_float(last.get(f"output_{key}", 0.0))
        hhi_value = _as_float(last.get(f"hhi_{key}", 0.0))
        fiat_value = price * output
        sector_angle = SECTOR_BASE_ANGLE.get(sector, config.angle_objective_good)
        # Stylized sector truth: correctness minus monopoly/resource penalties.
        sector_truth = clamp(correctness + 0.12 * (1.0 - hhi_value) - 0.35 * resource_stress * (1.0 if sector in {"Energy", "Transport", "Manufacturing", "Construction"} else 0.35), -1.0, 1.0)
        arete_height = _angle_good_circumference_for(sector_angle, config.angle_objective_good)
        rows.append((sector, fiat_value, sector_truth, sector_angle, arete_height))
    rows.sort(key=lambda r: r[1], reverse=True)
    max_fiat = max(1.0, max(abs(r[1]) for r in rows))
    for sector, fiat_value, sector_truth, sector_angle, arete_height in rows[: max(1, sector_limit)]:
        lines.append(
            f"{sector:22s}  {fiat_value:12.3f}    [{_signed_bar(sector_truth, -1.0, 1.0, 24)}] {sector_truth:+.2f}    {sector_angle:6.1f}° [{_bar(arete_height, math.pi, 14)}] {arete_height:.3f}"
        )
    lines.append("")
    lines.append("Fiat-Rang-Balken der gleichen Marktgegenstände:")
    for sector, fiat_value, _, _, _ in rows[: max(1, sector_limit)]:
        lines.append(f"{sector:22s} [{_bar(fiat_value, max_fiat, max(16, width // 3))}] {_fmt_float(fiat_value)}")
    return "\n".join(lines)


def build_utf8_currency_report(
    ...
    sector_limit = max(1, int(sector_limit))
    last = metrics[-1] if metrics else {}
    scenario = summary.get("scenario", config.scenario)
    fiat_name = getattr(config, "currency_name", "ECU")
    truth_name = summary.get("truth_currency_name", config.truth_currency_name)
    angle_name = summary.get("angle_currency_name", config.angle_currency_name)
    parts = []
    header = []
    header.append(f"Drei-Währungs-UTF-8-Diagrammbericht — Szenario: {scenario}")
    header.append(f"Währungen: Fiat={fiat_name}, Wahrheit={truth_name}, Winkel/Moral={angle_name}")
    header.append(f"Schritte={summary.get('steps', config.steps)}  Seed={summary.get('seed', config.seed)}  Population={summary.get('population', config.households)}  Firmen={summary.get('firms_initial', config.firms)}")
    header.append("Lesart: Fiat misst Marktwert; LOGOS misst verifizierbare Richtigkeit; ARETE misst Winkelnähe zum gemeinsam Guten.")
    parts.append(_framed("0. Berichtskopf", "\n".join(header), width))
    parts.append(_framed("1. ARETE-Kreis: Böse / Gut / Beliebt / Unbeliebt und Teil-Halbkreisumfang", _arete_diagrams(summary, metrics, config, width), width))
    parts.append(_framed("2. Fiat-Währung: gewöhnlicher Wert wie im Kapitalismus", _fiat_value_diagrams(summary, metrics, width, sector_limit), width))
    parts.append(_framed("3. LOGOS-Währung: Höhe gestapelter Fuzzy-Wahrheitsbits", _truth_diagrams(summary, metrics, width), width))
    parts.append(_framed("4. Dreifach-Markt-Matrix: Fiat × LOGOS × ARETE", _tri_currency_market_matrix(summary, metrics, config, width, sector_limit), width))
    welfare_lines = []
    welfare_items = [
        ("Welfare klassisch", _as_float(summary.get("final_welfare_index", last.get("welfare_index", 0.0))), 1.5),
        ("Truth-adjusted welfare", _as_float(summary.get("final_truth_adjusted_welfare", last.get("truth_adjusted_welfare", 0.0))), 2.0),
        ("Truth+Angle welfare", _as_float(summary.get("final_truth_angle_adjusted_welfare", last.get("truth_angle_adjusted_welfare", 0.0))), 2.5),
        ("Finanzstress", _as_float(summary.get("final_financial_stress", last.get("financial_stress", 0.0))), 1.0),
        ("Arbeitslosigkeit", _as_float(summary.get("final_unemployment", last.get("unemployment", 0.0))), 1.0),
        ("Emissionen", _as_float(summary.get("final_emissions", last.get("emissions", 0.0))), max(1.0, _as_float(summary.get("final_emissions", last.get("emissions", 0.0))))),
    ]
    for label, value, maxv in welfare_items:
        welfare_lines.append(f"{label:24s} [{_bar(value, maxv, max(22, width // 2))}] {value:.6f}")
    welfare_lines.append("")
    welfare_lines.append("Kerninterpretation:")
    welfare_lines.append("  Mehr Fiat allein heißt nicht besser wirtschaften. LOGOS fragt: Ist der Wert korrekt, verifiziert und planetarisch zulässig?")
    welfare_lines.append("  ARETE fragt zusätzlich: Liegt der Marktgegenstand näher am gemeinsamen Guten oder nur an Beliebtheit, Macht und Ausbeutung?")
    parts.append(_framed("5. Gesamtergebnis: kapitalistischer Wert vs. Richtigkeit vs. Gütewinkel", "\n".join(welfare_lines), width))
    return "\n\n".join(parts)


def build_utf8_comparison_report(summaries: Dict[str, Any], rows: List[Dict[str, Any]], config: SimulationConfig, width: int = 96) -> str:
    width = max(72, int(width))
    if not rows:
        return _framed("Scenario comparison", "No rows.", width)
    keys = [
        ("final_gdp", "GDP"),
        ("final_welfare_index", "Welfare"),
        ("final_truth_adjusted_welfare", "Truth-Welfare"),
        ("final_truth_angle_adjusted_welfare", "Truth+Angle"),
        ("final_truth_correctness_index", "LOGOS-Korrektheit"),
        ("final_angle_market_alignment", "ARETE-Ausrichtung"),
        ("final_financial_stress", "Stress"),
    ]
    lines = ["Szenariovergleich über die drei Währungen:", ""]
    for key, label in keys:
        vals = [abs(_as_float(row.get(key, 0.0))) for row in rows]
        maxv = max(1.0, max(vals))
        lines.append(label)
        for row in rows:
            scenario = str(row.get("scenario", "?"))[:22]
            value = _as_float(row.get(key, 0.0))
            if key in {"final_truth_correctness_index", "final_angle_market_alignment", "final_financial_stress"}:
                bar = _bar(value, 1.0, max(18, width // 2))
            else:
                bar = _bar(value, maxv, max(18, width // 2))
            lines.append(f"  {scenario:22s} [{bar}] {value:.6f}")
        lines.append("")
    return _framed("UTF-8 Drei-Währungs-Szenariovergleich", "\n".join(lines), width)



def _runtime_truth_predicate_diagram(world: "EconomicWorld", width: int, limit: int) -> str:
    if not getattr(world.config, "truth_market_enabled", False):
        return "LOGOS/Wahrheitsmarkt ist deaktiviert."
    preds = list(world.truth_market.predicate_log)
    lines: List[str] = []
    lines.append("Aktuelle echte Prädikat-Logs aus dem Simulationslauf; sortiert nach |currency_value|.")
    lines.append("Jede Zeile ist n-stellig: zusätzliche Verifikationen/Falsifikationen erhöhen n und depth.")
    lines.append("")
    if not preds:
        lines.append("Keine Prädikate im Log.")
        return "\n".join(lines)
    sample = sorted(preds[-min(len(preds), 1200):], key=lambda p: abs(rational_float(p.currency_value())), reverse=True)[: max(1, limit)]
    lines.append("pid     n  d  fuzzy    conf     value rational        Name / letzte Elemente")
    lines.append("──────  ─  ─  ───────  ───────  ────────────────────  ─────────────────────────────────────────")
    for pred in sample:
        value = pred.currency_value()
        elem = "; ".join(pred.elements[-2:]) if pred.elements else ""
        elem = elem.replace("\n", " ")
        if len(elem) > max(24, width - 82):
            elem = elem[: max(24, width - 83)] + "…"
        lines.append(
            f"{pred.pid:<6d}  {pred.arity():<1d}  {pred.depth:<1d}  "
            f"{rational_float(pred.fuzzy):+7.3f}  {rational_float(pred.confidence):7.3f}  "
            f"{str(value):>20s}  {pred.name} / {elem}"
        )
    deepest = max(preds, key=lambda p: (p.depth, p.arity(), abs(rational_float(p.currency_value()))))
    lines.append("")
    lines.append(f"Tiefstes Beispiel: pid={deepest.pid}, arity={deepest.arity()}, depth={deepest.depth}, parent={deepest.parent_pid}, value={deepest.currency_value()}")
    lines.append("Stack-Interpretation: currency_value = fuzzy × confidence × arity_weight; gestapelt wird rational addiert/subtrahiert.")
    return "\n".join(lines)


def _runtime_angle_event_diagram(world: "EconomicWorld", width: int, limit: int) -> str:
    if not getattr(world.config, "angle_market_enabled", False):
        return "ARETE/Winkelmarkt ist deaktiviert."
    am = world.angle_market
    am.configure_from_world(world)
    events = list(am.event_log)
    lines: List[str] = []
    lines.append("Echte Winkelereignisse aus dem Marktlog. G/X/P/U sind Teil-Halbkreisumfang-Höhen für Gut/Böse/Beliebt/Unbeliebt.")
    lines.append(f"Achsen: Gut={am.objective_good:.1f}°, Böse={am.evil_angle:.1f}°, Beliebt={am.popular_axis:.1f}°, Unbeliebt={am.unpopular_axis:.1f}°; Max-Höhe=π={math.pi:.6f}")
    lines.append("")
    if not events:
        lines.append("Keine Winkelereignisse im Log.")
        return "\n".join(lines)
    sample = sorted(events[-min(len(events), 1500):], key=lambda e: abs(e.amount) * (1.0 + abs(e.units)), reverse=True)[: max(1, limit)]
    lines.append("eid     t     θ object  units       G-H    X-H    P-H    U-H    Zweck / Währung")
    lines.append("──────  ────  ────────  ─────────   ─────  ─────  ─────  ─────  ───────────────────────────────")
    for e in sample:
        gh = _angle_good_circumference_for(e.object_angle, am.objective_good)
        xh = _angle_good_circumference_for(e.object_angle, am.evil_angle)
        ph = _angle_good_circumference_for(e.object_angle, am.popular_axis)
        uh = _angle_good_circumference_for(e.object_angle, am.unpopular_axis)
        purpose = e.purpose.replace("\n", " ")
        if len(purpose) > max(24, width - 82):
            purpose = purpose[: max(24, width - 83)] + "…"
        lines.append(
            f"{e.eid:<6d}  {e.time:<4d}  {e.object_angle:8.2f}  {e.units:+9.4f}   "
            f"{gh:5.2f}  {xh:5.2f}  {ph:5.2f}  {uh:5.2f}  {purpose} / {e.currency_context} amount={e.amount:.3f}"
        )
    lines.append("")
    lines.append("Lesart: Nähe zu 45° macht G-H groß; Nähe zu 225° macht X-H groß; P/U zeigen soziale Popularität statt moralischer Güte.")
    return "\n".join(lines)


def _runtime_distribution_diagram(world: "EconomicWorld", width: int) -> str:
    def quantile_block(name: str, values: Sequence[float]) -> List[str]:
        vals = [v for v in values if math.isfinite(float(v))]
        if not vals:
            return [f"{name}: ∅"]
        q10 = percentile(vals, 0.10, 0.0)
        q50 = percentile(vals, 0.50, 0.0)
        q90 = percentile(vals, 0.90, 0.0)
        hi = max(abs(q10), abs(q50), abs(q90), 1.0)
        return [
            f"{name} q10 [{_signed_bar(q10, -hi, hi, max(18, width // 3))}] {q10:+.4f}",
            f"{name} q50 [{_signed_bar(q50, -hi, hi, max(18, width // 3))}] {q50:+.4f}",
            f"{name} q90 [{_signed_bar(q90, -hi, hi, max(18, width // 3))}] {q90:+.4f}",
        ]
    lines: List[str] = []
    lines.append("Agentenverteilungen: dieselben Akteure besitzen Fiat-Cash, LOGOS-Stacks und ARETE-Winkelvermögen.")
    lines.append("")
    lines.extend(quantile_block("Haushalt Fiat-Cash ", [h.cash for h in world.households]))
    lines.extend(quantile_block("Haushalt LOGOS     ", [rational_float(h.truth_balance) for h in world.households]))
    lines.extend(quantile_block("Haushalt ARETE     ", [h.angle_balance - h.angle_debt for h in world.households]))
    lines.append("")
    alive_firms = [f for f in world.firms if f.alive()]
    lines.extend(quantile_block("Firma Fiat-Cash    ", [f.cash for f in alive_firms]))
    lines.extend(quantile_block("Firma LOGOS        ", [rational_float(f.truth_balance) for f in alive_firms]))
    lines.extend(quantile_block("Firma ARETE        ", [f.angle_balance - f.angle_debt for f in alive_firms]))
    return "\n".join(lines)


def build_utf8_currency_report_from_world(world: "EconomicWorld", width: int = 96, sector_limit: int = 14) -> str:
    summary = world.final_summary()
    report = build_utf8_currency_report(summary, world.metrics, world.config, width=width, sector_limit=sector_limit)
    extra = [
        _framed("6. Runtime-LOGOS: echte n-stellige Prädikate aus dem Lauf", _runtime_truth_predicate_diagram(world, width, sector_limit), width),
        _framed("7. Runtime-ARETE: echte Winkelereignisse aus dem Lauf", _runtime_angle_event_diagram(world, width, sector_limit), width),
        _framed("8. Agentenvergleich: Fiat-Cash × LOGOS-Stack × ARETE-Winkelvermögen", _runtime_distribution_diagram(world, width), width),
    ]
    return report + "\n\n" + "\n\n".join(extra)
...
def run_single(config: SimulationConfig, out_csv: str = "", out_json: str = "") -> Dict[str, Any]:
    ...
    return {"summary": summary, "metrics": metrics, "world": world}
...
def make_arg_parser() -> argparse.ArgumentParser:
    ...
    # UTF-8 diagram output knobs.
    p.add_argument("--diagrams", action="store_true", help="Print many UTF-8 diagrams for Fiat, LOGOS and ARETE after the run")
    p.add_argument("--diagram-out", type=str, default="", help="Write the UTF-8 diagram report to this text file")
    p.add_argument("--diagram-width", type=int, default=120, help="Visual width of UTF-8 diagram boxes")
    p.add_argument("--diagram-sectors", type=int, default=14, help="Number of sector rows in the diagram report")
    p.add_argument("--diagram-top", type=int, default=None, help="Alias/override for how many top rows, predicates and events to show in diagrams")
    return p
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.compare is not None and len(args.compare) > 0:
        ...
        if args.diagrams or args.diagram_out:
            report = build_utf8_comparison_report(result["summaries"], result["rows"], cfg, width=args.diagram_width)
            if args.diagrams:
                print("\n" + report)
            if args.diagram_out:
                write_text(args.diagram_out, report)
                print(f"UTF-8 diagrams written to: {args.diagram_out}")
        return 0
    ...
    if args.json:
        print(f"JSON written to: {args.json}")
    if args.diagrams or args.diagram_out:
        diagram_limit = args.diagram_top if args.diagram_top is not None else args.diagram_sectors
        report = build_utf8_currency_report_from_world(result["world"], width=args.diagram_width, sector_limit=diagram_limit)
        if args.diagrams:
            print("\n" + report)
        if args.diagram_out:
            write_text(args.diagram_out, report)
