...
## UTF-8-Art-Diagramme für alle drei Währungen

Diese Version kann nach dem Simulationslauf einen großen UTF-8-Diagrammbericht erzeugen. Der Bericht visualisiert alle drei Währungen:

1. **ARETE / Winkelwährung**: Gut, Böse, Beliebt und Unbeliebt als Kreisachsen. Die Güte wird als Teil des Halbkreisumfangs berechnet. Nähe zu `45°` erzeugt hohe Güte-Höhe, Nähe zu `225°` erzeugt Böse-Höhe, und die orthogonale Achse zeigt Beliebt/Unbeliebt.
2. **Fiat / gewöhnlicher kapitalistischer Wert**: GDP, Kredit, Aktienindex, Wohnungspreise, Import/Export, Sektorwerte, Sparkline-Zeitreihen.
3. **LOGOS / gestapelte Wahrheitsbits**: fuzzy Werte von `-1` bis `+1`, rational addierbar/subtrahierbar, inklusive Beispiel `0.5 + 0.7 - 1 + 1 + 1 = 2.2 = 11/5`.

Zusätzlich enthält der Bericht eine dreifache Markt-Matrix `Fiat × LOGOS × ARETE`, echte Runtime-Logs für n-stellige LOGOS-Prädikate, echte ARETE-Winkelereignisse und Agentenverteilungen über Fiat-Cash, LOGOS-Stackhöhe und ARETE-Winkelvermögen.

### Diagramme in eine Textdatei schreiben
...
pypy3 mega_economy_angle_truth_currency_sim.py \
  ...
  --scenario climate_transition \
  --policy green \
  --out climate_utf8.csv \
  --json climate_utf8_summary.json \
  --diagram-out climate_utf8_diagrams.txt \
  --diagram-width 120 \
  --diagram-top 18 \
  --quiet
```

### Diagramme direkt im Terminal ausgeben
...
### Relevante neue Parameter

```text
--diagrams          Diagrammbericht direkt ausgeben
--diagram-out PATH  Diagrammbericht in UTF-8-Textdatei schreiben
--diagram-width N   Breite der Diagrammrahmen
--diagram-sectors N Anzahl der Sektorzeilen im Bericht
--diagram-top N     Alias/Override für Anzahl Top-Prädikate, Top-Events und Top-Zeilen
```
