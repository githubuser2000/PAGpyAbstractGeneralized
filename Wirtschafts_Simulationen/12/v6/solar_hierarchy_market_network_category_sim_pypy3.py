...
def running_on_pypy() -> bool:
    return platform.python_implementation().lower().startswith("pypy")
...
class Category:
    __slots__ = ("name", "objects", "morphisms", "hom", "identities", "next_morphism_id", "composition_attempts", "composition_successes", "morphism_cap", "virtual_morphisms", "virtual_kind_counts", "object_cap", "virtual_objects")
    ...
    def __init__(self, name: str) -> None:
        ...
        self.morphism_cap = 10 ** 12
        self.virtual_morphisms = 0
        self.virtual_kind_counts: Dict[str, int] = defaultdict(int)
        self.object_cap = 10 ** 12
        self.virtual_objects = 0

    def add_object(self, obj: CategoryObject) -> None:
        if obj.id not in self.objects and len(self.objects) >= self.object_cap:
            self.virtual_objects += 1
            return
    ...
    def add_morphism(self, m: Morphism, is_identity: bool = False) -> None:
        if m.source not in self.objects or m.target not in self.objects:
            return
        if (not is_identity) and len(self.morphisms) >= self.morphism_cap:
            self.virtual_morphisms += 1
            self.virtual_kind_counts[m.kind] += 1
            return
    ...
    def new_morphism(self, source: int, target: int, name: str, kind: str, bundle: Optional[HierarchyBundle] = None, strength: float = 1.0, data: Optional[Dict[str, Any]] = None) -> Optional[Morphism]:
        if source not in self.objects or target not in self.objects:
            return None
        mid = self.allocate_morphism_id()
        m = Morphism(mid, source, target, name, kind, bundle, strength, data)
        if len(self.morphisms) >= self.morphism_cap:
            self.virtual_morphisms += 1
            self.virtual_kind_counts[kind] += 1
            return m
        self.add_morphism(m)
        return m
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        for k, v in self.virtual_kind_counts.items():
            kinds[k] = kinds.get(k, 0) + v
        return {"name": self.name, "objects": len(self.objects), "virtual_objects": self.virtual_objects, "morphisms": len(self.morphisms) + self.virtual_morphisms, "materialized_morphisms": len(self.morphisms), "virtual_morphisms": self.virtual_morphisms, "hom_sets": len(self.hom), "composition_attempts": self.composition_attempts, "composition_successes": self.composition_successes, "kind_counts": kinds}
...
class SolarNetworkCategoryEconomy:
    def __init__(self, profile: str = "demo", seed: int = 42, quiet: bool = False, safe_mode: Optional[bool] = None) -> None:
        ...
        self.quiet = quiet
        auto_safe = running_on_pypy() and profile in ("large", "huge")
        self.pypy_safe_mode = auto_safe if safe_mode is None else bool(safe_mode)
        self.python_impl = platform.python_implementation()
        self.safe_worker_node_cap = 10 ** 9
        self.safe_product_node_cap = 10 ** 9
        self.safe_privilege_node_cap = 10 ** 9
        self.safe_burden_node_cap = 10 ** 9
        self.safe_packet_cap = 10 ** 9
        if self.pypy_safe_mode:
            if profile == "large":
                self.safe_worker_node_cap = 3200
                self.safe_product_node_cap = 3200
                self.safe_privilege_node_cap = 1200
                self.safe_burden_node_cap = 1200
                self.safe_packet_cap = 9000
                self.cfg["network_packets"] = min(self.cfg["network_packets"], 4500)
                self.cfg["product_trades"] = min(self.cfg["product_trades"], 24000)
                self.cfg["privilege_trades"] = min(self.cfg["privilege_trades"], 1200)
                self.cfg["burden_trades"] = min(self.cfg["burden_trades"], 900)
                self.cfg["market_trades"] = min(self.cfg["market_trades"], 360)
            elif profile == "huge":
                self.safe_worker_node_cap = 6000
                self.safe_product_node_cap = 6000
                self.safe_privilege_node_cap = 1800
                self.safe_burden_node_cap = 1800
                self.safe_packet_cap = 16000
                self.cfg["network_packets"] = min(self.cfg["network_packets"], 8000)
                self.cfg["product_trades"] = min(self.cfg["product_trades"], 45000)
                self.cfg["privilege_trades"] = min(self.cfg["privilege_trades"], 2200)
                self.cfg["burden_trades"] = min(self.cfg["burden_trades"], 1600)
                self.cfg["market_trades"] = min(self.cfg["market_trades"], 700)
        self.tick = 0
        ...
        self.materialized_workers = 0
        self.materialized_products = 0
        self.materialized_privileges = 0
        self.materialized_burdens = 0
        self.virtual_graph_entities = 0
        self.routable_entity_ids: List[int] = []

        if self.pypy_safe_mode:
            self.econ_cat.morphism_cap = 52000 if profile == "large" else 90000
            self.net_cat.morphism_cap = 52000 if profile == "large" else 90000
            self.hier_cat.morphism_cap = 14000 if profile == "large" else 22000
            self.top_cat.morphism_cap = 3000
    ...
    def should_materialize_entity_graph(self, e: AccountEntity, network_role: str) -> bool:
        """Return whether an entity should receive a live network node/category object.

        In PyPy-safe mode the economic population remains large, but the graph is
        represented as a sampled materialized skeleton plus virtual counts. This
        prevents PyPy from carrying a huge live graph of queues, identities and
        morphisms while preserving the large market population itself.
        """
        if not self.pypy_safe_mode:
            return True
        if network_role in ("body", "governance", "defense", "country", "alliance", "company", "market"):
            return True
        if network_role == "worker":
            if self.materialized_workers < self.safe_worker_node_cap:
                self.materialized_workers += 1
                return True
            self.virtual_graph_entities += 1
            return False
        if network_role == "product":
            if self.materialized_products < self.safe_product_node_cap:
                self.materialized_products += 1
                return True
            self.virtual_graph_entities += 1
            return False
        if network_role == "privilege":
            if self.materialized_privileges < self.safe_privilege_node_cap:
                self.materialized_privileges += 1
                return True
            self.virtual_graph_entities += 1
            return False
        if network_role == "burden":
            if self.materialized_burdens < self.safe_burden_node_cap:
                self.materialized_burdens += 1
                return True
            self.virtual_graph_entities += 1
            return False
        return True
    ...
    def register_entity(self, e: AccountEntity, network_role: str = "economic") -> AccountEntity:
        ...
        if not self.should_materialize_entity_graph(e, network_role):
            return e
        ...
        self.routable_entity_ids.append(e.id)
    ...
    def build(self) -> None:
        if not self.quiet:
            print(self.output_intro())
            print(self.output_glossary())
    ...
    def emit_packet(self, source_entity: int, target_entity: int, kind: str, bundle: HierarchyBundle, morphism_id: Optional[int] = None, payload: Optional[Dict[str, Any]] = None, priority: int = 5) -> bool:
        if self.pypy_safe_mode and self.packets_created >= self.safe_packet_cap:
        ...
        if src_node is None or dst_node is None:
            ...
            return False
        route = self.network.route(src_node, dst_node, max_depth=8 if self.pypy_safe_mode else 18)
        if not route:
            self.route_failures += 1
            return False
    ...
    def trade_products(self) -> None:
        ...
        for _ in range(self.cfg["product_trades"]):
            ...
            morph = self.add_econ_morphism(buyer_id, p.id, "buy %s" % p.name[:32], "trade_product", price, strength=value, data={"market": m.id, "seller": p.owner_id}, create_packet=(not self.pypy_safe_mode or self.packets_created < self.safe_packet_cap), priority=min(12, p.min_level + 1))
    ...
    def trade_privileges(self) -> None:
        ...
        for _ in range(self.cfg["privilege_trades"]):
            ...
            if self.settle(buyer_id, issuer_id, price, prefer_level=p.level):
                ...
                m.volume += value
                self.trade_volume += value
                self.add_econ_morphism(issuer_id, buyer_id, "grant privilege %s" % domain, "privilege", price, value, {"privilege": p.id, "market": m.id}, create_packet=(not self.pypy_safe_mode or self.packets_created < self.safe_packet_cap), priority=9)
    ...
    def trade_burdens(self) -> None:
        ...
        for _ in range(self.cfg["burden_trades"]):
            ...
            if self.entities[comp_id].wallet.can_pay(price.to_number()) and taker.level > 1:
                ...
                m.volume += value
                self.trade_volume += value
                self.add_econ_morphism(comp_id, taker_id, "assign burden %s" % domain, "burden", price, value, {"burden": b.id, "market": m.id}, create_packet=(not self.pypy_safe_mode or self.packets_created < self.safe_packet_cap), priority=10)
    ...
    def trade_markets_as_assets(self) -> None:
        ...
        for _ in range(self.cfg["market_trades"]):
            ...
            if self.settle(buyer_id, seller_id, price, prefer_level=asset.level):
                ...
                self.add_econ_morphism(seller_id, buyer_id, "market control transfer %s" % asset.name[:32], "trade_market", price, value, {"exchange": ex.id, "market_asset": asset.id}, create_packet=(not self.pypy_safe_mode or self.packets_created < self.safe_packet_cap), priority=12)
    ...
    def manual_lift_helpers_media(self) -> None:
        ...
        for target_id in candidates[: max(6, len(candidates) // 18)]:
            ...
            self.manual_lifts += 1
            self.add_econ_morphism(issuer_id, target_id, "manual lift %s" % target.name[:32], "manual_lift", p.quote(), p.quote().to_number(), {"privilege": p.id}, create_packet=(not self.pypy_safe_mode or self.packets_created < self.safe_packet_cap), priority=11)
    ...
    def generate_background_network_packets(self) -> None:
        ...
        if self.pypy_safe_mode:
            sample_count = min(sample_count, max(0, self.safe_packet_cap - self.packets_created))
        all_entities = self.routable_entity_ids if self.routable_entity_ids else list(self.entities.keys())
    ...
    def build_report(self) -> str:
        return "Built: entities=%d countries=%d alliances=%d companies=%d workers=%d products=%d markets=%d nodes=%d virtual_graph_entities=%d channels=%d econ_morphisms=%d/%d net_morphisms=%d/%d safe_mode=%s python=%s" % (
            len(self.entities), len(self.countries), len(self.alliances), len(self.companies), len(self.workers), len(self.products), len(self.markets), len(self.network.nodes), self.virtual_graph_entities, len(self.network.channels), len(self.econ_cat.morphisms), len(self.econ_cat.morphisms) + self.econ_cat.virtual_morphisms, len(self.net_cat.morphisms), len(self.net_cat.morphisms) + self.net_cat.virtual_morphisms, self.pypy_safe_mode, self.python_impl
        )

    def tick_report(self) -> str:
        return "tick=%d volume=%sH product_sales=%d market_trades=%d packets(created/delivered/dropped)=%d/%d/%d queues=%d sem_blocked=%d sheaf_glued=%d safe_mode=%s" % (
            ...
            sum(ch.semaphore.blocked for ch in self.network.channels.values()), len(self.market_sheaf.glued), self.pypy_safe_mode
        )
    ...
    def report(self) -> Dict[str, Any]:
        ...
        return {
            "seed": self.seed, "profile": self.profile, "ticks": self.tick, "safe_mode": self.pypy_safe_mode, "python": self.python_impl,
            "counts": {"entities": len(self.entities), "graph_nodes": len(self.network.nodes), "virtual_graph_entities": self.virtual_graph_entities, "countries": len(self.countries), "alliances": len(self.alliances), "companies": len(self.companies), "workers": len(self.workers), "products": len(self.products), "markets": len(self.markets), "privileges": len(self.privileges), "burdens": len(self.burdens)},
    ...
    def output_intro(self) -> str:
        safe = "aktiv" if self.pypy_safe_mode else "inaktiv"
        return """
================================================================================
GESAMTERKLÄRUNG DER SIMULATION
================================================================================
Diese Simulation modelliert eine Sonnensystem-Wirtschaft, in der Hierarchie nicht
nur ein sozialer Rang ist, sondern die eigentliche Währung bildet. Jeder Mensch,
jedes Unternehmen, jedes Land, jede UN-ähnliche Organisation, jedes Produkt und
jeder Markt besitzt eine Hierarchieposition von L01 bis L12. Ein Handel ist daher
nicht bloß ein Geldtransfer, sondern ein Transfer strukturierter Hierarchie-Bündel:
Arbeitskraft, Kapital, Marktzugang, Privilegien, Lasten, Topologie-Rechte,
Semaphore-Rechte, Datenströme und Governance-Rechte werden als gewichtete Teile
eines Werts behandelt. Die Zahl H ist nur die abrechenbare Kompression dieses
Bündels.

Das Ziel ist nicht, eine reale Volkswirtschaft präzise vorherzusagen. Das Ziel ist,
ein großes künstliches System zu erzeugen, in dem man sehen kann, wie Hierarchie,
Märkte, Netzwerke, Warteschlangen, Duplex-Kommunikation, Semaphore, Kategorien,
Funktoren und Garben zusammenspielen. Märkte sind dabei doppelt: Auf ihnen wird
gehandelt, aber sie selbst können auch als Assets den Besitzer wechseln.

PyPy-Sicherheitsmodus: %s. Wenn dieser Modus aktiv ist, bleibt die Bevölkerung groß,
aber der Graph wird sparsamer materialisiert. Nicht jeder Arbeiter und nicht jedes
Produkt erhält dann einen eigenen Live-Netzwerkknoten mit Queues und Morphismen.
Der wirtschaftliche Akteur bleibt vorhanden; nur seine Netzwerk-/Kategorieabbildung
wird teilweise virtuell gezählt. Das verhindert PyPy-Segfaults durch zu große live
Objektgraphen.
""" % safe

    def output_glossary(self) -> str:
        return """
================================================================================
GLOSSAR DER KÜRZEL UND AUSGABEN
================================================================================
H: Hierarchy Units. Das ist keine gewöhnliche Währung, sondern die Zahl, die aus
einem Hierarchie-Bündel entsteht. Das Bündel selbst enthält Level und Elemente;
die Zahl H ist die Settlement-Kompression.

L01-L12: Die zwölf Hierarchiestufen. L01 steht für Überleben/Basis, L12 für Solar
Apex. Höhere Stufen haben stärkere Multiplikatoren und bedeuten Zugriff auf
knappere Produkte, Märkte, Privilegien und Governance-Rechte.

entity: Jede wirtschaftliche Entität: Arbeiter, Unternehmen, Produkt, Markt, Land,
Bündnis, Verteidigungsorganisation oder UN-ähnliche Organisation.

node: Netzwerk-Knoten. Eine Entity kann zusätzlich als Knoten im Datenstromnetz
auftreten. Im PyPy-safe Modus werden viele kleine Akteure virtuell statt als Live-
Knoten geführt.

channel: Verbindung zwischen zwei Netzwerkknoten. Channels besitzen Topologie,
Duplex-Modus, Queue-Disziplin, Bandbreite, Latenz und Semaphore.

FIFO: First-In-First-Out. Ältere Pakete werden zuerst verarbeitet.
LIFO: Last-In-First-Out. Neuere Pakete werden zuerst verarbeitet.
priority: Prioritätsqueue. Höhere Priorität wird früher verarbeitet.

half-duplex: Ein Channel kann pro Zeitschritt effektiv nur eine Richtung bedienen.
full-duplex: Beide Richtungen können parallel Daten übertragen.
simplex: Nur eine Richtung ist erlaubt.

semaphore: Kapazitätswächter eines Channels. Wenn alle Permits verbraucht sind,
werden Pakete blockiert oder fallen aus.

morphism: Gerichtete Beziehung in einer Kategorie. In der Simulation sind z. B.
Charter, Beschäftigung, Produktlistings, Markttransfers und Netzwerkflüsse
Morphismen.

hom-set: Menge der Morphismen zwischen zwei Objekten einer Kategorie.
functor: Strukturabbildung von einer Kategorie in eine andere, z. B. von Ökonomie
zu Netzwerk oder von Ökonomie zu Hierarchiestufen.
NT / natürliche Transformation: Vergleich zweier Funktoren; hier besonders Status-
Funktor gegen Access-/Entitlement-Funktor.

presheaf: Lokale Zustände, etwa Marktinformationen je Markt.
sheaf / Garbe: Lokale Zustände werden auf höheren Ebenen zusammengeklebt, wenn sie
kompatibel sind: Markt -> planetare UN -> Solar-UN.

AB/BA: Warteschlangenrichtung eines Channels.
Fly: Pakete, die gerade in Flug/Latenz sind.
Block: Semaphore-Blockierungen.
"""

    def explain_counts_before(self) -> str:
        return """
--------------------------------------------------------------------------------
ERKLÄRUNG VOR COUNTS
--------------------------------------------------------------------------------
Dieser Block zählt die Grundpopulation der Simulation. Wichtig ist die Trennung
zwischen wirtschaftlicher Existenz und graphischer Materialisierung. workers und
products zählen reale simulierte Akteure/Objekte. graph_nodes zählt nur diejenigen,
die zusätzlich einen Live-Netzwerkknoten erhalten. virtual_graph_entities zeigt,
wie viele Entitäten im PyPy-safe Modus wirtschaftlich existieren, aber graphisch
nur virtuell repräsentiert werden.
"""

    def interpret_counts_after(self, report: Dict[str, Any]) -> str:
        c = report["counts"]
        return """
AUSWERTUNG NACH COUNTS
--------------------------------------------------------------------------------
Die Simulation enthält %(entities)s Entitäten. Davon sind %(graph_nodes)s als Live-
Netzwerkknoten materialisiert und %(virtual)s graphisch virtuell. Daraus folgt:
Die Makroökonomie bleibt groß, aber das Netzwerk wird als repräsentativer Skeleton-
Graph geführt. Genau diese Entkopplung ist für PyPy wichtig, weil Millionen kleiner
Queue-/Morphismen-/Identity-Objekte sonst nicht mehr stabil sind.
""" % {"entities": c.get("entities"), "graph_nodes": c.get("graph_nodes"), "virtual": c.get("virtual_graph_entities")}

    def explain_trade_before(self) -> str:
        return """
--------------------------------------------------------------------------------
ERKLÄRUNG VOR TRADE
--------------------------------------------------------------------------------
Dieser Block misst den simulierten Austausch. volume_H ist die Summe der in H
komprimierten Hierarchie-Bündel. product_sales sind erfolgreiche Produktkäufe.
market_ownership_changes zählt Märkte, die selbst als Assets gehandelt wurden.
manual_lifts sind manuelle Statushebungen für schlecht messbare Helfer-, Medien-,
Wissens- oder Vertrauensleistungen.
"""

    def interpret_trade_after(self, report: Dict[str, Any]) -> str:
        t = report["trade"]
        return """
AUSWERTUNG NACH TRADE
--------------------------------------------------------------------------------
Das Handelsvolumen beträgt %(vol)s H. %(sales)s Produktverkäufe zeigen, dass die
Märkte nicht nur symbolisch existieren. %(markets)s Marktbesitzerwechsel zeigen den
entscheidenden Paradigmenpunkt: Märkte sind selbst handelbare Hierarchie-Assets.
%(lifts)s manuelle Lifts zeigen, dass nicht nur rohe Kaufkraft zählt, sondern auch
sozial schwer messbare Leistungen in die Hierarchie gehoben werden können.
""" % {"vol": t.get("volume_H"), "sales": t.get("product_sales"), "markets": t.get("market_ownership_changes"), "lifts": t.get("manual_lifts")}

    def explain_network_before(self) -> str:
        return """
--------------------------------------------------------------------------------
ERKLÄRUNG VOR NETWORK
--------------------------------------------------------------------------------
Der Netzwerkblock beschreibt die Datenstrom-Architektur. nodes sind Live-Knoten,
channels sind Verbindungen, components sind getrennte Zusammenhangskomponenten.
degree_min/mean/max zeigen, wie stark Knoten vernetzt sind. Viele Komponenten sind
kein Fehler: ein großer Teil der Wirtschaft bleibt lokal oder virtuell, während
UNs, Länder, Unternehmen und Märkte als Backbone agieren.
"""

    def interpret_network_after(self, report: Dict[str, Any]) -> str:
        n = report["network"]
        p = n.get("packets", {})
        return """
AUSWERTUNG NACH NETWORK
--------------------------------------------------------------------------------
Das Netzwerk hat %(nodes)s Knoten, %(channels)s Channels und %(components)s
Komponenten. Ein hoher Maximalgrad (%(degmax)s) bedeutet, dass einzelne Hubs wie UNs
oder große Märkte stark zentral sind. Pakete created/delivered/dropped =
%(created)s/%(delivered)s/%(dropped)s. Daraus kann man ableiten, ob der
Datenstrom tragfähig ist oder ob Queues, Routen und Semaphore zum Flaschenhals
werden.
""" % {"nodes": n.get("nodes"), "channels": n.get("channels"), "components": n.get("components"), "degmax": n.get("degree_max"), "created": p.get("created"), "delivered": p.get("delivered"), "dropped": p.get("dropped")}

    def explain_category_before(self) -> str:
        return """
--------------------------------------------------------------------------------
ERKLÄRUNG VOR CATEGORY
--------------------------------------------------------------------------------
Kategorien fassen Struktur statt Einzelpreise. Objects sind Dinge wie Akteure,
Netzwerkknoten, Hierarchiestufen oder Topologietypen. Morphisms sind gerichtete
Beziehungen. materialized_morphisms sind live gespeicherte Beziehungen;
virtual_morphisms werden gezählt, aber nicht als vollständiges Objekt gehalten.
Das ist ein Kerntrick des PyPy-safe Modus.
"""

    def interpret_category_after(self, report: Dict[str, Any]) -> str:
        ec = report["category"]["economic_category"]
        nc = report["category"]["network_category"]
        return """
AUSWERTUNG NACH CATEGORY
--------------------------------------------------------------------------------
Die ökonomische Kategorie enthält %(eo)s Objekte und %(em)s Morphismen; die
Netzwerkkategorie enthält %(no)s Objekte und %(nm)s Morphismen. Wenn virtuelle
Morphismen auftauchen, bedeutet das keine fehlende Simulation, sondern eine
komprimierte Darstellung. Die Kategorie sagt: Welche Beziehungen existieren? Der
Markt sagt: Welche davon werden real gehandelt? Das Netzwerk sagt: Welche davon
können Daten transportieren?
""" % {"eo": ec.get("objects"), "em": ec.get("morphisms"), "no": nc.get("objects"), "nm": nc.get("morphisms")}

    def visual_explained(self, title: str, body: str, interpretation: str) -> str:
        return """
--------------------------------------------------------------------------------
ERKLÄRUNG VOR UTF-8-ARTEFAKT: %s
--------------------------------------------------------------------------------
%s

%s

AUSWERTUNG NACH UTF-8-ARTEFAKT: %s
--------------------------------------------------------------------------------
%s
""" % (title, body, body if body.startswith("═") or body.startswith("#") else "", title, interpretation)

    def final_outcome_summary(self, report: Dict[str, Any]) -> str:
        return """
================================================================================
ABSCHLUSS: MEHRERE MÖGLICHE AUSGÄNGE DER ZUFALLSSIMULATION
================================================================================
1. Konzentrationspfad: Zufallswerte können dazu führen, dass einige Länder,
Unternehmen oder Märkte sehr früh viele erfolgreiche Trades erhalten. Dann steigen
sie weiter, weil höhere Hierarchie besseren Zugang zu Märkten und Produkten gibt.
Das System tendiert dann zu starken Hubs.

2. Pluraler Marktpfad: Wenn Marktbesitzerwechsel und Produktkäufe breiter verteilt
sind, entstehen mehrere konkurrierende Zentren. Das ist stabiler, aber weniger
schnell koordiniert.

3. Sozialer Korrekturpfad: Wenn manuelle Lifts viele Pflege-, Medien-, Bildungs-
oder Vertrauensakteure treffen, wird reine Kapitaldominanz abgeschwächt. Das kann
Gemeinwohl stabilisieren, birgt aber immer die Gefahr manueller Fehlbewertung.

4. Netzengpasspfad: Wenn Semaphore blockieren, Queues wachsen oder viele Pakete
fallen, ist nicht der Marktpreis das Problem, sondern die Kommunikations- und
Topologiearchitektur. Dann braucht das System mehr Bandbreite, bessere Routing-
Hubs oder andere Queue-Disziplinen.

5. Virtueller-Skeleton-Pfad: Im PyPy-safe Modus ist die Auswertung bewusst zweistufig:
wirtschaftlich existieren viele Akteure, graphisch wird nur ein repräsentativer
Teil live geführt. Schlüsse über Marktvolumen und Hierarchiestufen bleiben stark;
Schlüsse über einzelne Worker-zu-Worker-Netzwege sind dagegen als Sampling zu lesen.

Ausgeglichen betrachtet ist ein guter Lauf nicht zwingend der mit maximalem
Volumen. Ein guter Lauf ist einer, in dem Handel, Statusmobilität, Marktwechsel,
Netzwerkdurchsatz und manuelle Korrekturen nicht auseinanderlaufen.
"""
    ...
    def print_report(self, json_path: Optional[str] = None, include_visuals: bool = True) -> None:
        ...
        print("=" * 96)
        print("seed=%s profile=%s ticks=%s safe_mode=%s python=%s" % (report["seed"], report["profile"], report["ticks"], report.get("safe_mode"), report.get("python")))
        print(self.explain_counts_before())
        print("Counts:")
        for k, v in report["counts"].items():
            print("  %-30s %s" % (k, v))
        print(self.interpret_counts_after(report))
        print(self.explain_trade_before())
        print("Trade:")
        for k, v in report["trade"].items():
            print("  %-30s %s" % (k, v))
        print(self.interpret_trade_after(report))
        print(self.explain_network_before())
        print("Network:")
        for k in ["nodes", "channels", "components", "degree_min", "degree_mean", "degree_max"]:
            print("  %-30s %s" % (k, report["network"][k]))
        print(self.interpret_network_after(report))
        print(self.explain_category_before())
        print("Category:")
        for key in ["economic_category", "network_category", "hierarchy_category", "topology_category"]:
            c = report["category"][key]
            print("  %-30s objects=%s(+%s virtual) morphisms=%s(%s materialized) hom_sets=%s" % (key, c.get("objects"), c.get("virtual_objects", 0), c.get("morphisms"), c.get("materialized_morphisms", c.get("morphisms")), c.get("hom_sets")))
        print(self.interpret_category_after(report))
        ...
        print(self.final_outcome_summary(report))
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    p.add_argument("--safe-mode", action="store_true", help="force PyPy-safe sparse graph mode even on CPython")
    p.add_argument("--unsafe-full-graph", action="store_true", help="disable automatic PyPy-safe sparse graph mode")
    # Compatibility flags from the earlier process-parallel variant.
    # They are accepted so old commands keep running. In PyPy-safe mode the script
    # intentionally avoids post-build process spawning because fork/exec from a
    # huge PyPy heap was one of the segfault triggers.
    p.add_argument("--processes", type=int, default=0, help="accepted for compatibility; safe mode uses sparse graph instead of process fan-out")
    p.add_argument("--no-parallel", action="store_true", help="accepted for compatibility")
    p.add_argument("--live-process-cap", type=int, default=0, help="accepted for compatibility")
    p.add_argument("--debug-stages", action="store_true", help="accepted for compatibility")
    p.add_argument("positional_ticks", nargs="?", type=int, help="optional positional tick count, e.g. --profile large 1")
    args = p.parse_args(argv)
    if args.positional_ticks is not None:
        args.ticks = args.positional_ticks
    return args


def main(argv: Sequence[str]) -> int:
    args = parse_args(argv)
    forced_safe = True if args.safe_mode else (False if args.unsafe_full_graph else None)
    sim = SolarNetworkCategoryEconomy(profile=args.profile, seed=args.seed, quiet=args.quiet, safe_mode=forced_safe)
    sim.build()
