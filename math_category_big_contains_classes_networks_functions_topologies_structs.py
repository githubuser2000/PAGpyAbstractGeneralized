...
# full_extended_mathematical_category.py
...
class Morphism:
    ...
    def __call__(self, x: Any) -> Any:
        if self.fn is None:
            raise TypeError(f"Morphismus {self.name!r} ist rein symbolisch und nicht ausführbar.")
        return self.fn(x)
    ...
    def __str__(self) -> str:
        return (
            ...
            f" -> {self.target.name}<{self.target.kind.name}> [{self.kind.name}]"
        )
        ...
        for g in self.source_category.morphisms.values():
            for f in self.source_category.morphisms.values():
                if f.target == g.source:
                    comp = self.source_category.compose(g.name, f.name, register=False)
                    try:
                    ...
                    rhs = self.target_category.compose(Fg.name, Ff.name, register=False)
                    if lhs.source != rhs.source or lhs.target != rhs.target:
                        raise FunctorError(
...
class NaturalTransformation:
    ...
    def validate(self) -> None:
        ...
        if F.source_category is not G.source_category:
            raise NaturalTransformationError(
                "Funktoren haben unterschiedliche Quellkategorien."
            )
        if F.target_category is not G.target_category:
            raise NaturalTransformationError(
                "Funktoren haben unterschiedliche Zielkategorien."
            )
        ...
        for f in C.morphisms.values():
            try:
                ...
                Gf = G.map_morphism(f)
                Ff = F.map_morphism(f)
            except (NaturalTransformationError, FunctorError):
                continue

            left = D.compose(Gf.name, eta_a.name, register=False)
            right = D.compose(eta_b.name, Ff.name, register=False)
...
class MathematicalCategory:
    ...
    def add_object(
        ...
        *,
        ...
        obj = CatObject(name=name, kind=kind, payload=payload, tags=frozenset(tags or ()))
    ...
    def ensure_morphism(
        ...
        *,
        ...
        return self.add_morphism(name, source, target, fn, metadata=metadata, kind=kind)
    ...
    def _validate_morphism_kind_compatibility(self, morph: Morphism) -> None:
        ...
        if k in topo and (s != DomainKind.TOPOLOGY or t != DomainKind.TOPOLOGY):
            raise DomainCompatibilityError(
                f"{k.name} verlangt TOPOLOGY -> TOPOLOGY, nicht {s.name} -> {t.name}."
            )
        if k in net and (s != DomainKind.NETWORK or t != DomainKind.NETWORK):
            raise DomainCompatibilityError(
                f"{k.name} verlangt NETWORK -> NETWORK, nicht {s.name} -> {t.name}."
            )
        if k in cls and (s != DomainKind.CLASS or t != DomainKind.CLASS):
            raise DomainCompatibilityError(
                f"{k.name} verlangt CLASS -> CLASS, nicht {s.name} -> {t.name}."
            )
        if k in fun and (s != DomainKind.FUNCTION or t != DomainKind.FUNCTION):
            raise DomainCompatibilityError(
                f"{k.name} verlangt FUNCTION -> FUNCTION, nicht {s.name} -> {t.name}."
            )
        if k in struct and (s != DomainKind.STRUCT or t != DomainKind.STRUCT):
            raise DomainCompatibilityError(
    ...
    def _validate_payload_level_constraints(self, morph: Morphism) -> None:
        ...
        if k == MorphismKind.CLASS_INHERITANCE:
            ...
            if isinstance(src, StructData) and isinstance(tgt, StructData):
                if not tgt.field_names().issubset(src.field_names()):
                    raise DomainCompatibilityError(
                        f"STRUCT_PROJECTION {morph.name!r}: Ziel-Felder sind keine Teilmenge."
                    )
    ...
    # ---------------------------------------------------------------------
    # Bequeme Morphismuskonstruktoren
    # ---------------------------------------------------------------------

    def add_continuous_map(self, name: str, source: Union[str, CatObject], target: Union[str, CatObject], *, fn=None, metadata=None) -> Morphism:
        return self.add_morphism(name, source, target, fn, metadata=metadata, kind=MorphismKind.CONTINUOUS_MAP)

    def add_network_homomorphism(self, name: str, source: Union[str, CatObject], target: Union[str, CatObject], *, fn=None, metadata=None) -> Morphism:
        return self.add_morphism(name, source, target, fn, metadata=metadata, kind=MorphismKind.NETWORK_HOMOMORPHISM)

    def add_class_inheritance(self, name: str, source: Union[str, CatObject], target: Union[str, CatObject], *, metadata=None) -> Morphism:
        return self.add_morphism(name, source, target, None, metadata=metadata, kind=MorphismKind.CLASS_INHERITANCE)

    def add_function_map(self, name: str, source: Union[str, CatObject], target: Union[str, CatObject], *, fn=None, metadata=None) -> Morphism:
        return self.add_morphism(name, source, target, fn, metadata=metadata, kind=MorphismKind.FUNCTION_MAP)

    def add_struct_projection(self, name: str, source: Union[str, CatObject], target: Union[str, CatObject], *, fn=None, metadata=None) -> Morphism:
        return self.add_morphism(name, source, target, fn, metadata=metadata, kind=MorphismKind.STRUCT_PROJECTION)

    def add_translation(self, name: str, source: Union[str, CatObject], target: Union[str, CatObject], *, fn=None, metadata=None) -> Morphism:
        return self.add_morphism(name, source, target, fn, metadata=metadata, kind=MorphismKind.TRANSLATION)
    ...
    def is_identity(self, morphism_name: str) -> bool:
        ...
        return m.source == m.target and self._identities.get(m.source.name) == m.name
    ...
    def _compose_morphism_kinds(self, g: Morphism, f: Morphism) -> MorphismKind:
        ...
        if g.source.kind == g.target.kind == f.source.kind == f.target.kind:
            ...
            if dom == DomainKind.NETWORK:
                return MorphismKind.NETWORK_HOMOMORPHISM
            if dom == DomainKind.CLASS:
                return MorphismKind.CLASS_EMBEDDING
            if dom == DomainKind.FUNCTION:
                return MorphismKind.FUNCTION_COMPOSITION
            if dom == DomainKind.STRUCT:
                if g.kind == MorphismKind.STRUCT_PROJECTION or f.kind == MorphismKind.STRUCT_PROJECTION:
    ...
    def register_isomorphism_pair(self, f_name: str, g_name: str) -> None:
    ...
    def register_product(self, left_obj, right_obj, product_obj, proj_left, proj_right) -> None:
    ...
    def get_product(self, left_obj, right_obj) -> Dict[str, Union[CatObject, Morphism]]:
        A = self.get_object(left_obj) if isinstance(left_obj, str) else left_obj
        B = self.get_object(right_obj) if isinstance(right_obj, str) else right_obj
        key = (A.name, B.name)
    ...
    def pair(self, f, g, *, name: Optional[str] = None, fn: Optional[Callable[[Any], Any]] = None) -> Morphism:
        fm = self.get_morphism(f) if isinstance(f, str) else f
        gm = self.get_morphism(g) if isinstance(g, str) else g
    ...
    def register_coproduct(self, left_obj, right_obj, coproduct_obj, inj_left, inj_right) -> None:
        A = self.get_object(left_obj) if isinstance(left_obj, str) else left_obj
        B = self.get_object(right_obj) if isinstance(right_obj, str) else right_obj
    ...
    def copair(self, f, g, *, name: Optional[str] = None, fn: Optional[Callable[[Any], Any]] = None) -> Morphism:
        fm = self.get_morphism(f) if isinstance(f, str) else f
        gm = self.get_morphism(g) if isinstance(g, str) else g
    ...
    def register_equalizer(self, f, g, equalizer_obj, inclusion) -> None:
        fm = self.get_morphism(f) if isinstance(f, str) else f
        gm = self.get_morphism(g) if isinstance(g, str) else g
    ...
    def register_pullback(self, f, g, pullback_obj, proj_left, proj_right) -> None:
        fm = self.get_morphism(f) if isinstance(f, str) else f
        gm = self.get_morphism(g) if isinstance(g, str) else g
    ...
    def register_pushout(self, f, g, pushout_obj, inj_left, inj_right) -> None:
        fm = self.get_morphism(f) if isinstance(f, str) else f
        gm = self.get_morphism(g) if isinstance(g, str) else g
    ...
    def register_exponential(self, base_obj, exponent_obj, exponential_obj, evaluation) -> None:
    ...
    def opposite(self, name: Optional[str] = None) -> "MathematicalCategory":
        ...
        for obj in self.objects.values():
            op.add_object(obj.name, kind=obj.kind, payload=obj.payload, tags=obj.tags)
    ...
    def subcategory(
        ...
        *,
        ...
        for obj_name in object_set:
            ...
            sub.add_object(obj.name, kind=obj.kind, payload=obj.payload, tags=obj.tags)
    ...
    def clone(self, name: Optional[str] = None) -> "MathematicalCategory":
        ...
        for obj in self.objects.values():
            cloned.add_object(obj.name, kind=obj.kind, payload=obj.payload, tags=obj.tags)
    ...
    def compose_and_apply(self, g: Union[str, Morphism], f: Union[str, Morphism], value: Any) -> Any:
    ...
    def End(
        ...
        *,
        ...
        o = self.get_object(obj) if isinstance(obj, str) else obj
        return self.Mor(o, o, name=name, metadata=metadata, overwrite=overwrite, kind=kind)
    ...
    def describe(self) -> str:
        ...
        for obj in sorted(self.objects.values(), key=lambda x: x.name):
            lines.append(f"  - {obj.name}: {obj.kind.name}")
...
if __name__ == "__main__":
    C = MathematicalCategory("MixedStructures")
    ...
    C.add_continuous_map("collapse", T1, T2)
    ...
    C.add_network_homomorphism("net_map", N1, N2)

    Base = C.add_class_object(
        "BaseClass",
        fields={"id"},
        methods={"save"},
    )
    User = C.add_class_object(
        "UserClass",
        fields={"id", "name", "email"},
        methods={"save", "login"},
        base_classes={"BaseClass"},
    )
    C.add_class_inheritance("user_inherits_base", User, Base)

    S_full = C.add_struct_object(
        "PersonStruct",
        fields={"id": "int", "name": "str", "email": "str"},
    )
    S_small = C.add_struct_object(
        "IdNameStruct",
        fields={"id": "int", "name": "str"},
    )
    C.add_struct_projection("drop_email", S_full, S_small)

    F1 = C.add_function_object(
        "IntToInt",
        domain_name="int",
        codomain_name="int",
    )
    F2 = C.add_function_object(
        "IntToStr",
        domain_name="int",
        codomain_name="str",
    )
    C.add_function_map("compile_fn", F1, F2)

    C.add_translation("struct_to_class", S_full, User)
